...
ELSE ASSIGN
   ...
   */
   lcSpoolDir = "/store/riftp/mach2/yts2128/spool/"
   lcOutGDir  = "/store/riftp/mach2/yts2128/outgoing/".
...
FOR EACH RoamOper NO-LOCK:
   ...
   CASE plTest:
      ...
      WHEN FALSE THEN DO:
         IF pcPLMN NE "" AND (pcPLMN NE RoamOper.PLMN) THEN NEXT.
...
/*         RUN pFillTempTable1. */
   ...
   ASSIGN
      lcCDRFile   = lcSpoolDir + (IF plTest THEN "TD" ELSE "CD") +
                    "ESPXF" + lcRoamOper + "_" + STRING(YEAR(pdaDate1),"9999") + STRING(MONTH(pdaDate1),"99") + STRING(DAY(pdaDate1),"99") + "_" + STRING(liFileSeq,"99999") + ".asc".
...
PROCEDURE pTap3File:
   ...
   FOR EACH ttFields NO-LOCK:
      ...
      CASE ttFields.tcField:
         ...
         WHEN "charge" THEN DO:
            ASSIGN
              lcValue = REPLACE(STRING(ttTAPFile.Amount,">>>>>9.999999"),",","")
         ...
         WHEN "cause_for_record_closing" THEN DO:
            IF LOOKUP(ttTapFile.CloseCause,"3,4,5,20,21") > 0
               ...
               ELSE lcValue = "".
         END.
         WHEN "tap_decimal_places" THEN lcValue = "6".
