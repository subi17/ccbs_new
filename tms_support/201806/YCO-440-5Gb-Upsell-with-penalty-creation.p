/* YCO-275-5Gb-Upsell-creation-with-penalty.p 
   Purpose: create a total of 3 upsells with the same 5Gb size and penalty 
*/
...
DEF VAR cUpsell_Desc    AS CHARACTER NO-UNDO INITIAL "Bono de datos 5Gb adicionales 3 meses con penalización,Bono de datos 5Gb adicionales 6 meses con penalización,Bono de datos 5Gb adicionales 12 meses con penalización".
...
FORM
  SKIP "This program will create upsells for YCO-440: 5G with codes" SKIP(1)
  "RET5GB_3mP_R_UPSELL,RET5GB_6mP_R_UPSELL and RET5GB_12mP_R_UPSELL" SKIP
  "and penalty" SKIP 
  ...
  "Upsell Valid to  : " dto
  WITH OVERLAY CENTERED ROW 6 TITLE " Add Upsells for YCO-440 " NO-LABELS
  FRAME f-yco440.

UPDATE dfrom
       dto WITH FRAME f-yco440.
...
DO TRANSACTION ON ERROR UNDO blk-upsell, LEAVE blk-upsell
  ...
  FIND FIRST daycampaign WHERE daycampaign.Brand   = Syst.Var:gcBrand
                           AND daycampaign.DCEVent = "RET5GB_3mP_R_UPSELL"
                         NO-LOCK NO-ERROR. 
  IF AVAILABLE daycampaign THEN
  DO:
     MESSAGE 
       "YCO-440 Upsell 5Gb (with penalty) creation program has already being executed"
       VIEW-AS ALERT-BOX.
     RETURN.
  END.
  ...
  DO liUpsells = 1 TO 3:
     ...
     ASSIGN 
        ...
        ServiceLimitGroup.Groupname = "Mobile Data Usage Upsell 5G permanency"
     ...
     ASSIGN 
            ServiceLimitTarget.Slseq          = ServiceLimit.SLSeq            /* Sequence for Service Limit: ServiceLimit.SLSeq */
            ServiceLimitTarget.ServiceLMember = "14104019"                    /* 14100001 for DATA */
            ServiceLimitTarget.ServiceLimitMt = 0
            ServiceLimitTarget.InsideRate     = "GPRS_" + ServiceLimitGroup.groupcode /* Limit rate: GPRS_RET5GB_R_UPSELL, GPRS_RET5GB_3m_R_UPSELL, ... */
            ServiceLimitTarget.outsideRate    = "".
  ...
  FIND TMSParam WHERE TMSParam.Brand = Syst.Var:gcBrand
                  AND TMSParam.Paramgroup = "Bundles" 
                  AND TMSParam.ParamCode  = "RETENTION_5GB_UPSELLS"
                exclusive-lock
                NO-ERROR.
  IF AVAIL TMSParam THEN 
      TMSParam.CharVal = TMSParam.CharVal + 
                         ",RET5GB_3mP_R_UPSELL,RET5GB_6mP_R_UPSELL,RET5GB_12mP_R_UPSELL".
  ELSE 
      MESSAGE "TMSParam for ParamCode = RETENTION_5GB_UPSELLS failed. Please fix manually." 
