...
INPUT STREAM sFile THROUGH VALUE("ls -1 " + lcIncDir + "*.fix").
REPEAT:
...
PROCEDURE pMNPStatusCheck:
   ...
   /* Set Requested status for MNP OUT with Sent Status */
   IF MNPProcess.MNPType = {&MNP_TYPE_IN} AND
      MNPProcess.StatusCode = {&MNP_ST_AENV} AND
      icNCStatus = "ASOL" THEN DO:
      ...
      ASSIGN 
       ...
       RELEASE Order.
       /* fLogLine("UPDATE: ASOL update"). */

   END.
   
   /* Set Ported status for MNP OUT processes without subscriptions */
   ELSE IF MNPProcess.MNPType = {&MNP_TYPE_OUT} AND
