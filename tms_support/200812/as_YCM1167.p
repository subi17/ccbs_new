...
DEFINE VARIABLE lcPos AS CHARACTER NO-UNDO INIT "POS". 

def stream slog.
def stream sskip.
def stream sgold.
output stream slog to  /apps/snet/200812/as_ycm1167_handled.txt.
output stream sgold to /apps/snet/200812/as_ycm1167_golden.txt.
output stream sskip to /apps/snet/200812/as_ycm1167_exceptions.txt.

LOOPPI:

DO iCli = 633100000 to 633499999:
  ...
  ELSE DO:
      ...
      if liHandled > 200000 then do:
         lcPos = "webshop".
      end.
      if liHandled > 230000 then do:
         lcPos = "CC".
      end.
      if liHandled > 250000 then do:
         LEAVE LOOPPI.
      end.
