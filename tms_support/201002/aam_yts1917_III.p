...
def stream sread.
input stream sread from /apps/snet/201002/Second+partial+credit+Notes.txt.

def stream slog.
output stream slog to /apps/snet/201002/yts1917_invoices_status.log.
...
put stream slog unformatted
   ...
   "Amount"     chr(9)
   "1. credit"  chr(9)
   "Should Be Unpaid" chr(9)
   "Current Unpaid Balance" skip.
...
repeat:
   ...
   assign
      liinv = invoice.crinvnum
      ld1credamt = -1 * invoice.invamt
      ld1credtax = -1 * invoice.vatamt.
   ...
   put stream slog unformatted
      ...
      -1 * ld1credamt            chr(9)
      invoice.invamt - ld1credamt chr(9)
      ldbal            skip.
...
PROCEDURE pCreditNote:
   ...
   ASSIGN 
      ...
      bCreditInv.VATAmt      = Invoice.VatAmt - ld1credtax
      bCreditInv.InvAmt      = Invoice.InvAmt - ld1credamt
   ...
   ASSIGN
      ...
      Memo.Memotext  = Func.Common:mGetHdrText(50,Customer.Language) + " " +
                       ...
                       ", YTS-1916. Handler: " + Syst.Var:katun.
   ...
   ASSIGN
      ...
      Memo.CustNum   = Invoice.CustNum
      Memo.Memotext  = "YTS-1916, credited by " + Syst.Var:katun.
