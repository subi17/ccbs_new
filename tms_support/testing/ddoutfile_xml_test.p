   ...
   /* Group Header */
    lhXML:START-ELEMENT("GrpHdr").
     ...
     lhXML:START-ELEMENT("InitgPty").
      lhXML:WRITE-DATA-ELEMENT("Nm",STRING(Company.CompName)).
...
PROCEDURE pCollectData2XML:
   ...
   /* PREVIOUS CODE:  IF NOT AVAIL bInvoiceDD2 AND NOT AVAIL bInvoiceDD THEN */
   IF (NOT AVAIL bInvoice OR 
       bInvoice.MandateId NE Invoice.MandateId) AND
       NOT AVAIL bInvoiceDD2 AND NOT AVAIL bInvoiceDD THEN
      ASSIGN
   ...
   IF (liFrstCounter = 1 AND NOT llFrstCounter) OR 
      ...
      ASSIGN
         ...
         ttDDCreditor.CompNm = "YOIGO"
         ttDDCreditor.CompAdd = Company.Address
   ...
   ASSIGN
      ...
      ttDDItem.CustNm = CAPS(fCheckSEPASpecialChar(DYNAMIC-FUNCTION("fPrintCustName" IN ghFunc1, BUFFER Customer)))
      ttDDItem.CustAdd = CAPS(fCheckSEPASpecialChar(Customer.Address))
      ttDDItem.CustZip = Customer.ZipCode + " " + CAPS(fCheckSEPASpecialChar(Customer.PostOffice))
      ttDDItem.CustIBAN = Customer.BankAcct
