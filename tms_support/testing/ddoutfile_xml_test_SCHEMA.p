...
DEF VAR llXMLError AS LOG NO-UNDO. 
DEF VAR lcSchemaValLog AS CHAR NO-UNDO. 
...
PROCEDURE pFinalize:
   ...
   DEF VAR lcSchema AS CHAR NO-UNDO.
   DEF VAR lcXsdFile AS CHAR NO-UNDO.
   ...
   */
   ...
   IF ilCSBValidate AND
      ...
      ASSIGN
         ...
         lcCommand = lcSchema + lcXsdFile + " " + icFile + " > " +
            lcSchemaValLog + " 2>&1".
      INPUT STREAM sin THROUGH VALUE(lcCommand).
      IMPORT STREAM sin lcResult.

      IF INDEX(lcResult,":") > 0 THEN DO:
         lcTransDir = "".
         llXMLError = True.
      END.
   END.
...
PROCEDURE pLogErrors:
   ...
   IF CAN-FIND(FIRST ttError) OR 
      ...
      IF xMailAddr > "" THEN DO:
         ...
         IF llXMLError THEN
            PUT STREAM slog UNFORMATTED
            "XML Schema validation failed, see results: " + lcNewLine +
            lcSchemaValLog + lcNewLine + lcNewLine + " ".
         OUTPUT STREAM slog CLOSE.
