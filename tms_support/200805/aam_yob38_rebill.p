...
RUN /apps/snet/200805/lamupers_rebill.p persistent set pHandle.
...
repeat:
   ...
   for first msowner no-lock where
             ...
             msowner.tsend >= 20080401 and
             msowner.tsbeg <  20080501 and
             msowner.paytype = false:
      if not can-find(first ttcust where ttcust.custnum = msowner.invcust)
      then do:
         create ttcust.
         ttcust.custnum = msowner.invcust.
      end.
   end.
end.
...
message licustqty "customers picked"
view-as alert-box.
