...
ASSIGN 
   ...
   lcMarkPref  = fCParamC("PrefixBillItem")
   ...
   FOR EACH ttIR NO-LOCK:
      ...
      */
      IF ttIR.SlsAccNum = 0 THEN DO:
         ...
         */
         CASE icCategory:
         WHEN lcOwnUse THEN ttIR.SlsAccNum = BillItem.AltAccNum.
         WHEN lcVipUse THEN ttIR.SlsAccNum = BillItem.VIPAccNum.
         
         OTHERWISE DO:
            CASE iiVATUsage:
            WHEN 0 OR 
            WHEN 1 THEN ttIR.SlsAccNum = BillItem.AccNum.
...
PROCEDURE pCreateInv:
   ...
   BREAK BY ttInvCust.InvRec
      ...
      IF CAN-FIND(FIRST ttIR) AND iiInvType NE 6 AND iiInvType NE 7 THEN 
      ...
      DO TRANS:
         ...
         IF CAN-FIND(FIRST ttIR) THEN DO:
            ...
            FOR EACH CustIntEvent no-lock USE-INDEX CustNum where
               ...
               /* billing denied */
               IF Invoice.MsSeq > 0 AND 
                  LOOKUP(STRING(Invoice.MsSeq),lcBillDeny) > 0 
               THEN NEXT.
...
PROCEDURE pInvoiceHeader:
   ...
   BREAK BY ttRowVat.MsSeq
      ...
      IF LAST-OF(ttRowVat.MsSeq) THEN DO:
         ...
         IF lcDepoItem = "" AND iiInvType NE 6 AND iiInvType NE 7 THEN ASSIGN 
         ...
         */
         ...
         IF NOT lRejBill AND lNotBilled > "" THEN DO:
            ASSIGN lRejBill    = TRUE
         ...
         THEN DO:
            ...
            IF iiInvType NE 6 AND iiInvType NE 7 THEN DO:
            ...
            /* IF customer is in "own use" or in "vip use" category 
               then post accordingly */
            ELSE IF Customer.Category = lcOwnUse OR 
                    Customer.Category = lcVipUse
            THEN DO:
               ...
               RUN Ar/makepaym.p (BUFFER Invoice,
                             ...
                             (IF Customer.Category = lcOwnUse
                              THEN "Own"
                              ELSE "VIP") + " use.  Handler: " + Syst.CUICommon:katun,
                             OUTPUT liVoucher).
            END.
                 
            /* nul current month's saldo counter */
            IF Invoice.ToDate >= TODAY AND 
               iiInvType NE 6 AND iiInvType NE 7 THEN 
