...
PROCEDURE pFillTempTable2:
   ...
   FOR EACH RoamGPRS NO-LOCK WHERE
      ...
      fRoamCSV(RoamGPRS.Version). 
      
      ASSIGN
         ttTAPFile.RowType  = 2
         ttTAPFile.RowNum   = RECID(RoamGPRS)
         ttTAPFile.TotUnits = INT(ENTRY(ttRoamCSV.DataIn,RoamGPRS.CSV,"|")) +
                              INT(ENTRY(ttRoamCSV.DataOut,RoamGPRS.CSV,"|"))
         ttTAPFile.DataIn   = INT(ENTRY(ttRoamCSV.DataIn,RoamGPRS.CSV,"|"))
         ttTAPFile.DataOut  = INT(ENTRY(ttRoamCSV.DataOut,RoamGPRS.CSV,"|"))
         ttTAPFile.Duration = INT(ENTRY(ttRoamCSV.Duration,RoamGPRS.CSV,"|")).
      ...
      IF RoamGPRS.PartRecNum NE 0 THEN DO:
         ...
         FOR EACH bufPartial NO-LOCK WHERE
            ...
            ASSIGN
               ttTAPFile.TotUnits   = ttTAPFile.TotUnits + 
      ...
      IF ttTAPFile.TotUnits > 0 THEN DO:
         ...
         ASSIGN   
            lcIMSI   = ENTRY(ttRoamCSV.OrigIMSI, RoamGPRS.CSV,"|") /* originating */
            lcPLMN   = "N/A" 
            lcGroup  = "N/A".
