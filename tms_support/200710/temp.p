...
FOR EACH msisdn no-lock where
   ...
   cli begins "622":
   /*
   find first msisdnnumber where msisdnnumber.cli = msisdn.cli/* and rank = 0*/ no-lock no-error.
...
def stream excel.
OUTPUT STREAM excel TO "/apps/snet/200710/yoi-497.txt2".
...
if deCliStart ne ttMsRange.cli then 
   PUT STREAM excel UNFORMATTED deCliStart "-" deCliPrev SKIP.
