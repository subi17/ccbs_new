...
for each msisdnnumber where
    ...
    if (msisdn.pos = "" or msisdn.pos = ?) and msisdn.StatusCode = 0 then do:
        fMakeMSIDNHistory(recid(msisdn)).
        assign msisdn.statuscode = 1
               msisdn.pos = "ONLINE".
    end. /* (if msisdn.pos = "" or msisdn.pos = ?) and msisdn.StatusCode = 0 */
    else do:
        put stream slog unformatted msisdn.cli "|error:statuscode is not zero or stock is not null" skip.
        next each_loop.
    end. /* else do: */
