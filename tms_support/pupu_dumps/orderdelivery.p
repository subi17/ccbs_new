...
ASSIGN lcLogFile = lcSpoolDir + "/orderdelivery_" +
...
FOR EACH OrderDelivery WHERE
         OrderDelivery.Brand = gcBrand NO-LOCK:
 
   ASSIGN liEvents  = liEvents + 1
          lcMessage = "OrderDelivery"                             + lcDel +
                      "CREATE"                                    + lcDel +
                      fNotNull(STRING(RECID(OrderDelivery)))      + lcDel +
                      fNotNull(STRING(OrderDelivery.OrderId)      + CHR(255) + 
                               STRING(OrderDelivery.LOTimeStamp)) + lcDel +
                      fNotNull(STRING(ldtTimeStamp))              + lcDel +
                      fNotNull(STRING(OrderDelivery.OrderId))     + lcDel +
                      fNotNull(STRING(OrderDelivery.LOStatus))    + lcDel +
   ...
   IF NOT SESSION:BATCH AND liEvents MOD 100 = 0 THEN DO:
      PAUSE 0.
      DISP liEvents LABEL "OrderDelivery" 
