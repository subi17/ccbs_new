...
ASSIGN lcLogFile = lcSpoolDir + "/customermservicelimit_" +
...
FOR EACH ServiceLimit WHERE
   ...
   FIRST Customer WHERE
         Customer.CustNum = MServiceLimit.CustNum NO-LOCK:

   liEvents = liEvents + 1.

   /* Exclude subscription based bundles */
   IF MServiceLimit.CustNum = ? OR MServiceLimit.CustNum = 0 THEN NEXT.
   liCustNum = MServiceLimit.CustNum.
