...
ASSIGN lcLogFile = lcSpoolDir + "/mservicelpool_" +
...
FOR EACH MobSub WHERE
    ...
    EACH MServiceLPool WHERE
         MServiceLPool.MsSeq  = MobSub.MsSeq AND
         MServiceLPool.EndTS >= ldeEndStamp NO-LOCK:

   /* Exclude customer based bundles */
   IF MServiceLPool.CustNum > 0 THEN NEXT.
   ...
   lcKeyValue = STRING(MServiceLPool.MsSeq) + lcDel2 +
   ...
   PUT STREAM slog UNFORMATTED
       ...
       fNotNull(STRING(MServiceLPool.EndTS))    SKIP.
END.
