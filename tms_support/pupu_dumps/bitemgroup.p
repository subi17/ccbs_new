...
ASSIGN lcLogFile = lcSpoolDir + "/bitemgroup_" +
...
FOR EACH BItemGroup WHERE BItemGroup.Brand = gcBrand NO-LOCK:

   liEvents = liEvents + 1.

   IF NOT SESSION:BATCH AND liEvents MOD 100 = 0 THEN DO:
      PAUSE 0.
      DISP liEvents LABEL "BItemGroup" 
   ...
   PUT STREAM slog UNFORMATTED
       "BItemGroup"                          lcDel
       "CREATE"                              lcDel
       fNotNull(STRING(RECID(BItemGroup)))   lcDel
       fNotNull(BItemGroup.BIGroup)          lcDel
       fNotNull(STRING(ldtTimeStamp))        lcDel
       fNotNull(BItemGroup.BIGroup)          lcDel
       fNotNull(BItemGroup.BIGName)          SKIP.
END.
