/* ----------------------------------------------------------------------
  ...
  TASK .........: Create DSS promotion if customer buy higher price of bundle
...
DEF VAR lcLogFile                AS CHAR NO-UNDO
 FORMAT "X(256)" VIEW-AS FILL-IN SIZE 45 BY 1.
...
FORM
   SKIP
   "Run ONLY Simulation Mode (Yes/No): " lcSimulate SKIP
   "Enter Log File Name and Path     : " lcLogFile
   WITH OVERLAY CENTERED ROW 10 TITLE " Add DSS Promotion " NO-LABELS
   FRAME finvrow.

UPDATE lcSimulate
       lcLogFile with FRAME finvrow.
...
FOR EACH ttDSSFat NO-LOCK:
   ...
   IF fFatExists("DSSCPFREE",ttDSSFat.DSSMsSeq,liPeriod) THEN DO:
      PUT STREAM sout UNFORMATTED
   ...
   END.
   ELSE lcError = "FAT will be added later.".
...
PROCEDURE pGetCustomerSubscriptions:
   ...
   IF NOT fGetDSSMsSeqLimit(INPUT  iiInvCust,
                            INPUT  ldPeriodTo,
                            OUTPUT liDSSMsSeq,
                            OUTPUT ldeDSSLimit) THEN RETURN.
