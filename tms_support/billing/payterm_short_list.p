...
DEFINE VARIABLE liSFConcerns AS INT NO-UNDO. 
...
ASSIGN
   ...
   lcfile      = "payterm_list_" + lcnow + ".log"
   lcErrfile = "payterm_list_errors_" + lcnow + ".log".
...
DISP " -- Payterm shortening LIST program -- " SKIP
     "This collect cases, where " SKIP
     "payterms and monthlyfees(FF) are not in sync" SKIP
     "list is preliminary report for Yoigo." WITH FRAME a.
PAUSE 0.

/* -------  
    NOTE: This is a list program, not any changes to be made.
   -------  */


/* First is checked FixedFee shortening events from EventLog */
FOR EACH EventLog NO-LOCK USE-INDEX EventDate WHERE
   ...
   IF AVAIL SingleFee AND SingleFee.InvNum = 0 THEN DO:
         ...
         /*  This is not populated / logged in the listing phase
         liSFConcerns = YEAR(ldaffee + 1) * 10000 +
                     MONTH(ldaffee + 1) * 100  + 
                     DAY(ldaffee + 1). */
