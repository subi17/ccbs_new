...
DEF VAR lcOrigBundle AS CHARACTER NO-UNDO. 
...
FUNCTION fGetOriginalBundle RETURNS CHAR
   ...
   DEFINE VARIABLE ldaActivated AS DATE NO-UNDO. 
...
DO i = 1 TO NUM-ENTRIES(lcRequestTypes):
   ...
   FOR EACH MsRequest NO-LOCK WHERE
      ...
      FOR EACH DCCLI NO-LOCK WHERE
         ...
         IF lcOrigCLIType begins "contrd" or
            lcOrigCLIType eq "contf" or
            lcOrigCLIType eq "contd" or
            lcOrigCLIType eq "conts" or
            lcOrigCLIType eq "contsf" then do:
            lcOrigBundle =  fGetOriginalBundle 
                               (msrequest.MsSeq,
                                DCCLI.DCEvent,
                                DCCLI.ValidFrom,
                                OUTPUT llRenewal).
            if not lcOrigBundle > "" THEN llError = true.
         END.
         
         FIND FIRST bOrigCLIType NO-LOCK where
                    bOrigCLIType.brand = "1" and
                    bOrigCLIType.clitype = (IF lcOrigBundle > "" THEN
                                            lcOrigBundle ELSE lcOrigCLIType) no-error.
         ...
         ELSE IF msrequest.reqtype = 0 then do:
            ...
            ELSE IF liFeecreated = 0 THEN DO:
               ...
               IF msrequest.reqcparam1 begins "cont" and
                       msrequest.reqcparam2 begins "tarj" then llError  = true.
                  ...
                  lookup(DCCLI.dcevent,"term18,term24,term18-50,term24-50") > 0 then . 
                       ...
                       msrequest.reqiparam5 ne 1 then llError = true.
         ...
         IF lcOrigBundle > "" THEN 
            lcOrigCLIType = lcOrigCLIType + "/" + lcOrigBundle.
            ...
            IF NOT (liFeecreated EQ 0 AND 
               ...
               IF liFeecreated EQ 0 THEN DO:
                  FIND FIRST bMsrequest NO-LOCK WHERE
                             ...
                             bMsrequest.Reqcparam1 EQ "ValidTo" AND
                             ...
                             bMsrequest.ReqSource EQ {&REQUEST_SOURCE_NEWTON}
                  NO-ERROR.
