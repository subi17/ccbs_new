   ...
   */
   ...
   if fixedfee.orderid eq 0 then do:
      put stream sout unformatted
         ...
         fixedfee.ffnum "|" 
         avail(mobsub) "|"
         (if avail(ffitem) then string(round(ffitem.amt,2)) ELSE "N/A") "|"
         (if avail(singlefee) then string(round(singlefee.amt,2)) else "N/A") "|" 
         fixedfee.begperiod "|"
   ...
   IF NOT AVAIL singlefee and avail(ffitem) then do:
      ...
      put stream sout unformatted
         ...
         (if avail(ffitem) then string(round(ffitem.amt,2)) ELSE "N/A") "|"
         (if avail(singlefee) then string(round(singlefee.amt,2)) else "N/A") "|" 
         (if avail(bsinglefee) then string(round(bsinglefee.amt,2)) else "N/A") "|" 
