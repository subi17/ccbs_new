...
DEFINE TEMP-TABLE ttSL NO-UNDO LIKE ServiceLimit
   FIELD i AS INT.
...
ASSIGN lcInputFile = icInFile + "rateplan.txt"
       lcLogFile   = icSpoolDir + "rateplan.log".
      
INPUT STREAM RPIn FROM VALUE(ilcInputFile).
...
IF AVAILABLE ttRatePlan THEN DO:
   ...
   DO TRANSACTION:
      ...
      IF SEARCH(acInputFile) EQ ? THEN 
         LEAVE TTRATEPLAN.

      INPUT STREAM RPNIn FROM VALUE(icInputFile).
      OUTPUT STREAM RPNLog TO VALUE(icLogFile) APPEND.
...
PROCEDURE pValidateFileData:
   ...
   FIND FIRST ttSL NO-LOCK NO-ERROR.
   
   IF AVAILABLE ttSL THEN DO:
      IF ttSL.SLCode EQ "" THEN DO:
         fError("No SLCode data available").
         RETURN "ERROR".
      END.
      IF ttSL.SLName EQ "" THEN DO:
         fError("No SLName data available").
         RETURN "ERROR".
      END.
   ...
   END.
   ELSE DO:
      fError("No ttSL records available").
      RETURN "ERROR".
   END.        
   ...
   END.
   ELSE DO:
      fError("No ttSLTarget records available").
      RETURN "ERROR".
   END.        
