...
DEFINE VARIABLE icOldStat AS INTEGER NO-UNDO. 
...
repeat:
   ...
   ELSE DO:
      ...
      IF sim.simstat NE icOldStat then do:
         put stream slog unformatted "sim with icc " sim.icc " is not in status "  STRING(icOldStat) skip.
         next icc_loop.
      end.
