...
def var moberror        like moberror.moberror     no-undo. 
...
form
      moberror.moberror
      moberror.MEName  format "x(30)"
    with scroll 1 11 down  row 4 centered color value(Syst.CUICommon:cfc)
    title color value(Syst.CUICommon:ctc) " Mob Rating Error Codes " overlay frame sel.

form /* SEEK Code */
    moberror
    help "Enter Code of a Mobile Rating Error"
...
repeat:
   ...
   find first moberror no-lock no-error.
...
print-line:
   do :
      if must-print then do:
         ...
         /* Print to screen */
         rtab = ?.
         do while frame-line<= frame-down and available moberror:
            display
            moberror.moberror
            moberror.MEName
            with frame sel.
            ...
            down with frame sel.
            find next moberror no-lock no-error.
         end.
         must-print = false.
      ...
      repeat with frame sel on endkey undo, retuRN:
         ...
         with frame sel:
            if frame-line = 1 then do:
               find moberror where recid(moberror) = rtab[frame-line] no-lock.
               find prev moberror no-lock no-error.
               if not available moberror then do:
                  bell.
                  ...
                  display moberror.moberror moberror.MEName.
                  rtab[frame-line] = recid(moberror).
                  memory = recid(moberror).
               end.
            end.
        ...
        /* Seek */
        if lookup(Syst.CUICommon:nap,"1,f1") > 0 then do:  /* moberror */
           Syst.CUICommon:cfc = "puyr". RUN Syst/ufcolor.p.
           Syst.CUICommon:ehto = 9. RUN Syst/ufkey.p. ufkey = true.
           update moberror with frame hayr.
           hide frame hayr no-pause.
           if moberror ENTERED then do:
              find first moberror where moberror.moberror >= moberror
              no-lock no-error.
               if not available moberror then do:
                       bell.
                       message "None found !".    
        ...
        /* Choose */
           ...
           find moberror where recid(moberror) = rtab[frame-line] no-lock.
           siirto = string(moberror.moberror).
           leave MAIN.
        end. /* Choose */
        /* First record */
        else if lookup(Syst.CUICommon:nap,"home,h") > 0 then do:
           find first moberror no-lock.
           memory = recid(moberror).
           must-print = true.
           next LOOP.
        end. /* First record */
