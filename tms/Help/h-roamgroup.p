/*------------------------------------------------------
  Module .......: H-ROAMGROUP.P
  Parent .......: APPLHELP.P
  FUNCTION .....: Help browser of RoamGroup 
  APPLICATION ..: TMS
  AUTHOR .......: anttis
  CREATED ......: 10-07-07
  MODIFIED .....: 
...
def var se-code     like RoamGroup.RoamGroup   no-undo.
...
with scroll 1 11 down  row 4 centered color value(Syst.Var:cfc)
   title color value(Syst.Var:ctc) " Roaming Group " overlay 
frame sel.
...
print-line:
   do :
      if must-print then do:
         ...
         /* Print to screen */
         rtab = ?.
         do while frame-line<= frame-down and available RoamGroup:
            display
               RoamGroup.RoamGroup
               RoamGroup.Name
            with frame sel.
...
BROWSE:
      repeat with frame sel on endkey undo, retuRN:
         ...
         color display value(Syst.Var:ccc) RoamGroup.RoamGroup with frame sel.
         ...
         with frame sel:
            if frame-line = 1 then do:
               find RoamGroup where recid(RoamGroup) = rtab[frame-line] no-lock.
               ...
               if not available RoamGroup then do:
                  ...
                  display RoamGroup.RoamGroup RoamGroup.Name.
                  rtab[frame-line] = recid(RoamGroup).
                  memory = recid(RoamGroup).
               end.
            end.
        ...
        if lookup(Syst.Var:nap,"1,f1") > 0 then do on ENDkey undo, NEXT LOOP:
           ...
           if se-code ENTERED then do:
              find first RoamGroup where RoamGroup.RoamGroup >= se-code
              no-lock no-error.
             if not available RoamGroup then do:
                       bell.
                       message "None found !".    
        ...
        /* Choose */
           ...
           find RoamGroup where recid(RoamGroup) = rtab[frame-line] no-lock.
           siirto = string(RoamGroup.RoamGroup).
           leave MAIN.
        end. /* Choose */
        /* First record */
        else if lookup(Syst.Var:nap,"home,h") > 0 then do:
           find first RoamGroup no-lock.
           memory = recid(RoamGroup).
           must-print = true.
           next LOOP.
        end. /* First record */
