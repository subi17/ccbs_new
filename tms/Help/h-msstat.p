/*------------------------------------------------------
  Module .......: H-SIMSTAT.P
  Parent .......: APPLHELP.P
  FUNCTION .....: Help browser of MSISDN codes
  APPLICATION ..: NN
  AUTHOR .......: JP
  CREATED ......: 10-06-99
  MODIFIED .....: 
...
form
      MSStat.StatusCode
      MSStat.StatusName  format "x(30)"
    with scroll 1 11 down  row 4 centered color value(Syst.CUICommon:cfc)
    title color value(Syst.CUICommon:ctc) " MSISDN Status codes " overlay frame sel.

form /* SEEK Code */
    StatusCode
    help "Enter Code of an MSISDN"
...
repeat:
   ...
   find first MSStat no-lock no-error.
...
print-line:
   do :
      if must-print then do:
         ...
         /* Print to screen */
         rtab = ?.
         do while frame-line<= frame-down and available MSStat:
            display
            MSStat.StatusCode
            MSStat.StatusName
            with frame sel.
            ...
            down with frame sel.
            find next MSStat no-lock no-error.
         end.
         must-print = false.
...
BROWSE:
      repeat with frame sel on endkey undo, retuRN:
         ...
         color display value(Syst.CUICommon:ccc) MSStat.StatusCode with frame sel.
         ...
         with frame sel:
            if frame-line = 1 then do:
               find MSStat where recid(MSStat) = rtab[frame-line] no-lock.
               find prev MSStat no-lock no-error.
               if not available MSStat then do:
                  bell.
                  ...
                  display MSStat.StatusCode MSStat.StatusName.
                  rtab[frame-line] = recid(MSStat).
                  memory = recid(MSStat).
               end.
            end.
        ...
        /* Seek */
        if lookup(Syst.CUICommon:nap,"1,f1") > 0 then do:  /* StatusCode */
        ...
        /* Choose */
           ...
           find MSStat where recid(MSStat) = rtab[frame-line] no-lock.
           siirto = string(MSStat.StatusCode).
           leave MAIN.
        end. /* Choose */
        /* First record */
        else if lookup(Syst.CUICommon:nap,"home,h") > 0 then do:
           find first MSStat no-lock.
           memory = recid(MSStat).
           must-print = true.
           next LOOP.
        end. /* First record */
