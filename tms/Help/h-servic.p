/*------------------------------------------------------
  Module .......: H-SERVIC.P
  Parent .......: APPLHELP.P
  FUNCTION .....: Help browser of Service
  APPLICATION ..: NN
  AUTHOR .......: jp
  CREATED ......: 17-06-99
...
form
    ...
    title color value(Syst.Var:ctc) " Service " overlay frame sel.
...
repeat:
   ...
   find first Service WHERE 
              Service.Brand = Syst.Var:gcBrand no-lock no-error.
...
print-line:
   do :
      if must-print then do:
         ...
         /* Print to screen */
         rtab = ?.
         do while frame-line<= frame-down and available Service:
            display
            Service.Service
            Service.SEName
            with frame sel.
            ...
            down with frame sel.
            find next Service WHERE service.Brand = Syst.Var:gcBrand  no-lock no-error.
         end.
         must-print = false.
...
BROWSE:
      repeat with frame sel on endkey undo, retuRN:
         ...
         color display value(Syst.Var:ccc) Service.Service with frame sel.
         ...
         with frame sel:
            if frame-line = 1 then do:
               ...
               find prev Service  WHERE service.Brand = Syst.Var:gcBrand 
               no-lock no-error.
               if not available Service then do:
                  bell.
                  ...
                  display Service.Service Service.SEName.
                  rtab[frame-line] = recid(Service).
                  memory = recid(Service).
               end.
            end.
         ...
         if lookup(Syst.Var:nap,"cursor-down") > 0 then do with frame sel:
            if frame-line = frame-down then do:
               find Service where recid(Service) = rtab[frame-line] no-lock .
               find next Service  WHERE service.Brand = Syst.Var:gcBrand  
               no-lock no-error.
               if not available Service then do:
                  bell.
        ...
        /* First record */
           ...
           find first Service  WHERE service.Brand = Syst.Var:gcBrand  no-lock.
           memory = recid(Service).
           must-print = true.
           next LOOP.
        end. /* First record */
        ...
        /* last record */
           ...
           find last Service  WHERE service.Brand = Syst.Var:gcBrand  no-lock.
           memory = recid(Service).
           must-print = true.
           next LOOP.
        end. /* last record */
