      ...
      repeat with frame sel on endkey undo, retuRN:
         ...
         with frame sel:
            if frame-line = 1 then do:
               ...
               find prev servattr WHERE servattr.servcom = servcom AND servattr.brand = Syst.Var:gcBrand
                no-lock no-error.
               if not available servattr then do:
                  bell.
         ...
         if lookup(Syst.Var:nap,"cursor-down") > 0 then do with frame sel:
            if frame-line = frame-down then do:
               find servattr where recid(servattr) = rtab[frame-line] no-lock .
               find next servattr  WHERE servattr.servcom = servcom AND servattr.brand = Syst.Var:gcBrand
               no-lock no-error.
               if not available servattr then do:
                  bell.
        ...
        /* First record */
           ...
           find first servattr  WHERE servattr.servcom = servcom AND 
                      servattr.brand = Syst.Var:gcBrand no-lock.
           memory = recid(servattr).
           must-print = true.
           next LOOP.
        end. /* First record */
        ...
        /* last record */
           ...
           find last servattr  WHERE 
            servattr.servcom = servcom AND servattr.brand = Syst.Var:gcBrand no-lock.
           memory = recid(servattr).
           must-print = true.
           next LOOP.
        end. /* last record */
