...
WITH centered OVERLAY scroll 1 10 DOWN ROW 6
    ...
    title color value(Syst.Var:ctc) " OPERATOR BROWSER: SEEK FROM '"  + Operator + "' " FRAME sel.
...
BROWSE:
   repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF order = 2 THEN DO:
         ...
         COLOR DISPLAY value(Syst.Var:ccc) Operator.Operator WITH FRAME sel.
      END.
      ELSE IF order = 1 THEN DO:
         CHOOSE ROW Operator.OperName {Syst/uchoose.i} no-error WITH FRAME sel.
         COLOR DISPLAY value(Syst.Var:ccc) Operator.OperName WITH FRAME sel.
      END.
