/*------------------------------------------------------
  Module .......: H-SERVPAC.P
  Parent .......: APPLHELP.P
  FUNCTION .....: Help browser of Service Package
  APPLICATION ..: NN
  AUTHOR .......: PT
  CREATED ......: 17-06-99
  MODIFIED .....: 10.03.03 tk exit if there are no records
                  05.09.03 jp Brandcode
  Version ......: M15
  ------------------------------------------------------ */
...
form
    ...
    title color value(Syst.Var:ctc) " Service Package " overlay frame sel.
...
repeat:
   ...
   find first ServPac  WHERE 
              ServPac.Brand = Syst.Var:gcBrand no-lock no-error.
...
print-line:
   do :
      if must-print then do:
         ...
         /* Print to screen */
         rtab = ?.
         do while frame-line<= frame-down and available ServPac:
            display
            ServPac.ServPac
            ServPac.SPName
            with frame sel.
            ...
            down with frame sel.
            find next ServPac WHERE servpac.Brand = Syst.Var:gcBrand no-lock no-error.
         end.
         must-print = false.
...
BROWSE:
      repeat with frame sel on endkey undo, retuRN:
         ...
         color display value(Syst.Var:ccc) ServPac.ServPac with frame sel.
         ...
         with frame sel:
            if frame-line = 1 then do:
               ...
               find prev ServPac WHERE Servpac.Brand = Syst.Var:gcBrand no-lock no-error.
               if not available ServPac then do:
                  bell.
                  ...
                  display ServPac.ServPac ServPac.SPName.
                  rtab[frame-line] = recid(ServPac).
                  memory = recid(ServPac).
               end.
            end.
         ...
         if lookup(Syst.Var:nap,"cursor-down") > 0 then do with frame sel:
            if frame-line = frame-down then do:
               find ServPac where recid(ServPac) = rtab[frame-line] no-lock .
               find next ServPac WHERE Servpac.Brand = Syst.Var:gcBrand 
               no-lock no-error.
               if not available ServPac then do:
                  bell.
         ...
         /* previous page */
            ...
            find ServPac where recid(ServPac) = memory no-lock no-error.
            find prev ServPac WHERE Servpac.Brand = Syst.Var:gcBrand 
            no-lock no-error.
        ...
        if lookup(Syst.Var:nap,"1,f1") > 0 then do on ENDkey undo, NEXT LOOP:
           ...
           if ServPac ENTERED then do:
              find first ServPac where 
                         ...
                         ServPac.brand    = Syst.Var:gcBrand 
              no-lock no-error.
             if not available ServPac then do:
                       bell.
                       message "None found !".    
        ...
        /* Choose */
           ...
           find ServPac where recid(ServPac) = rtab[frame-line] no-lock.
           siirto = string(ServPac.ServPac).
           leave MAIN.
        end. /* Choose */
        /* First record */
        else if lookup(Syst.Var:nap,"home,h") > 0 then do:
           find first ServPac WHERE Servpac.Brand = Syst.Var:gcBrand no-lock.
           memory = recid(ServPac).
           must-print = true.
           next LOOP.
        end. /* First record */
        ...
        /* last record */
           ...
           find last ServPac no-lock WHERE Servpac.Brand = Syst.Var:gcBrand .
           memory = recid(ServPac).
           must-print = true.
           next LOOP.
        end. /* last record */
