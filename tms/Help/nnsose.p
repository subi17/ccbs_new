...
WITH
    ...
    title color value(Syst.Var:ctc) " SALES OFFICE (" + Syst.Var:gcBrand + ") " 
    OVERLAY FRAME sel.
...
repeat:
   ...
   FIND FIRST Salesoffice WHERE SalesOffice.Brand = Syst.Var:gcBrand NO-LOCK no-error.
...
add-new:
      repeat WITH FRAME tlli:
         ...
         ASSIGN
           ylin = recid(Salesoffice)
           SalesOffice.Brand       = Syst.Var:gcBrand
           Salesoffice.SalesOffice = Salesoffice.
      ...
      FIND FIRST Salesoffice  WHERE 
         SalesOffice.Brand = Syst.Var:gcBrand NO-LOCK no-error.
...
BROWSE:
      repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
         ...
         WITH FRAME sel.
         COLOR DISPLAY value(Syst.Var:ccc) Salesoffice.SalesOffice WITH FRAME sel.
        ...
        if lookup(Syst.Var:nap,"1,f1") > 0 THEN DO:  /* haku */
           ...
           Salesoffice = "". Syst.Var:ehto = 9. RUN Syst/ufkey.p. ufkey = TRUE.
