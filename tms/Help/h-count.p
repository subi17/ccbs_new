/*------------------------------------------------------
  Module .......: H-COUNT.P
  Parent .......: APPLHELP.P
  FUNCTION .....: Help browser of Country
  APPLICATION ..: NN
  AUTHOR .......: jp
  CREATED ......: 21-06-99
  MODIFIED .....: 
...
def var se-code     like Country.Country   no-undo.
...
with scroll 1 11 down  row 4 centered color value(Syst.Var:cfc)
   title color value(Syst.Var:ctc) " Country " overlay 
frame sel.
...
repeat:
   ...
   find first Country no-lock no-error.
...
print-line:
   do :
      if must-print then do:
         ...
         /* Print to screen */
         rtab = ?.
         do while frame-line<= frame-down and available Country:
            display
               Country.Country
               Country.COName
            with frame sel.
            ...
            down with frame sel.
            find next Country no-lock no-error.
         end.
         must-print = false.
      ...
      repeat with frame sel on endkey undo, retuRN:
         ...
         with frame sel:
            if frame-line = 1 then do:
               ...
               find prev Country no-lock no-error.
               if not available Country then do:
                  bell.
                  ...
                  display Country.Country Country.COName.
                  rtab[frame-line] = recid(Country).
                  memory = recid(Country).
               end.
            end.
        ...
        if lookup(Syst.Var:nap,"1,f1") > 0 then do on ENDkey undo, NEXT LOOP:
           ...
           if se-code ENTERED then do:
              find first Country where Country.Country >= se-code
              no-lock no-error.
             if not available Country then do:
                       bell.
                       message "None found !".    
        ...
        /* Choose */
           ...
           find Country where recid(Country) = rtab[frame-line] no-lock.
           siirto = string(Country.Country).
           leave MAIN.
        end. /* Choose */
        /* First record */
        else if lookup(Syst.Var:nap,"home,h") > 0 then do:
           find first Country no-lock.
           memory = recid(Country).
           must-print = true.
           next LOOP.
        end. /* First record */
