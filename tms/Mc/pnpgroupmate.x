/* -----------------------------------------------
  ...
  AUTHOR .......: KL
  CREATED ......: 25-02-01
  MODIFIED .....: 26.06.03/aam validation for new group changed,
                               use-index pnpgroup (pnpseq is primary)
                  07.07.03 jp quantity of sms  (f3)             
...
form
   ...
   pnpgroup.product  format "x(8)"
      tuote.tu-nimi  format "x(10)"
   pnpgroup.ccn     
      nnmaa.ma-nimi  format "x(10)"
   pnpgroup.rbsub    format "x(8)" column-label "RBSub"
   pnpgroup.dto
...
form
   ...
   "Product ...:" pnpgroup.product  
   tuote.tu-nimi  SKIP
   "CCN .......:" pnpgroup.ccn 
   nnmaa.ma-nimi  SKIP
   "Rate BSUB .:" pnpgroup.rbsub                 SKIP
...
BROWSE:
   repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF ufkey THEN DO:
        ASSIGN
        ufk[1]= 35  ufk[2]= 30 ufk[3]= 1775 ufk[4]= 1761
     ...
     /* Haku 1 */
       ...
       if haku-pnpgroup <> "" THEN DO:
          FIND FIRST pnpgroup where pnpgroup.pnpgroup >= haku-pnpgroup
     ...
     ELSE IF LOOKUP(nap,"3,F3") > 0 THEN DO:
       ...
       IF pnpgroup.prod NE "SMS" then DO:
          MESSAGE
          "Only quantity of SMS!"
          VIEW-AS ALERT-BOX.
          NEXT.
       END.
       RUN pnpqty.p(INPUT pnpgroup.pnpgroup).
       ufkey = true.
       run ufkey.
       PAUSE 0.
     END.
     ...
     END.
        ...
        FIND FIRST pnpgroup WHERE 
             RECID(pnpgroup) = rtab[FRAME-LINE] 
        NO-LOCK NO-ERROR.
        IF CAN-FIND(FIRST ratebsub where
                          ratebsub.bsub     = pnpgroup.rbsub  ) THEN DO:
           ufkey = TRUE.

           RUN nntuhi(0,pnpgroup.rbsub).
        END.
        ELSE 
           MESSAGE "This price has a different rating BSUB !"
...
PROCEDURE LOCAL-DISP-ROW: 
   ...
   FIND FIRST tuote WHERE
              tuote.tu-nro = pnpgroup.product
   NO-LOCK NO-ERROR.
 
   FIND FIRST nnmaa WHERE
              nnmaa.ma-nro = pnpgroup.ccn
   NO-LOCK NO-ERROR.
   ...
   DISPLAY
      ...
      pnpgroup.pnpgroup  
      pnpgroup.name
      pnpgroup.product tuote.tu-nimi when AVAIL tuote 
      pnpgroup.ccn     nnmaa.ma-nimi
      pnpgroup.rbsub   
      pnpgroup.dto
   WITH FRAME sel.
...
PROCEDURE LOCAL-FIND-NEXT.
   ...
   IF order = 1 THEN 
      FIND NEXT pnpgroup USE-INDEX pnpgroup
      NO-LOCK NO-ERROR.
...
PROCEDURE LOCAL-FIND-PREV.
   ...
   IF order = 1 THEN 
      FIND PREV pnpgroup USE-INDEX pnpgroup
      NO-LOCK NO-ERROR.
...
PROCEDURE LOCAL-FIND-FIRST.
   ...
   IF order = 1 THEN 
      FIND FIRST pnpgroup USE-INDEX pnpgroup
      NO-LOCK NO-ERROR.
...
PROCEDURE LOCAL-FIND-LAST.
   ...
   IF order = 1 THEN 
      FIND LAST pnpgroup USE-INDEX pnpgroup
      NO-LOCK NO-ERROR.
...
PROCEDURE LOCAL-UPDATE-RECORD. 
   ...
   IF NOT bNew THEN DO:
      ...
      DISP 
         pnpgroup.pnpgroup 
         tuote.tu-nimi   WHEN AVAIL tuote
         nnmaa.ma-nimi   WHEN AVAIL nnmaa
      WITH FRAME lis.
   ...
   WITH FRAME lis EDITING: 
      ...
      IF lookup(nap,poisnap) > 0 THEN DO:
         ...
         ELSE IF FRAME-FIELD = "rbsub" THEN DO:
            if input frame lis pnpgroup.rbsub = "" THEN DO:
               BELL.
               MESSAGE "Unknown Ratebsub!".
               NEXT-PROMPT pnpgroup.rbsub.
               NEXT.
            END.
         END.
         ...
         ELSE IF FRAME-FIELD = "dto" THEN DO:
            ...
            IF CAN-FIND(FIRST xxpnpgroup WHERE
                              ...
                              xxpnpgroup.product  = input pnpgroup.product  AND
                              xxpnpgroup.dto      = input pnpgroup.dto      AND
                              RECID(xxpnpgroup)   ne RECID(pnpgroup))
            THEN DO:
               BELL.
               MESSAGE "PNP Group already exists !".
