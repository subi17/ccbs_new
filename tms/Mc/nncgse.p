...
WITH scroll 1 11 DOWN  ROW 4 centered COLOR value(Syst.Var:cfc)
    title color value(Syst.Var:ctc) " Customer Groups (" + Syst.Var:gcBrand + ") " 
    OVERLAY FRAME tlse.
...
repeat:
   ...
   FIND FIRST CustGroup WHERE CustGroup.Brand = Syst.Var:gcBrand NO-LOCK no-error.
...
LOOP:
   ...
   IF must-add THEN DO:  /* Invoicing Group  lisays  */
      ...
      repeat WITH FRAME tlli:
         ...
         ASSIGN
           muisti = recid(CustGroup)
           CustGroup.Brand     = Syst.Var:gcBrand
           CustGroup.CustGroup = INPUT CustGroup.CustGroup.
   ...
   DO :
      IF must-print THEN DO:
         ...
         DO WHILE frame-line<= FRAME-DOWN AND AVAILABLE CustGroup:
            ...
            FIND NEXT CustGroup WHERE CustGroup.Brand = Syst.Var:gcBrand NO-LOCK no-error.
         END.
         must-print = FALSE.
      ...
      IF ufkey THEN DO:
         ASSIGN
         Syst.Var:ufk = 0 Syst.Var:ufk[1] = 35 Syst.Var:ufk[4] = 510 Syst.Var:ufk[5] = 11
...
BROWSE:
      repeat WITH FRAME tlse ON ENDKEY UNDO, RETURN:
         ...
         COLOR DISPLAY value(Syst.Var:ccc) CustGroup.CustGroup WITH FRAME tlse.
         ...
         WITH FRAME tlse:
            IF FRAME-LINE = 1 THEN DO:
               ...
               FIND prev CustGroup WHERE CustGroup.Brand = Syst.Var:gcBrand 
                  NO-LOCK no-error.
               IF NOT AVAILABLE CustGroup THEN DO:
                  BELL.
         ...
         if lookup(Syst.Var:nap,"cursor-down") > 0 THEN DO WITH FRAME tlse:
            IF FRAME-LINE = FRAME-DOWN THEN DO:
               FIND CustGroup where recid(CustGroup) = rtab[FRAME-LINE] 
                  no-lock .
               FIND NEXT CustGroup WHERE CustGroup.Brand = Syst.Var:gcBrand 
                  NO-LOCK no-error.
               IF NOT AVAILABLE CustGroup THEN DO:
                  BELL.
         ...
         /* previous page */
            ...
            FIND CustGroup where recid(CustGroup) = muisti no-lock no-error.
            FIND prev CustGroup WHERE CustGroup.Brand = Syst.Var:gcBrand NO-LOCK no-error.
        ...
        /* Ensimmainen tietue */
           ...
           FIND FIRST CustGroup WHERE CustGroup.Brand = Syst.Var:gcBrand NO-LOCK.
           muisti = recid(CustGroup).
           must-print = TRUE.
           NEXT LOOP.
        ...
        /* LAST record */
           ...
           FIND LAST CustGroup WHERE CustGroup.Brand = Syst.Var:gcBrand NO-LOCK.
           muisti = recid(CustGroup).
           must-print = TRUE.
           NEXT LOOP.
        END. /* LAST record */
