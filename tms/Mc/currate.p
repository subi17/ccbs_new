...
form /*  search WITH FIELD Currency */
    Currency
    help "Give ...."
    with row 4 col 2 title color value(Syst.Var:ctc) " FIND Currency CODE "
    COLOR value(Syst.Var:cfc) NO-LABELS OVERLAY FRAME f1.

form /*  search WITH FIELD RateDate */
    RateDate
    help "Give ..."
    with row 4 col 2 title color value(Syst.Var:ctc) " FIND Currency Date "
...
BROWSE:
   repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF order = 1 THEN DO:
        ...
        COLOR DISPLAY value(Syst.Var:ccc) CurRate.Currency WITH FRAME sel.
      END.
      ELSE IF order = 2 THEN DO:
        CHOOSE ROW CurRate.RateDate {Syst/uchoose.i} no-error WITH FRAME sel.
        COLOR DISPLAY value(Syst.Var:ccc) CurRate.RateDate WITH FRAME sel.
      END.
