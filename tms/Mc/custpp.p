...
BROWSE:
   REPEAT WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF ufkey THEN DO:
        ASSIGN
        Syst.CUICommon:ufk[1]= 35  Syst.CUICommon:ufk[2]= 1750 Syst.CUICommon:ufk[3]= 250  Syst.CUICommon:ufk[4]= 927
      ...
      IF order = 1 THEN DO:
        CHOOSE ROW CustPP.CustNum {Syst/uchoose.i} NO-ERROR WITH FRAME sel.
        COLOR DISPLAY VALUE(Syst.CUICommon:ccc) CustPP.CustNum WITH FRAME sel.
      END.
     ...
     ELSE IF LOOKUP(Syst.CUICommon:nap,"2,f2") > 0 THEN DO TRANS:  /* customer's ProdPack */

       FIND CustPP WHERE recid(CustPP) = rtab[frame-line(sel)] NO-LOCK.
       RUN Mc/custpp2.p(CustPP.CustNum).
       ufkey = TRUE.
       NEXT LOOP.
     END.

     ELSE IF LOOKUP(Syst.CUICommon:nap,"3,f3") > 0 THEN DO TRANS:  /* Package Contains */
       RUN local-find-this(FALSE).                                        
