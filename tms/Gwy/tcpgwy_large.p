...
DEFINE OUTPUT PARAMETER ocResponse AS LONGCHAR NO-UNDO.
...
IF lhSocket:CONNECTED() = FALSE THEN DO:
   ...
   ocResponse = 'ERR:Unable to Connect'.
...
ELSE IF ETIME > 3500 THEN DO:
   ...
   ocResponse = 'ERR:Took too long to Connect ' + STRING(ETIME).
...
IF ocResponse = "" THEN DO:
   ...
   DO liLoop = 1 TO piLoops ON ERROR UNDO WAITLOOP, LEAVE WAITLOOP
      ...
      IF NOT lhSocket:CONNECTED() THEN DO:
         ocResponse = "ERR:Lost connection".
         LEAVE WAITLOOP.
      END.
      ...
      IF pcCheck NE "" AND INDEX(ocResponse,pcCheck) > 0 THEN
         LEAVE WAITLOOP.

      IF liLoop = piLoops AND llTimeOut THEN
         ocResponse = "ERR:No response".  
...
PROCEDURE pServerResponse:
   ...
   IF SELF:CONNECTED() = FALSE THEN DO:
      ocResponse = "ERR:Connection lost".
      RETURN.
   END.
   ...
   IF llRC = FALSE OR ERROR-STATUS:GET-MESSAGE(1) <> '' THEN DO:
      ...
      ocResponse = 'ERR:Unable to read response'.
      RETURN.
   END.

   ocResponse = GET-STRING(lmData,1) NO-ERROR.
...
PROCEDURE pWriteSocket:
   ...
   IF llRC = FALSE OR ERROR-STATUS:GET-MESSAGE(1) <> '' THEN DO:
      ocResponse = 'ERR:Unable to write message'.
      RETURN.
   END.
