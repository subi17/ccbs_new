...
DEF VAR dTot   AS DE NO-UNDO FORMAT ">>>>>9.99999".
...
/*      WHEN 4 THEN RETURN ptar / 1024 / 1024. */
   END.
...
FUNCTION fPrice RETURNS DECIMAL:
   ...
   DEF VAR llStartCharge AS LOG NO-UNDO INIT TRUE.
   ...
   ASSIGN
      ...
      xTime  = fUnits(c_dur,ttTariff.RateType). 

   /* Check what prices are found: weekday, weekend, national Holiday ... */
   DO iLoop = 1 TO 6:

      /* how many time zones are defined */
      IF ttTariff.DayType[iLoop] > 0 THEN ASSIGN
         /* calculate amount */
         lPrAmt = lPrAmt + 1.
      /* zone limitation minutes, any bigger than zero ? */
      IF SUBSTR(ttTariff.TZFrom[iLoop],3) NE "00" THEN 
         lTLim  = MIN(lTLim,INT(SUBSTR(ttTariff.TZFrom[iLoop],3))).

      /* check EXTENT types */
      CASE ttTariff.DayType[iLoop]:
         WHEN 1 THEN ASSIGN
            wd1 = MIN(wd1,iLoop)
            wd2 = MAX(wd2,iLoop).
         WHEN 2 THEN ASSIGN
   ...
   /* Minutes.sec. used */
   IF NOT ttTariff.Discount[4] THEN DO:
      /* check IF Minutes.sec. is needed */
      IF xTime < ttTariff.MinSec THEN ASSIGN
         xTime = ttTariff.MinSec
         h2    = c_time + ttTariff.MinSec * ttTariff.RateType
   ...
   IF AVAIL ttCall AND
      ...
      ttCall.EventType EQ "CALL" THEN DO:
      IF ttCall.OrigRecordType NE 0 AND
         ttCall.OrigRecordType NE 1 AND 
   ...
   CASE lPRAmt:
      ...
      WHEN 1 THEN DO:
         ASSIGN
            dTot     = xTime * 
                       (fM2S(ttTariff.Price[1],
                             ttTariff.DataType) * ttTariff.RateType)
            xAmt[1]  = fM2S(ttTariff.Price[1],
                            ttTariff.DataType) * ttTariff.RateType
            xUnit[1] = xTime.
         /* add possible starting fee */
         IF ttTariff.Discount[4] THEN DO:
            ASSIGN
             /* always sub units per  */
            base = (IF llStartCharge THEN ttTariff.StartCharge[1]
                    ELSE 0).  

               /* * (IF llCurrUnit THEN 100 ELSE 1) */ 
            
            IF ttTariff.DataType ne 4 THEN 
            dTot = (xTime * (fM2S(ttTariff.Price[1],
                               ttTariff.DataType) * ttTariff.RateType)) + base.
            ELSE DO:
                dTot = xTime / 1024 * (ttTariff.Price[1] / 1024) + base.
            END.
         END.
   ...
   DO h3 = h1 TO h2:
      ...
      DO iLoop = d1 TO d2:
         IF fC2Sec(ttTariff.TZFrom[iLoop]) <= h4 AND 
            fC2Sec(ttTariff.TZTo[iLoop])   >= h4 THEN DO:
            ASSIGN
               /* Price used FOR rating */
               dPrice = fM2S(ttTariff.Price[iLoop],
                             ttTariff.DataType).

            IF ltBE NE -1 THEN 
               /* END of this time band */
               lTBE   = fC2Sec(ttTariff.TZTo[iLoop]).
               
            /* FIRST TIME: mark possible starting fee */
            IF NOT bFst THEN ASSIGN
               bFst = TRUE
               base = (IF llStartCharge THEN ttTariff.StartCharge[iLoop] 
                       ELSE 0).
                    /* (IF llCurrUnit THEN 100 ELSE 1) */ 
            LEAVE.
         END.
      ...
      IF h4 + ((xTime - iTime) * ttTariff.RateType) <= lTBE OR
         ltBE = -1
      THEN ASSIGN
      ...
      ELSE ASSIGN
         iUnit = iUnit + fUnits((lTBE - h4),ttTariff.RateType)
         iTime = iTime + fUnits((lTBE - h4),ttTariff.RateType)
         h3    = h3    + fUnits((lTBE - h4),ttTariff.RateType).

      ASSIGN
         /* Price * units */
         dTot = dTot + iUnit * (dPrice * ttTariff.RateType)
         /* time band units */
