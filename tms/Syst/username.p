...
ADD-ROW:
      REPEAT WITH FRAME lis ON ENDKEY UNDO ADD-ROW, LEAVE ADD-ROW.
        ...
        REPEAT TRANSACTION WITH FRAME lis:
           ...
           PROMPT-FOR username.usercode
           VALIDATE
              (username.usercode NOT ENTERED OR
              NOT CAN-FIND(username using  username.usercode),
              "Billing RepType " + string(INPUT username.usercode) +
              " already exists !").
           IF INPUT FRAME lis username.usercode = "" THEN 
           LEAVE add-row.
           CREATE username.
           ASSIGN
           username.usercode = INPUT FRAME lis username.usercode.
...
BROWSE:
   REPEAT WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF ufkey THEN DO:
        ASSIGN
        Syst.CUICommon:ufk[1]= 35  Syst.CUICommon:ufk[2]= 30 Syst.CUICommon:ufk[3]= 2229  Syst.CUICommon:ufk[4]= 0
      ...
      IF order = 1 THEN DO:
        ...
        COLOR DISPLAY VALUE(Syst.CUICommon:ccc) username.usercode WITH FRAME sel.
      END.
      ELSE IF order = 2 THEN DO:
        CHOOSE ROW lcname {Syst/uchoose.i} NO-ERROR WITH FRAME sel.
        COLOR DISPLAY VALUE(Syst.CUICommon:ccc) lcname WITH FRAME sel.
      END.
     ...
     ELSE IF LOOKUP(Syst.CUICommon:nap,"2,f2") > 0 THEN DO ON ENDKEY UNDO, NEXT LOOP:
       ...
       IF name ENTERED THEN DO:
          FIND FIRST username WHERE username.name >= name
     ...
     ELSE IF LOOKUP(Syst.CUICommon:nap,"3,f3") > 0 THEN DO:
        run local-find-this(false).
        RUN Mm/namekeys.p(username.usercode).
