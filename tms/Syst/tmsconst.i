/* TMS Global Constants */
/* Hint: Use ({&SOME_VARIABLE}) with messages so that compiler notices if constant does not exists  */
&IF "{&TMSCONST_I}" NE "YES"
&THEN
&GLOBAL-DEFINE TMSCONST_I YES

&GLOBAL-DEFINE REQ_INACTIVE_STATUSES "2,4,9,99"
&GLOBAL-DEFINE REQ_ONGOING_STATUSES "0,1,3,5,6,7,8,15,16,17,19,20"
&GLOBAL-DEFINE ORDER_INACTIVE_STATUSES "6,7,8,9"
&GLOBAL-DEFINE ORDER_CLOSE_STATUSES "7,8,9"
&GLOBAL-DEFINE ORDER_ROI_STATUSES "41,42,43,44"
&GLOBAL-DEFINE SKIP_MsRequest "999"
&GLOBAL-DEFINE ORDER_LOGISTICS_STATUSES "15,16"

/*Payment Methods for OrderPayment.Method*/
&GLOBAL-DEFINE ORDERPAYMENT_M_POD 1
&GLOBAL-DEFINE ORDERPAYMENT_M_CREDIT_CARD 2
&GLOBAL-DEFINE ORDERPAYMENT_M_PAYPAL 6

/* Order.StatusCode */
&GLOBAL-DEFINE ORDER_STATUS_NEW "1"
&GLOBAL-DEFINE ORDER_STATUS_ERROR "2"
&GLOBAL-DEFINE ORDER_STATUS_MNP "3"
&GLOBAL-DEFINE ORDER_STATUS_IN_CONTROL "4"
&GLOBAL-DEFINE ORDER_STATUS_DELIVERED "6"
&GLOBAL-DEFINE ORDER_STATUS_CLOSED "7"
&GLOBAL-DEFINE ORDER_STATUS_CLOSED_BY_FRAUD "8"
&GLOBAL-DEFINE ORDER_STATUS_AUTO_CLOSED "9"
&GLOBAL-DEFINE ORDER_STATUS_ONGOING "12"
&GLOBAL-DEFINE ORDER_STATUS_SENDING_TO_LO "15"
&GLOBAL-DEFINE ORDER_STATUS_PENDING_ICC_FROM_LO "16"
&GLOBAL-DEFINE ORDER_STATUS_PENDING_ICC_FROM_INSTALLER "17"
&GLOBAL-DEFINE ORDER_STATUS_COMPANY_NEW "20"
&GLOBAL-DEFINE ORDER_STATUS_COMPANY_MNP "21"
&GLOBAL-DEFINE ORDER_STATUS_MNP_ON_HOLD "22"
&GLOBAL-DEFINE ORDER_STATUS_RENEWAL "30"
&GLOBAL-DEFINE ORDER_STATUS_RENEWAL_HOLD "31"
&GLOBAL-DEFINE ORDER_STATUS_RENEWAL_STC "32"
&GLOBAL-DEFINE ORDER_STATUS_RENEWAL_STC_COMPANY "33"
&GLOBAL-DEFINE ORDER_STATUS_ROI_LEVEL_1 "41" 
&GLOBAL-DEFINE ORDER_STATUS_ROI_LEVEL_2 "42" 
&GLOBAL-DEFINE ORDER_STATUS_ROI_LEVEL_3 "43" 
&GLOBAL-DEFINE ORDER_STATUS_OFFER_SENT "50" 
&GLOBAL-DEFINE ORDER_STATUS_RESIGNATION "51" 
&GLOBAL-DEFINE ORDER_STATUS_MORE_DOC_NEEDED "44" 
&GLOBAL-DEFINE ORDER_STATUS_MIGRATION_PENDING "60" /*waiting for sending NC*/
&GLOBAL-DEFINE ORDER_STATUS_MIGRATION_ONGOING "61" /*request sent to NC*/
&GLOBAL-DEFINE ORDER_STATUS_MIGRATION_WAITING_ACTIVATION "62" /*waiting for activation time*/
&GLOBAL-DEFINE ORDER_STATUS_MNP_REJECTED "73" 
&GLOBAL-DEFINE ORDER_STATUS_MNP_PENDING "74" 
&GLOBAL-DEFINE ORDER_STATUS_MNP_RETENTION "75"
&GLOBAL-DEFINE ORDER_STATUS_PENDING_MAIN_LINE "76"
&GLOBAL-DEFINE ORDER_STATUS_PENDING_FIXED_LINE "77"
&GLOBAL-DEFINE ORDER_STATUS_WAITING_SENDING_LO "78"
&GLOBAL-DEFINE ORDER_STATUS_PENDING_MOBILE_LINE "79"
&GLOBAL-DEFINE ORDER_STATUS_PENDING_FIXED_LINE_CANCEL "80"
&GLOBAL-DEFINE ORDER_STATUS_SIM_ONLY_MNP_IN "99"

/* OrderFusion.FusionStatusCode  */
&GLOBAL-DEFINE FUSION_ORDER_STATUS_NEW "NEW"
&GLOBAL-DEFINE FUSION_ORDER_STATUS_INITIALIZED "INT"
&GLOBAL-DEFINE FUSION_ORDER_STATUS_ONGOING "ONG"
&GLOBAL-DEFINE FUSION_ORDER_STATUS_CANCELLED "CAN"
&GLOBAL-DEFINE FUSION_ORDER_STATUS_FINALIZED "FIN"
&GLOBAL-DEFINE FUSION_ORDER_STATUS_PENDING_CANCELLED "PCAN"
&GLOBAL-DEFINE FUSION_ORDER_STATUS_ERROR "ERR"
&GLOBAL-DEFINE FUSION_ORDER_STATUS_REJECTED "REJ"

&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_NEW "NEW"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_ONGOING "ONGOING"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_HANDLED "HANDLED"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_ERROR "ERROR"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_WRONG_TYPE "WRONG_TYPE"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_SENT "SENT"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_CANCELLED "CANCELLED"

&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_LOGISTICS_SENT "SENT_TO_LOGISTICS"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_PENDING_ACTIVATION "ACTIVATION_PENDING_BY_VENDOR"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_WAITING_TO_SENT_FOR_LOGISTICS "WAITING_TO_SEND_FOR_LOGISTICS"
&GLOBAL-DEFINE FUSIONMESSAGE_STATUS_WAITING_FOR_ACTIVATION "WAITING_FOR_ACTIVATION"

&GLOBAL-DEFINE FUSIONMESSAGE_SOURCE_MASMOVIL "MasMovil"
&GLOBAL-DEFINE FUSIONMESSAGE_SOURCE_TMS "TMS"

&GLOBAL-DEFINE FUSION_FIXED_NUMBER_TYPE_MNP "MNP"
&GLOBAL-DEFINE FUSION_FIXED_NUMBER_TYPE_NEW "NEW"
/* Third Party Service */
&GLOBAL-DEFINE SOURCE_TMS 			             		 "TMS"
&GLOBAL-DEFINE SOURCE_LOGISTICS 						 "LOperator"
&GLOBAL-DEFINE SOURCE_TV_STB_VENDOR   				     "Huawei"
&GLOBAL-DEFINE STATUS_NEW                        		 "NEW"
&GLOBAL-DEFINE STATUS_ERROR                      		 "ERROR"
&GLOBAL-DEFINE STATUS_ONGOING                    		 "ONGOING"
&GLOBAL-DEFINE STATUS_CANCELLATION_NEW				     "CANCELLATION_NEW"
&GLOBAL-DEFINE STATUS_CANCELLATION_ONGOING				 "CANCELLATION_ONGOING"			
&GLOBAL-DEFINE STATUS_HANDLED                    	     "HANDLED"
&GLOBAL-DEFINE STATUS_CANCELED                    	     "CANCELED"
&GLOBAL-DEFINE STATUS_ACTIVATED 				 		 "ACTIVATED"
&GLOBAL-DEFINE STATUS_DEACTIVATED 				 		 "DEACTIVATED"
&GLOBAL-DEFINE STATUS_LOGISTICS_INITIATED        		 "LOGISTICS_INITIATED"
&GLOBAL-DEFINE TYPE_ACTIVATION                   		 "ACTIVATION"
&GLOBAL-DEFINE TYPE_DEACTIVATION                 		 "DEACTIVATION"
&GLOBAL-DEFINE WAITING_FOR_STB_ACTIVATION        		 "WAITING_FOR_STB_ACTIVATION"
&GLOBAL-DEFINE WAITING_FOR_STB_DEACTIVATION      		 "WAITING_FOR_STB_DEACTIVATION"
&GLOBAL-DEFINE WAITING_FOR_STB_ACTIVATION_CONFIRMATION   "WAITING_FOR_STB_ACTIVATION_CONFIRMATION"
&GLOBAL-DEFINE WAITING_FOR_STB_DEACTIVATION_CONFIRMATION "WAITING_FOR_STB_DEACTIVATION_CONFIRMATION"

/* OrderGroup */
&GLOBAL-DEFINE OG_LOFILE "LOFILE"

/* Logistics Despachar Value */
&GLOBAL-DEFINE DESPACHAR_TRUE_VALUE "01"
&GLOBAL-DEFINE DESPACHAR_FALSE_VALUE "02"

/* OrderCustomer.RowType  */
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_AGREEMENT 1
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_INVOICE 2
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_USER 3
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_DELIVERY 4
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_CIF_CONTACT 5
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_FIXED_INSTALL 6
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_FIXED_BILLING 7
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_LOGISTICS 8
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_MOBILE_POUSER 9
&GLOBAL-DEFINE ORDERCUSTOMER_ROWTYPE_FIXED_POUSER 10

/* Dextra */
&GLOBAL-DEFINE DEXTRA_CANCELLED_STATUSES "3,66,875,22"

/* OrderType */
&GLOBAL-DEFINE ORDER_TYPE_NEW 0
&GLOBAL-DEFINE ORDER_TYPE_MNP 1
&GLOBAL-DEFINE ORDER_TYPE_RENEWAL 2
&GLOBAL-DEFINE ORDER_TYPE_ROLLBACK 3
&GLOBAL-DEFINE ORDER_TYPE_STC 4

/* Order.DeliveryType */
&GLOBAL-DEFINE ORDER_DELTYPE_COURIER 1
&GLOBAL-DEFINE ORDER_DELTYPE_KIALA 2
&GLOBAL-DEFINE ORDER_DELTYPE_POST_SECURE 3
&GLOBAL-DEFINE ORDER_DELTYPE_POST 4
&GLOBAL-DEFINE ORDER_DELTYPE_POS_SECURE 5
&GLOBAL-DEFINE ORDER_DELTYPE_POS 6

/* OrderTimeStamp */
&GLOBAL-DEFINE ORDERTIMESTAMP_CHANGE 1
&GLOBAL-DEFINE ORDERTIMESTAMP_DELIVERY 2
&GLOBAL-DEFINE ORDERTIMESTAMP_CLOSE 3
&GLOBAL-DEFINE ORDERTIMESTAMP_PRINT 4
&GLOBAL-DEFINE ORDERTIMESTAMP_SIMONLY 5
&GLOBAL-DEFINE ORDERTIMESTAMP_SEND 6

/* Funcrun Config cosntants */
&GLOBAL-DEFINE INVPRINTSPLIT_CONFIG 5

/* TMRule */
&GLOBAL-DEFINE TMRULE_PERIOD_BALANCE 3

/* Common TMS CUI Messages */
&GLOBAL-DEFINE MSG_ONG_REQUEST "Function not allowed due to existing requests"
&GLOBAL-DEFINE MSG_NOT_ALLOWED "Function not allowed due to business rules"
&GLOBAL-DEFINE MSG_NOT_RIGHTS  "Function not allowed due to user rights"
&GLOBAL-DEFINE MSG_INCORRECT_VALUE "Incorrect or missing value"
&GLOBAL-DEFINE MSG_TRY_AGAIN "Try again"
&GLOBAL-DEFINE MSG_DATA_NOT_FOUND "Data not found"
&GLOBAL-DEFINE MSG_RECORD_DELETED "Record has been deleted by another user!"
&GLOBAL-DEFINE MSG_RECORD_CHANGED "Record has changed while you were working"
&GLOBAL-DEFINE MSG_RECORD_LOCKED "Record has been locked by another user"

&GLOBAL-DEFINE MSG_LAST_PAGE "YOU ARE ON THE LAST PAGE !"

/* MSISDN STOCKS */
&GLOBAL-DEFINE MSISDN_STOCK_ONLINE "ONLINE"
&GLOBAL-DEFINE MSISDN_STOCK_OFFLINE "OFFLINE"
&GLOBAL-DEFINE MSISDN_STOCK_VIP "VIP"
&GLOBAL-DEFINE MSISDN_STOCK_NOTPUBLIC "NOTPUBLIC"
&GLOBAL-DEFINE MSISDN_STOCK_PREACTIVATED "PREACTIVATED"

/* msisdn statuses */
&GLOBAL-DEFINE MSISDN_ST_UNKNOWN 0
&GLOBAL-DEFINE MSISDN_ST_AVAILABLE 1
&GLOBAL-DEFINE MSISDN_ST_ASSIGNED_TO_SUBSCR 3
&GLOBAL-DEFINE MSISDN_ST_QUARANTINE 4
&GLOBAL-DEFINE MSISDN_ST_MNP_OUT_YOIGO 6 
&GLOBAL-DEFINE MSISDN_ST_WAITING_RETURN 11
&GLOBAL-DEFINE MSISDN_ST_RETURNED 12
&GLOBAL-DEFINE MSISDN_ST_MNP_OUT_OTHER 13
&GLOBAL-DEFINE MSISDN_ST_RETURN_NOTICE_SENT 14
&GLOBAL-DEFINE MSISDN_ST_RETURNED_TO_YOIGO 15
&GLOBAL-DEFINE MSISDN_ST_ASSIGNED_TO_ORDER 22

/* PATHS & FILES */
&GLOBAL-DEFINE PATH_MSISDN_RELEASE "/scratch/log/msisdn_release/"

/* ICC Stock types */
&GLOBAL-DEFINE ICC_STOCK_TYPES "NEW,MNP,CC"
&GLOBAL-DEFINE ICC_STOCK_INSTALLER "INSTALLER"
&GLOBAL-DEFINE ICC_STOCK_LO "LO"

/* SIM.SimStat */
&GLOBAL-DEFINE SIM_SIMSTAT_AVAILABLE 1
&GLOBAL-DEFINE SIM_SIMSTAT_IN_USE 4
&GLOBAL-DEFINE SIM_SIMSTAT_TEMP 5
&GLOBAL-DEFINE SIM_SIMSTAT_LOST 7
&GLOBAL-DEFINE SIM_SIMSTAT_RELEASED 9
&GLOBAL-DEFINE SIM_SIMSTAT_RESERVED 13
&GLOBAL-DEFINE SIM_SIMSTAT_LOST 14
&GLOBAL-DEFINE SIM_SIMSTAT_CHOSEN_TO_ORDER 20
&GLOBAL-DEFINE SIM_SIMSTAT_SENT_TO_LOGISTICS 21

/* Limit Types */
&GLOBAL-DEFINE LIMIT_TYPE_TMRLIMIT 1
&GLOBAL-DEFINE LIMIT_TYPE_SUBQTY 2
&GLOBAL-DEFINE LIMIT_TYPE_BILLPERM 3
&GLOBAL-DEFINE LIMIT_TYPE_SUBACTQTY 4
&GLOBAL-DEFINE LIMIT_TYPE_RISKLIMIT 5
&GLOBAL-DEFINE LIMIT_TYPE_Q25_DISCOUNT 6

&GLOBAL-DEFINE LIMIT_BILLPERM_PROHIBITED 2
&GLOBAL-DEFINE LIMIT_BILLPERM_ALLOWED 0
&GLOBAL-DEFINE LIMIT_BILLPERM_SUSPENDED 1

&GLOBAL-DEFINE SLANALYSE_FULL_PACKET  1
&GLOBAL-DEFINE SLANALYSE_BROKEN_PACKET 2
&GLOBAL-DEFINE SLANALYSE_NO_PACKET 0

/* CustContactTypes */
&GLOBAL-DEFINE CUSTCONTACT_REPRESENTATIVE 1
&GLOBAL-DEFINE CUSTCONTACT_CONTACT 5

/* DayCampaign.DCType */
&GLOBAL-DEFINE DCTYPE_SERVICE_PACKAGE "1"
&GLOBAL-DEFINE DCTYPE_RATING_LIMIT "2"
&GLOBAL-DEFINE DCTYPE_DISCOUNT "3"
&GLOBAL-DEFINE DCTYPE_BUNDLE "4"
&GLOBAL-DEFINE DCTYPE_INSTALLMENT "5"
&GLOBAL-DEFINE DCTYPE_POOL_RATING "6"
&GLOBAL-DEFINE DCTYPE_CUMULATIVE_RATING "8"

&GLOBAL-DEFINE DAYCAMPAIGN_STATUSCODE_ACTIVE 1 

&GLOBAL-DEFINE DC_BUNDLE_TARGET_MOBILE 0
&GLOBAL-DEFINE DC_BUNDLE_TARGET_FIXED  1
&GLOBAL-DEFINE TELEVISION_BUNDLE       2
&GLOBAL-DEFINE DC_BUNDLE_TARGET_SVA    3

/* DayCampaign.BundleType */
&GLOBAL-DEFINE DC_BUNDLE_TYPE_UNDEFINED     0
&GLOBAL-DEFINE DC_BUNDLE_TYPE_TARIFF        1
&GLOBAL-DEFINE DC_BUNDLE_TYPE_ADDITIONAL    2


&GLOBAL-DEFINE LANGUAGES "es_ES,es_CA,es_EU,es_GA,en"

/* Request Types */
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_TYPE_CHANGE 0        
&GLOBAL-DEFINE REQTYPE_SERVICE_CHANGE 1
&GLOBAL-DEFINE REQTYPE_SALDO_PAYMENT 2
&GLOBAL-DEFINE REQTYPE_USER_CHANGE 3
&GLOBAL-DEFINE REQTYPE_INVOICE_CUSTOMER_CHANGE 4
&GLOBAL-DEFINE REQTYPE_USER_ACCOUNT_CHANGE 5
&GLOBAL-DEFINE REQTYPE_ADDRESS_CHANGE 6
&GLOBAL-DEFINE REQTYPE_CONTRACT_PIN 7
&GLOBAL-DEFINE REQTYPE_CONTRACT_ACTIVATION 8
&GLOBAL-DEFINE REQTYPE_CONTRACT_TERMINATION 9
&GLOBAL-DEFINE REQTYPE_AGREEMENT_CUSTOMER_CHANGE 10
&GLOBAL-DEFINE REQTYPE_PAYMENT_PLAN 11
&GLOBAL-DEFINE REQTYPE_MARKETING_VALUES 12
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_CREATE 13
&GLOBAL-DEFINE REQTYPE_FIXED_LINE_CREATE 14
&GLOBAL-DEFINE REQTYPE_ICC_CHANGE 15
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_TERMINATION 18
&GLOBAL-DEFINE REQTYPE_MSISDN_CHANGE 19
&GLOBAL-DEFINE REQTYPE_ONDEMAND_INVOICE 20
&GLOBAL-DEFINE REQTYPE_CREDIT_NOTE 22       
&GLOBAL-DEFINE REQTYPE_REFUND 23       
&GLOBAL-DEFINE REQTYPE_BANK_ACCOUNT_NUMBER_CHANGE 24       
&GLOBAL-DEFINE REQTYPE_FEEMODEL_PRICE_CHANGE 27 
&GLOBAL-DEFINE REQTYPE_PRICE_GUARANTEE  28
&GLOBAL-DEFINE REQTYPE_SMS_SEND 30
&GLOBAL-DEFINE REQTYPE_PAYMENT_CREATE 31
&GLOBAL-DEFINE REQTYPE_BANK_REF_UPDATE 32       
&GLOBAL-DEFINE REQTYPE_CREDIT_CHECK 33
&GLOBAL-DEFINE REQTYPE_MANUAL_PAYMENT 34       
&GLOBAL-DEFINE REQTYPE_BARRING_REQUEST 35
&GLOBAL-DEFINE REQTYPE_HEAT_BALANCE 37
&GLOBAL-DEFINE REQTYPE_AFTER_SALES_ORDER 46
&GLOBAL-DEFINE REQTYPE_REVERT_RENEWAL_ORDER 49
&GLOBAL-DEFINE REQTYPE_RERATE 65
&GLOBAL-DEFINE REQTYPE_CHARGE_AND_COMPENSATION 76
&GLOBAL-DEFINE REQTYPE_DUPLICATE_INVOICE 77
&GLOBAL-DEFINE REQTYPE_BUNDLE 78
&GLOBAL-DEFINE REQTYPE_SMS_INVOICE 79
&GLOBAL-DEFINE REQTYPE_IMEI_CHANGE 80
&GLOBAL-DEFINE REQTYPE_BUNDLE_CHANGE 81
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_REACTIVATION 82
&GLOBAL-DEFINE REQTYPE_DSS 83
&GLOBAL-DEFINE REQTYPE_ACTIVATE_EMAIL_INVOICE 84
&GLOBAL-DEFINE REQTYPE_SEND_EMAIL_INVOICE 85
&GLOBAL-DEFINE REQTYPE_EMAIL_SENDING 86
&GLOBAL-DEFINE REQTYPE_LOGISTICS 87
&GLOBAL-DEFINE REQTYPE_INSTALLMENT_CONTRACT_CHANGE 88
&GLOBAL-DEFINE REQTYPE_TERMINAL_FINANCE_BANK_FILE 89
&GLOBAL-DEFINE REQTYPE_FUSION_EMAIL 90
&GLOBAL-DEFINE REQTYPE_TERMINAL_FINANCE_CAN_TER_BANK_FILE 91
&GLOBAL-DEFINE REQTYPE_PUBLISH_INVOICE 92
&GLOBAL-DEFINE REQTYPE_PUBLISH_IFS 93
&GLOBAL-DEFINE REQTYPE_PUSH_INVOICE 94
&GLOBAL-DEFINE REQTYPE_PRO_MIGRATION 95
&GLOBAL-DEFINE REQTYPE_E_INVOICE 96  
&GLOBAL-DEFINE REQTYPE_CATEGORY_CHG 97  

&GLOBAL-DEFINE REQTYPES_HPD "0,1,6,8,9,10,13,15,18,19,35,46,49,80,81,82,88"
&GLOBAL-DEFINE HPD_SERVICES "VMS,LANG,CF,IRDCUTOFF,BB,NAM,CALLSPEC,LTE,BPSUB,LP,NW"

&GLOBAL-DEFINE ORDER_CHANNEL_DIRECT "cc,telesales,self,emission,fusion_telesales,fusion_emission,fusion_cc,inversa,cc_pro,telesales_pro,emission_pro,fusion_telesales_pro,fusion_emission_pro,fusion_cc_pro,fusion_self"
&GLOBAL-DEFINE ORDER_CHANNEL_DIRECT_RENEWAL "renewal,renewal_telesales,retention,renewal_ctc,retention_stc"
&GLOBAL-DEFINE ORDER_CHANNEL_INDIRECT "pos,fusion_pos,pos_pro,fusion_pos_pro"

/* Request Statuses */
&GLOBAL-DEFINE REQUEST_STATUS_NEW 0
&GLOBAL-DEFINE REQUEST_STATUS_UNDER_WORK 1
&GLOBAL-DEFINE REQUEST_STATUS_DONE 2
&GLOBAL-DEFINE REQUEST_STATUS_REJECTED 3               /* Rejected (error occurred) */
&GLOBAL-DEFINE REQUEST_STATUS_CANCELLED 4              /* Cancelled */
&GLOBAL-DEFINE REQUEST_STATUS_HLR_PENDING 5            /* Pending HLR */
&GLOBAL-DEFINE REQUEST_STATUS_HLR_DONE 6               /* Pending - HLR DONE */
&GLOBAL-DEFINE REQUEST_STATUS_SUB_REQUEST_PENDING 7    /* Pending Sub-request */
&GLOBAL-DEFINE REQUEST_STATUS_SUB_REQUEST_DONE 8       /* Sub-request DONE */
&GLOBAL-DEFINE REQUEST_STATUS_HANDLED 9                /* Handled */
&GLOBAL-DEFINE REQUEST_STATUS_APPROVED 15              /* Approved for handling */
&GLOBAL-DEFINE REQUEST_STATUS_FILE_PENDING 16          /* Pending file creation */
&GLOBAL-DEFINE REQUEST_STATUS_FILE_DONE 17             /* File created */
&GLOBAL-DEFINE REQUEST_STATUS_CONFIRMATION_PENDING 19  /* Waiting for confirmation */
&GLOBAL-DEFINE REQUEST_STATUS_PROCESS_ERROR 99         /* Process error (Qvantel maintenance required) */

/* Request Sources */
&GLOBAL-DEFINE REQUEST_SOURCE_SUBSCRIPTION_CREATION "1"
&GLOBAL-DEFINE REQUEST_SOURCE_STC "2"
&GLOBAL-DEFINE REQUEST_SOURCE_SUBSCRIPTION_TERMINATION "3"
&GLOBAL-DEFINE REQUEST_SOURCE_MANUAL_TMS "4"
&GLOBAL-DEFINE REQUEST_SOURCE_SCRIPT "5"
&GLOBAL-DEFINE REQUEST_SOURCE_NEWTON "6"
&GLOBAL-DEFINE REQUEST_SOURCE_RENEWAL "7"
&GLOBAL-DEFINE REQUEST_SOURCE_IFS "9"
&GLOBAL-DEFINE REQUEST_SOURCE_YOIGO_TOOL "11"
&GLOBAL-DEFINE REQUEST_SOURCE_ACC "12"
&GLOBAL-DEFINE REQUEST_SOURCE_ORDER_CANCELLATION "14"
&GLOBAL-DEFINE REQUEST_SOURCE_EXTERNAL_API "15"
&GLOBAL-DEFINE REQUEST_SOURCE_BUNDLE_STC "16"
&GLOBAL-DEFINE REQUEST_SOURCE_BILLING "17"
&GLOBAL-DEFINE REQUEST_SOURCE_BTC "18"
&GLOBAL-DEFINE REQUEST_SOURCE_SUBSCRIPTION_REACTIVATION "19"
&GLOBAL-DEFINE REQUEST_SOURCE_DSS "20"
&GLOBAL-DEFINE REQUEST_SOURCE_CONTRACT_TERMINATION "21"
&GLOBAL-DEFINE REQUEST_SOURCE_REVERT_RENEWAL_ORDER "22"
&GLOBAL-DEFINE REQUEST_SOURCE_MULTISIM "23"
&GLOBAL-DEFINE REQUEST_SOURCE_INSTALLMENT_CONTRACT_CHANGE "24"
&GLOBAL-DEFINE REQUEST_SOURCE_ICC_CHANGE_AUTO "25"
&GLOBAL-DEFINE REQUEST_SOURCE_CONTS30_PROMOTION "26"
&GLOBAL-DEFINE REQUEST_SOURCE_SERVICE_CHANGE "27"
&GLOBAL-DEFINE REQUEST_SOURCE_FUSION_ORDER_FALLBACK "28"
&GLOBAL-DEFINE REQUEST_SOURCE_FUSION_ORDER "29"
&GLOBAL-DEFINE REQUEST_SOURCE_FUSION_EMAIL "30"
&GLOBAL-DEFINE REQUEST_SOURCE_CONTRACT_ACTIVATION "31"
&GLOBAL-DEFINE REQUEST_SOURCE_MAIN_LINE_DEACTIVATION "32"
&GLOBAL-DEFINE REQUEST_SOURCE_BARRING "33"
&GLOBAL-DEFINE REQUEST_SOURCE_RETAIL_NEWTON "34"
&GLOBAL-DEFINE REQUEST_SOURCE_MIGRATION "35"
&GLOBAL-DEFINE REQUEST_SOURCE_TV_SERVICE_ACTIVATION "36"
&GLOBAL-DEFINE REQUEST_SOURCE_TV_SERVICE_DEACTIVATION "37"

&GLOBAL-DEFINE FUSIONMESSAGE_TYPE_RESERVE_NUMBER "ReserveNumber"
&GLOBAL-DEFINE FUSIONMESSAGE_TYPE_CREATE_ORDER "CreateOrder"
&GLOBAL-DEFINE FUSIONMESSAGE_TYPE_CANCEL_ORDER "CancelOrder"
&GLOBAL-DEFINE FUSIONMESSAGE_TYPE_UPDATE_STATUS "UpdateStatus"
&GLOBAL-DEFINE FUSIONMESSAGE_TYPE_LOGISTICS "Logistics"

&GLOBAL-DEFINE THIRDPARTY_DEVICE "ThirdPartyDevice"
&GLOBAL-DEFINE THIRDPARTY_DEVICE_TV "ThirdPartyDeviceTV"
&GLOBAL-DEFINE THIRDPARTY_SERVICE_ACTIVATION "ThirdPartyDeviceActivation"

&GLOBAL-DEFINE REQUEST_SOURCES_HPD "4,6,7,11,15"

/* Request Action Rule List */
&GLOBAL-DEFINE REQUEST_ACTIONLIST_ALL "1,2,3,5,6,11"
&GLOBAL-DEFINE REQUEST_ACTIONLIST_PENALTYFEE "5,11"
&GLOBAL-DEFINE REQUEST_ACTIONLIST_ALL_EXCL_PENALTYFEE "1,2,3,6"

/* Mobsub Termimation Reason */
&GLOBAL-DEFINE SUBSCRIPTION_TERM_REASON_MNP 2 
&GLOBAL-DEFINE SUBSCRIPTION_TERM_REASON_ORDER_CANCELLATION 3 
&GLOBAL-DEFINE SUBSCRIPTION_TERM_REASON_POS_ORDER_CANCELATION 11 
&GLOBAL-DEFINE SUBSCRIPTION_TERM_REASON_DIRECT_ORDER_CANCELATION 12 
&GLOBAL-DEFINE SUBSCRIPTION_TERM_REASON_MULTISIM 13
&GLOBAL-DEFINE SUBSCRIPTION_TERM_REASON_ADDITIONALSIM 14

/* Mobsub Termimation Types */
&GLOBAL-DEFINE TERMINATION_TYPE_FULL "Full"
&GLOBAL-DEFINE TERMINATION_TYPE_PARTIAL "Partial"

/* user limits for charges and compensations types */
&GLOBAL-DEFINE POST_CHARGE_LIMIT_TYPE 1
&GLOBAL-DEFINE POST_COMP_LIMIT_TYPE 2
&GLOBAL-DEFINE PREP_CHARGE_LIMIT_TYPE 3
&GLOBAL-DEFINE PREP_COMP_LIMIT_TYPE 4
&GLOBAL-DEFINE POST_CHARGE_MONTHLY_LIMIT_TYPE 5
&GLOBAL-DEFINE POST_COMP_MONTHLY_LIMIT_TYPE 6
&GLOBAL-DEFINE PREP_CHARGE_MONTHLY_LIMIT_TYPE 7
&GLOBAL-DEFINE PREP_COMP_MONTHLY_LIMIT_TYPE 8
&GLOBAL-DEFINE CREDIT_NOTE_LIMIT_TYPE 9

/* MNPProcess.MNPType */
&GLOBAL-DEFINE MNP_TYPE_OLD 0
&GLOBAL-DEFINE MNP_TYPE_IN 1
&GLOBAL-DEFINE MNP_TYPE_OUT 2
&GLOBAL-DEFINE MNP_TYPE_TERMINATION 3
&GLOBAL-DEFINE MNP_TYPE_MIGRATION 5

/* MNPProcess.StatusCode (common) */
&GLOBAL-DEFINE MNP_ST_NEW 0   /* MNP order status init value. */
&GLOBAL-DEFINE MNP_ST_ERROR 999
/* MNPProcess.StatusCode (in & out) */
&GLOBAL-DEFINE MNP_ST_AENV 1  /* MNP request has been sent */
&GLOBAL-DEFINE MNP_ST_ASOL 2  /* MNP request read by the donor operator. */
&GLOBAL-DEFINE MNP_ST_AREC 4  /* MNP rejected by donor operator. Status to 73*/
&GLOBAL-DEFINE MNP_ST_ACON 5  /* Both operators accepted porting. */
&GLOBAL-DEFINE MNP_ST_APOR 6  /* Porting accomplished. */
&GLOBAL-DEFINE MNP_ST_ACAN 7  /* MNP cancel by customer or operator. Close order*/
&GLOBAL-DEFINE MNP_ST_AREC_CLOSED 8
/* MNPProcess.StatusCode (migration) */
&GLOBAL-DEFINE MNP_ST_MENV 10 
&GLOBAL-DEFINE MNP_ST_MPRC 11
&GLOBAL-DEFINE MNP_ST_MFIN 12
&GLOBAL-DEFINE MNP_ST_NENV 13
&GLOBAL-DEFINE MNP_ST_NCON 14
&GLOBAL-DEFINE MNP_ST_NMIG 15
/* MNPProcess.StatusCode (termination) */
&GLOBAL-DEFINE MNP_ST_BNOT 20 
&GLOBAL-DEFINE MNP_ST_BCAN 21
&GLOBAL-DEFINE MNP_ST_BDEF 22
&GLOBAL-DEFINE MNP_ST_BDET 23

/* MNPProcess.StateFlag */
&GLOBAL-DEFINE MNP_STATEFLAG_NOT_ANALYSED 0 
&GLOBAL-DEFINE MNP_STATEFLAG_REJECT_PROPOSAL 1 
&GLOBAL-DEFINE MNP_STATEFLAG_REJECT 2 
&GLOBAL-DEFINE MNP_STATEFLAG_CONFIRM_PROPOSAL 3
&GLOBAL-DEFINE MNP_STATEFLAG_CONFIRM 4
&GLOBAL-DEFINE MNP_STATEFLAG_WAITING_CONFIRM 5

/* MNP cancel proposal statuses */
&GLOBAL-DEFINE MNP_CANCEL_PROPOSAL_NEW 0
&GLOBAL-DEFINE MNP_CANCEL_PROPOSAL_CONFIRMED 1
&GLOBAL-DEFINE MNP_CANCEL_PROPOSAL_REJECTED 2
&GLOBAL-DEFINE MNP_CANCEL_PROPOSAL_CANCELLED 3
&GLOBAL-DEFINE MNP_CANCEL_PROPOSAL_RECIPIENT 4

/* MNPOperation.Sender */
&GLOBAL-DEFINE MNP_SENDER_TMS 1
&GLOBAL-DEFINE MNP_SENDER_MNP 2
&GLOBAL-DEFINE MNP_SENDER_BUZON 3
&GLOBAL-DEFINE MNP_SENDER_ADAPTER 4

/* MNPProcess.MNPSeq */
&GLOBAL-DEFINE MNP_PROCESS_QUERY 50000000
&GLOBAL-DEFINE MNP_PROCESS_DUMMY_IN 100000000
&GLOBAL-DEFINE MNP_PROCESS_DUMMY_OUT 100000001

/* MNPOperation.StatusCode */
&GLOBAL-DEFINE MNP_MSG_WAITING_SEND 1
&GLOBAL-DEFINE MNP_MSG_SENT_TO_FILE 2
&GLOBAL-DEFINE MNP_MSG_WAITING_CONFIRM 4
&GLOBAL-DEFINE MNP_MSG_WAITING_RESPONSE_HANDLE 5
&GLOBAL-DEFINE MNP_MSG_HANDLED 10
&GLOBAL-DEFINE MNP_MSG_ERROR 999 /* ccbs handling error */
&GLOBAL-DEFINE MNP_MSG_DELAYED 998 /* message is delayed */
&GLOBAL-DEFINE MNP_MSG_PARSING 900 /* request/response parsing error */
&GLOBAL-DEFINE MNP_MSG_NC 901 /* NC error */
&GLOBAL-DEFINE MNP_MSG_HANDLING 902 /* handling error */
&GLOBAL-DEFINE MNP_MSG_ADAPTER 903 /* mnp adapter error */

/* MNPOperation.ErrorCode */
&GLOBAL-DEFINE MNP_ERRORCODE_PARSE "MNP_PARSE"
&GLOBAL-DEFINE MNP_ERRORCODE_HANDLE "MNP_PROCE"
&GLOBAL-DEFINE MNP_ERRORCODE_ADAPTER "MNP_ADAPT"

/* Common MNP handling errors */
&GLOBAL-DEFINE MNP_ERROR_UNKNOWN_PROCESS "Unknown MNP process"
&GLOBAL-DEFINE MNP_ERROR_WRONG_TYPE "Wrong MNP process type"
&GLOBAL-DEFINE MNP_ERROR_WRONG_STATUS "Wrong statuscode"

/* MNPOperation.ErrorHandled */
&GLOBAL-DEFINE MNP_ERRORHANDLED_NO 1
&GLOBAL-DEFINE MNP_ERRORHANDLED_YES 2

/* BillItem Definitions */
&GLOBAL-DEFINE BITEM_GRP_INTERNET "3"
&GLOBAL-DEFINE BITEM_GRP_TERMINAL "7"
&GLOBAL-DEFINE BITEM_GRP_CHARGE "31"
&GLOBAL-DEFINE BITEM_GRP_COMPENSATION "32"
&GLOBAL-DEFINE BITEM_GRP_LAPTOP "34"
&GLOBAL-DEFINE BITEM_GRP_DATABUNDLE "35"
&GLOBAL-DEFINE BITEM_GRP_DATABUNDLE2 "37"
&GLOBAL-DEFINE BITEM_GRP_TERMINAL_ACCOUNT 70020105
&GLOBAL-DEFINE BITEM_GRP_TERMINAL_TAXCLASS "2"
&GLOBAL-DEFINE BITEM_GRP_TERMINAL_SAPRID "032"
&GLOBAL-DEFINE BITEM_GRP_ACCESSORY "43"
&GLOBAL-DEFINE BITEM_GRP_GB "44"


/* ROI Send History Status */
&GLOBAL-DEFINE ROI_HISTORY_TO_SEND 1 
&GLOBAL-DEFINE ROI_HISTORY_SENT 2 
&GLOBAL-DEFINE ROI_HISTORY_ERROR 3 

/* Payment method */
&GLOBAL-DEFINE PAYM_METHOD_PAYPAL "6"

/* Invoice.ChargeType*/
&GLOBAL-DEFINE INV_CHARGE_T_PAYPAL 6

/* Invoice.InvType */
&GLOBAL-DEFINE INV_TYPE_NORMAL 1
&GLOBAL-DEFINE INV_TYPE_CASH 6
&GLOBAL-DEFINE INV_TYPE_RECEIPT 7
&GLOBAL-DEFINE INV_TYPE_TEST 99

/* Invoice.PrintState */
&GLOBAL-DEFINE INV_PRINTSTATE_PRINTHOUSE 1

/* Invoice delivery types */
&GLOBAL-DEFINE INV_DEL_TYPE_PAPER 1
&GLOBAL-DEFINE INV_DEL_TYPE_EMAIL 2
&GLOBAL-DEFINE INV_DEL_TYPE_FUSION_EMAIL 3
&GLOBAL-DEFINE INV_DEL_TYPE_SMS 4
&GLOBAL-DEFINE INV_DEL_TYPE_NO_DELIVERY 10
&GLOBAL-DEFINE INV_DEL_TYPE_EMAIL_PENDING 11
&GLOBAL-DEFINE INV_DEL_TYPE_NO_TRAFFIC 12
&GLOBAL-DEFINE INV_DEL_TYPE_FUSION_EMAIL_PENDING 13
&GLOBAL-DEFINE INV_DEL_TYPE_ESI 14  

/* call cases */
&GLOBAL-DEFINE ROAMING_CALLCASE "3,4,7,32,33,53,54"

/* SelfService API */
&GLOBAL-DEFINE DPL_REASON_MIYOIGO 0
&GLOBAL-DEFINE DPL_REASON_TRBLE_SHOOT 1
&GLOBAL-DEFINE DPL_REASON_AUTH_REQ 2
&GLOBAL-DEFINE DPL_REASON_INVOICE 3
&GLOBAL-DEFINE DPL_REASON_OTHER 4
&GLOBAL-DEFINE DPL_REASON_VISTA 5
&GLOBAL-DEFINE DPL_REASON_EAPI 6

/* Performance Indicator types */
&GLOBAL-DEFINE P_INDICATOR_TYPE_SATISFACTION_VALUE 1

/* Substerminal.TerminalType, OrderAccessory.TerminalType */
&GLOBAL-DEFINE TERMINAL_TYPE_PHONE 1
&GLOBAL-DEFINE TERMINAL_TYPE_LAPTOP 2
&GLOBAL-DEFINE TERMINAL_TYPE_ACCESSORY 3


/* Customer.InvoiceTargetRule */
&GLOBAL-DEFINE INVOICE_TARGET_RULE_UNDEFINED 0
&GLOBAL-DEFINE INVOICE_TARGET_RULE_DEFAULT_GROUP 1
&GLOBAL-DEFINE INVOICE_TARGET_RULE_SEPARATE_GROUP 2

/* Invoice target grouping states */
/* used on Web side, do not change descriptions */
&GLOBAL-DEFINE INVOICE_TARGET_ALL_GROUPED "all_grouped"
&GLOBAL-DEFINE INVOICE_TARGET_ALL_SPLIT "all_split"
&GLOBAL-DEFINE INVOICE_TARGET_CUSTOMIZED "customized"

/* OrderAccessory.IMEIStatus */
&GLOBAL-DEFINE IMEI_STATUS_UNKNOWN 0
&GLOBAL-DEFINE IMEI_STATUS_USED 1
&GLOBAL-DEFINE IMEI_STATUS_TO_BE_RELEASED 2
&GLOBAL-DEFINE IMEI_STATUS_RELEASED 3

/* CLIType.PayType */
&GLOBAL-DEFINE CLITYPE_PAYTYPE_NOT_DEFINED 0
&GLOBAL-DEFINE CLITYPE_PAYTYPE_POSTPAID 1
&GLOBAL-DEFINE CLITYPE_PAYTYPE_PREPAID 2
&GLOBAL-DEFINE CLITYPE_WEB_ACTIVE_STATUSES "1,2"
&GLOBAL-DEFINE CLITYPE_STC_ACTIVE_STATUSES "1,2"

&GLOBAL-DEFINE CLITYPE_WEBSTATUSCODE_ACTIVE 1

&GLOBAL-DEFINE CLITYPE_LINETYPE_NONMAIN 0
&GLOBAL-DEFINE CLITYPE_LINETYPE_MAIN 1
&GLOBAL-DEFINE CLITYPE_LINETYPE_ADDITIONAL 2
&GLOBAL-DEFINE CLITYPE_LINETYPE_EXTRA 3

&GLOBAL-DEFINE CLITYPE_TARIFFTYPE_MOBILEONLY 0
&GLOBAL-DEFINE CLITYPE_TARIFFTYPE_CONVERGENT 1
&GLOBAL-DEFINE CLITYPE_TARIFFTYPE_FIXEDONLY 2
&GLOBAL-DEFINE CLITYPE_TARIFFTYPE_FUSION 3

&GLOBAL-DEFINE YOIGO_CONVERGENT_BASE_BUNDLES_LIST "CONTDSL,CONTFH50,CONTFH300,CONTFH1000" 
/* MobSub.PayType */
&GLOBAL-DEFINE MOBSUB_PAYTYPE_POSTPAID False
&GLOBAL-DEFINE MOBSUB_PAYTYPE_PREPAID True

/* MobSub.CLIType */
&GLOBAL-DEFINE MOBSUB_CLITYPE_FUSION "CONTFF,CONTSF"

/* ActionLog.ActionStatus */
&GLOBAL-DEFINE ACTIONLOG_STATUS_ACTIVE 0
&GLOBAL-DEFINE ACTIONLOG_STATUS_ERROR 1
&GLOBAL-DEFINE ACTIONLOG_STATUS_SUCCESS 2
&GLOBAL-DEFINE ACTIONLOG_STATUS_LOGGED 3
&GLOBAL-DEFINE ACTIONLOG_STATUS_CANCELLED 5
&GLOBAL-DEFINE ACTIONLOG_STATUS_PROCESSING 6

/* MobSub.MSStatus */
&GLOBAL-DEFINE MSSTATUS_ACTIVE 4
&GLOBAL-DEFINE MSSTATUS_BARRED 8
&GLOBAL-DEFINE MSSTATUS_MOBILE_PROV_ONG 16
&GLOBAL-DEFINE MSSTATUS_MOBILE_NOT_ACTIVE 17


/* TMRule.TicketType */
&GLOBAL-DEFINE TICKET_TYPE_MOBILE 1
&GLOBAL-DEFINE TICKET_TYPE_FRAUD 2

/* periodical contract type */
&GLOBAL-DEFINE PERCONTRACT_RATING_PACKAGE "1,4,8"
&GLOBAL-DEFINE PERCONTRACT_DCCLI_DCTYPE "3,5,7"

/* ServiceLimit.InclUnit */
&GLOBAL-DEFINE INCLUNIT_MINUTE 1
&GLOBAL-DEFINE INCLUNIT_SECOND 2
&GLOBAL-DEFINE INCLUNIT_GIGABYTE 3
&GLOBAL-DEFINE INCLUNIT_MEGABYTE 4
&GLOBAL-DEFINE INCLUNIT_QUANTITY 5
&GLOBAL-DEFINE INCLUNIT_AMOUNT 6
&GLOBAL-DEFINE INCLUNIT_BDEST_QTY 7

&GLOBAL-DEFINE DIAL_TYPE_GENERAL 0
&GLOBAL-DEFINE DIAL_TYPE_FIXED_VOICE 1
&GLOBAL-DEFINE DIAL_TYPE_SMS 2
&GLOBAL-DEFINE DIAL_TYPE_VOICE 4
&GLOBAL-DEFINE DIAL_TYPE_GPRS 7
&GLOBAL-DEFINE DIAL_TYPE_FIXED_VOICE_BDEST 50

&GLOBAL-DEFINE PMDUB "PMDUB"
&GLOBAL-DEFINE HSPA_ROAM_EU "HSPA_ROAM_EU"
&GLOBAL-DEFINE BONO8_SMS_SENDER "22642"
&GLOBAL-DEFINE UPSELL_SMS_SENDER "622"
&GLOBAL-DEFINE BB_SMS_SENDER "622"
&GLOBAL-DEFINE STC_SMS_SENDER "622"
&GLOBAL-DEFINE FRAUD_BARR_CODES "Debt_Hotl,Debt_Restricted,Limits_Restricted,Risk_Restricted,ATC_Service_Suspension,Risk_Suspension,Debt_HOTLP,Debt_LP"
&GLOBAL-DEFINE DSS "DSS"
&GLOBAL-DEFINE DSS2 "DSS2"
&GLOBAL-DEFINE DSS4 "DSS4"
&GLOBAL-DEFINE DSS4SHAPERID "DSSINT"
&GLOBAL-DEFINE DSS_LOWSPEED_BDEST "GPRSDSS_B"
&GLOBAL-DEFINE DSS_BUNDLES "DSS,DSS2,DSS4"
&GLOBAL-DEFINE DSS2_LOWSPEED_BDEST "GPRSDSS2_B"
&GLOBAL-DEFINE TARJ_UPSELL "TARJ_UPSELL"
&GLOBAL-DEFINE BONO6DISCOUNTS "BONO6DISC,BONO6WEBDISC"

/* Additional lines */
&GLOBAL-DEFINE ADDLINE_CLITYPES "CONT10,CONT15,CONT25,CONT26,CONT33,CONT34"
&GLOBAL-DEFINE ADDLINE_DISCOUNTS "DISCCONT10H,DISCCONT15H,DISCCONT25H,DISCCONT26H,DISCCONT33H,DISCCONT34H"
&GLOBAL-DEFINE ADDLINE_DISCOUNTS_20 "DISCCONT10,DISCCONT15,DISCCONT25,DISCCONT26,DISCCONT33,DISCCONT34"
&GLOBAL-DEFINE ADDLINE_DISCOUNTS_HM "DISCCONT10HM,DISCCONT15HM,DISCCONT25HM,DISCCONT26HM,DISCCONT33HM,DISCCONT34HM" /* Additional Line with mobile only ALFMO-5 */

/* Value on Orders */
&GLOBAL-DEFINE ONGOING_ORDER_AVAIL "AVAILABLE"
&GLOBAL-DEFINE ONGOING_ORDER_ATLEAST_ONE "ATLEAST_ONE"
&GLOBAL-DEFINE ONGOING_ORDER_LIST "LIST"

/* CallAlarm.CrediType */
&GLOBAL-DEFINE SMSTYPE_STC 6 
&GLOBAL-DEFINE SMSTYPE_INFO 9 
&GLOBAL-DEFINE SMSTYPE_CONTRACT_ACTIVATION 10
&GLOBAL-DEFINE SMSTYPE_BARRING 24
&GLOBAL-DEFINE SMSTYPE_PREMIUM 26
&GLOBAL-DEFINE SMSTYPE_AFTER_SALES_ORDER 41 
&GLOBAL-DEFINE SMSTYPE_MNP_RETENTION 45
&GLOBAL-DEFINE SMSTYPE_MARKETING 46
&GLOBAL-DEFINE SMSTYPE_HPD 47
&GLOBAL-DEFINE SMSTYPE_MNP_CANCEL 48   /* MNP In cancel code handling */

&GLOBAL-DEFINE CA_DELISTAT_NEW 1
&GLOBAL-DEFINE CA_DELISTAT_SENT 3

/* orderdelivery.CourierId */
&GLOBAL-DEFINE COURIER_ID_DEXTRA 0
&GLOBAL-DEFINE COURIER_ID_CORREOS 1
&GLOBAL-DEFINE COURIER_ID_SEUR 2
&GLOBAL-DEFINE COURIER_ID_OTHER 3
&GLOBAL-DEFINE COURIER_ID_ASM 4

/* Prepaid service class id values */
&GLOBAL-DEFINE SC_TARJ5_NORMAL 6
&GLOBAL-DEFINE SC_TARJ5_NORMAL_BONO 86
&GLOBAL-DEFINE SC_TARJ5_PROMOTIONAL 106
&GLOBAL-DEFINE SC_TARJ5_PROMOTIONAL_BONO 186
&GLOBAL-DEFINE SC_TARJ7_OUTSIDE_DATABUNDLE 103
&GLOBAL-DEFINE SC_TARJ7_INSIDE_DATABUNDLE1 3
&GLOBAL-DEFINE SC_TARJ7_INSIDE_DATABUNDLE2 303
&GLOBAL-DEFINE SC_TARJ9_OUTSIDE_DATABUNDLE 109
&GLOBAL-DEFINE SC_TARJ9_INSIDE_DATABUNDLE1 9
&GLOBAL-DEFINE SC_TARJ9_INSIDE_DATABUNDLE2 309
&GLOBAL-DEFINE SC_TARJ10_OUTSIDE_DATABUNDLE 110
&GLOBAL-DEFINE SC_TARJ10_INSIDE_DATABUNDLE1 10
&GLOBAL-DEFINE SC_TARJ10_INSIDE_DATABUNDLE2 310
&GLOBAL-DEFINE SC_TARJ11_OUTSIDE_DATABUNDLE 111
&GLOBAL-DEFINE SC_TARJ11_INSIDE_DATABUNDLE1 11
&GLOBAL-DEFINE SC_TARJ11_INSIDE_DATABUNDLE2 311
&GLOBAL-DEFINE SC_TARJ12_OUTSIDE_DATABUNDLE 112
&GLOBAL-DEFINE SC_TARJ12_INSIDE_DATABUNDLE1 12
&GLOBAL-DEFINE SC_TARJ12_INSIDE_DATABUNDLE2 312

&GLOBAL-DEFINE SC_TARJ13_OUTSIDE_DATABUNDLE 120
&GLOBAL-DEFINE SC_TARJ13_INSIDE_DATABUNDLE1 20
&GLOBAL-DEFINE SC_TARJ13_INSIDE_DATABUNDLE2 320

&GLOBAL-DEFINE MSISDN_YOIGO_PREFIXES "622,633,7220,7221,7222,7223,7224,7225,72261,72262,72263,72264,72265,72266,72267,72268,72269,7227,7228"
&GLOBAL-DEFINE YOIGO_FREE_NUMBERS "622,622622622,622100100,622FF,633,633633633,633800800,633633556,556"

&GLOBAL-DEFINE MSISDN_MASMOVIL_PREFIXES "72260"
/* InvText.SendRule */
&GLOBAL-DEFINE SMS_SENDRULE_NOT_DEFINED ""
&GLOBAL-DEFINE SMS_SENDRULE_OFFICEH "R2"
&GLOBAL-DEFINE SMS_SENDRULE_OFFICEH_EXCEPT_LAST_DAY "R2+R3"
&GLOBAL-DEFINE SMS_SENDRULE_24H "R1"
&GLOBAL-DEFINE SMS_SENDRULE_24H_EXCEPT_LAST_DAY "R1+R3"
&GLOBAL-DEFINE SMS_SENDRULES "R1,R2,R1+R3,R2+R3"

/* counter.countertype */
&GLOBAL-DEFINE COUNTERTYPE_FAT_SMSBUNDLE 1
&GLOBAL-DEFINE COUNTERTYPE_FAY_SMS 2
&GLOBAL-DEFINE COUNTERTYPE_FAT_AMOUNT 3
&GLOBAL-DEFINE COUNTERTYPE_DISCOUNT_AMOUNT 4

/* Order.MultiSimType */
&GLOBAL-DEFINE MULTISIMTYPE_PRIMARY 1
&GLOBAL-DEFINE MULTISIMTYPE_SECONDARY 2
&GLOBAL-DEFINE MULTISIMTYPE_EXTRALINE 3

&GLOBAL-DEFINE EXTRALINEMATRIX "EXTRALINE"

&GLOBAL-DEFINE AZULMORADAEL "CONT28"
&GLOBAL-DEFINE INTERMINABLEEL "CONT29"

/* Extralines */
&GLOBAL-DEFINE ELTABLENAME "CLIType"
&GLOBAL-DEFINE ELKEYTYPE "ExtraLine"
&GLOBAL-DEFINE ELMANDATORY "MANDATORY"

/* RepText TextType */
&GLOBAL-DEFINE REPTEXT_DISCOUNTPLAN 31
&GLOBAL-DEFINE REPTEXT_SMS 32

/* FixedFee.FinancedResult */
&GLOBAL-DEFINE TF_STATUS_BANK "00"
&GLOBAL-DEFINE TF_STATUS_HOLD_SENDING "B00"
&GLOBAL-DEFINE TF_STATUS_WAITING_SENDING "B01"
&GLOBAL-DEFINE TF_STATUS_SENT_TO_BANK "B02"
&GLOBAL-DEFINE TF_STATUS_BANK_MANUAL "B99"

&GLOBAL-DEFINE TF_STATUS_YOIGO "Y00"
&GLOBAL-DEFINE TF_STATUS_YOIGO_ACC "Y01"
&GLOBAL-DEFINE TF_STATUS_YOIGO_SUB_TERMINATED "Y02"
&GLOBAL-DEFINE TF_STATUS_YOIGO_FF_TERMINATED "Y03"
&GLOBAL-DEFINE TF_STATUS_YOIGO_FF_CHANGED "Y04"
&GLOBAL-DEFINE TF_STATUS_YOIGO_ANALYZE_FAILED "Y05"
&GLOBAL-DEFINE TF_STATUS_YOIGO_NO_BANK_RESPONSE "Y06"
&GLOBAL-DEFINE TF_STATUS_YOIGO_REACTIVATION "Y07"
&GLOBAL-DEFINE TF_STATUS_YOIGO_INSTALLMENT_CHANGE "Y08"
&GLOBAL-DEFINE TF_STATUS_YOIGO_REVERT_RENEWAL "Y09"
&GLOBAL-DEFINE TF_STATUS_YOIGO_RENEWAL "Y10"
&GLOBAL-DEFINE TF_STATUS_YOIGO_STC "Y11"
&GLOBAL-DEFINE TF_STATUS_YOIGO_RVTERM_FEE_DELETION "Y12"
&GLOBAL-DEFINE TF_STATUS_YOIGO_LOGISTICS "Y13"
&GLOBAL-DEFINE TF_STATUS_YOIGO_OTHER "Y14"
&GLOBAL-DEFINE TF_STATUS_YOIGO_REACTIVATION_FBB "Y15"
&GLOBAL-DEFINE TF_STATUS_YOIGO_MANUAL "Y99"

&GLOBAL-DEFINE TF_STATUSES_MANUAL "B99,Y99"
&GLOBAL-DEFINE TF_STATUSES_BANK "00,B99"

&GLOBAL-DEFINE TF_CANCEL_ORDER_CANCEL "ORDER_CANCEL"
&GLOBAL-DEFINE TF_CANCEL_RENEWAL "RENEWAL"
&GLOBAL-DEFINE TF_CANCEL_INSTALLMENT "INSTALLMENT"
&GLOBAL-DEFINE TF_CANCEL_TERMINATION "TERMINATION"
&GLOBAL-DEFINE TF_CANCEL_STC "STC"
&GLOBAL-DEFINE TF_CANCEL_RENEWAL_CANCEL "RENEWAL_CANCEL"
&GLOBAL-DEFINE TF_CANCEL_MNP "MNP"
&GLOBAL-DEFINE TF_CANCEL_ACC "ACC"
&GLOBAL-DEFINE TF_CANCEL_OTHER "OTHER"

&GLOBAL-DEFINE TF_BANK_CODES "0049,0081,0225"
&GLOBAL-DEFINE TF_BANK_UNOE "0049"
&GLOBAL-DEFINE TF_BANK_SABADELL "0081"
&GLOBAL-DEFINE TF_BANK_BBVA "0182"
&GLOBAL-DEFINE TF_BANK_LACAXIA "2100"
&GLOBAL-DEFINE TF_BANK_CETELEM "0225"

&GLOBAL-DEFINE TF_BANK_RVTERM_BILLCODES "RVTERM1EF,RVTERMBSF,RVTERMBCF"
&GLOBAL-DEFINE TF_RVTERM_BILLCODES "RVTERM1EF,RVTERMBSF,RVTERMBCF,RVTERMF"
&GLOBAL-DEFINE TF_BANK_UNOE_PAYTERM_BILLCODES "PAYTERM1E,PAYTERMEND1E"
&GLOBAL-DEFINE TF_BANK_SABADELL_PAYTERM_BILLCODES "PAYTERMBS,PAYTERMENDBS"
&GLOBAL-DEFINE TF_BANK_CETELEM_PAYTERM_BILLCODES "PAYTERMBC,PAYTERMENDBC"
&GLOBAL-DEFINE TF_BANK_ALL_ACTIVE_PAYTERM_BILLCODES "PAYTERM1E,PAYTERMBS,PAYTERMBC"
&GLOBAL-DEFINE TF_BANK_ALL_CLOSED_PAYTERM_BILLCODES "PAYTERMEND1E,PAYTERMENDBS,PAYTERMENDBC"
&GLOBAL-DEFINE TF_BANK_COMMISSION_BILLCODES "PAYTERMCG1E,PAYTERMCGBS,PAYTERMCGBC"
&GLOBAL-DEFINE TF_BANK_UNOE_RVTERM_BILLCODES "RVTERM1E,RVTERMEND1E"
&GLOBAL-DEFINE TF_BANK_SABADELL_RVTERM_BILLCODES "RVTERMBS,RVTERMENDBS"
&GLOBAL-DEFINE TF_BANK_CETELEM_RVTERM_BILLCODES "RVTERMBC,RBTERMENDBC"
&GLOBAL-DEFINE TF_BANK_ALL_ACTIVE_RVTERM_BILLCODES "RVTERM1E,RVTERMBS,RVTERMBC"
&GLOBAL-DEFINE TF_BANK_ALL_CLOSED_RVTERM_BILLCODES "RVTERMEND1E,RVTERMENDBS,RBTERMENDBC"

&GLOBAL-DEFINE IFS_STATUS_WAITING_SENDING 1
&GLOBAL-DEFINE IFS_STATUS_SENT 2
&GLOBAL-DEFINE IFS_STATUS_SENDING_CANCELLED 3

&GLOBAL-DEFINE HOSTNAME_STAGING "angetenar,alpheratz,yanai"
&GLOBAL-DEFINE HOSTNAME_DEVEL "sadachbia,yoigodev"

&GLOBAL-DEFINE SERVCOM_TARGET_STATELESS 3

&GLOBAL-DEFINE DUMP_IFS_INVOICE "IFSInvoice"
&GLOBAL-DEFINE DUMP_INVOICE_PUPU "HPD_Invoice"
&GLOBAL-DEFINE DUMP_CLITYPE_TRACK "CLITYPEDUMP"

/*Active MQ*/
&GLOBAL-DEFINE AMQ_MSG_INIT_FAILED "AMQ init failed"
&GLOBAL-DEFINE AMQ_MSG_SENT "SENT"
&GLOBAL-DEFINE AMQ_RESENDER "AMQ_BATCH"

/*DMS */
&GLOBAL-DEFINE DMS_FILE_SEP "|"
&GLOBAL-DEFINE DMS_DOCLIST_SEP ";"
&GLOBAL-DEFINE DMS_REQ_FIELD_SEP "|"
&GLOBAL-DEFINE DMS_REQ_VAL_SEP CHR(255)

&GLOBAL-DEFINE DMS_CASE_TYPE_ID_ORDER_ACT "1"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_ORDER_RESTUDY "2"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_COMPANY "3"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_ORDER_VFR "4"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_DIRECT_CH "5"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_CANCEL "6"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_Q25_TERM_RETURN "9"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_Q25_STE "10"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_TELESALES "14"
&GLOBAL-DEFINE DMS_CASE_TYPE_ID_CATEGORY_CHG "15"

&GLOBAL-DEFINE DMS_HOST_TABLE_ORDER "Order"
&GLOBAL-DEFINE DMS_HOST_TABLE_MSREQ "MsRequest"
&GLOBAL-DEFINE DMS_VFR_REQUEST "retail_newton" 
&GLOBAL-DEFINE DMS_ON_OFF "DMSOnOff"
/*DMS processes. NAmes are used in avoiding parallel instasnces*/
&GLOBAL-DEFINE DMS_HIGH_FREQ_FILE_CREATOR "DMS_HIGH_FREQ"
&GLOBAL-DEFINE DMS_CASEFILE_READER "DMS_CF_READER"
&GLOBAL-DEFINE DMS_REMINDER_SENDER "DMS_REMINDER_SENDER"

&GLOBAL-DEFINE DMS_INITIAL_NOTIF_CASE "1"
&GLOBAL-DEFINE DMS_INIT_STATUS_SENT "SENT"
&GLOBAL-DEFINE DMS_INIT_STATUS_COMMENT "SENT"

&GLOBAL-DEFINE DMS_CASE_14_FILTER "fusion_telesales,fusion_cc,cc,telesales,renewal_telesales,cc_pro,telesales_pro,fusion_telesales_pro,renewal_ctc,fusion_cc_pro,emission_pro,retention,emission,retention_stc"

/*Barring */
/*Barring status*/
&GLOBAL-DEFINE BARR_STATUS_ACTIVE "ACTIVE"
&GLOBAL-DEFINE BARR_STATUS_INACTIVE "INACTIVE"
/*Barring Rule status values*/
&GLOBAL-DEFINE BARR_RULE_STATUS_ACTIVE "active"
&GLOBAL-DEFINE BARR_RULE_STATUS_INACTIVE "inactive"
&GLOBAL-DEFINE BARR_RULE_STATUS_RETIRED "retired"
/*Barring rule action values*/
&GLOBAL-DEFINE BARR_RULE_ACT_ON_HOLD "onhold"
&GLOBAL-DEFINE BARR_RULE_ACT_REMOVE_NO_RESTORE "removenorestore"
&GLOBAL-DEFINE BARR_RULE_ACT_REM_WITH_RESTORE "removewithrestore"
&GLOBAL-DEFINE BARR_RULE_ACT_NOT_ALLOWED "notallowed"

&GLOBAL-DEFINE MONTHS_ES "ENERO,FEBRERO,MARZO,ABRIL,MAYO,JUNIO,JULIO,AGOSTO,SEPTIEMBRE,OCTUBRE,NOVIEMBRE,DICIEMBRE"

/* Q25 SMS Sending related values */
&GLOBAL-DEFINE Q25_PASSPHRASE "YoigoQ25EncryptionPassword"
&GLOBAL-DEFINE ENCRYPTION_METHOD "AES_CFB_256"
&GLOBAL-DEFINE Q25_MONTH_24 0
&GLOBAL-DEFINE Q25_MONTH_23 1
&GLOBAL-DEFINE Q25_MONTH_22 2
&GLOBAL-DEFINE Q25_MONTH_24_FINAL_MSG 3

&GLOBAL-DEFINE Q25_LOGGING_SENT_MSGS 1
&GLOBAL-DEFINE Q25_LOGGING_COUNTERS  2
&GLOBAL-DEFINE Q25_LOGGING_DETAILED  3
&GLOBAL-DEFINE Q25_LOGGING_CUST_LOGS 99

&GLOBAL-DEFINE Q25_EXEC_TYPE_CUST_LOG_GENERATION 0
&GLOBAL-DEFINE Q25_EXEC_TYPE_CALCULATION 1
&GLOBAL-DEFINE Q25_EXEC_TYPE_SMS_SENDING 2
&GLOBAL-DEFINE Q25_EXEC_TYPE_HRLP_UNIV 3
&GLOBAL-DEFINE Q25_EXEC_TYPE_HRLP_ACT 4
&GLOBAL-DEFINE Q25_EXEC_TYPE_HRLP_REL 5
&GLOBAL-DEFINE Q25_EXEC_TYPE_PUSH_SENDING 6
&GLOBAL-DEFINE Q25_NOT_ACTION_PHASE 99
&GLOBAL-DEFINE Q25_RVTERM_RENEWAL_DISCOUNTS "RVTERMDTEQ25,RVTERMDTRW,RVTERMDTTD"

&GLOBAL-DEFINE Q25_HRLP_DELIM ";"

&GLOBAL-DEFINE Q25_HRLP_RESP_READER "HRLP_RESP_READER"
&GLOBAL-DEFINE Q25_HRLP_REMOVE_READER "Q25_HRLP_REMOVE_READER"

&GLOBAL-DEFINE Q25_HRLP_NO_TEST 0
&GLOBAL-DEFINE Q25_HRLP_ONLY_PROV_TEST 1
&GLOBAL-DEFINE Q25_HRLP_FULL_TEST 2

/*Google billing test level switches*/
&GLOBAL-DEFINE GB_NO_TEST 0
&GLOBAL-DEFINE GB_FULL_TEST 1

/*Actionlog constants, Parallel processing preventing*/
&GLOBAL-DEFINE GB_ACTION_GROUP_NAME "GOOGLE_BILLING"
&GLOBAL-DEFINE GB_REFUND_HANDLER "GB_REFUND_HANDLER"

/*return codes*/
&GLOBAL-DEFINE GB_RESP_OK "0"
&GLOBAL-DEFINE GB_RESP_SUBS_TERMINATED "1"
&GLOBAL-DEFINE GB_INCORRECT_PAYTYPE "2"
&GLOBAL-DEFINE GB_RESP_NO_SUBS "3"
&GLOBAL-DEFINE GB_RESP_REQUEST_ERROR "4"
&GLOBAL-DEFINE GB_POSTPAID_FAT_FAILURE "5"
&GLOBAL-DEFINE GB_POSTPAID_CN_FAILURE "6"

&GLOBAL-DEFINE GB_CCN 790
&GLOBAL-DEFINE GB_B_NBR "Google"

&GLOBAL-DEFINE FIXED_LINE_TYPE_ADSL 1
&GLOBAL-DEFINE FIXED_LINE_TYPE_FIBER 2

&GLOBAL-DEFINE LO_STATUS_ROUTER_PREFIX "9999"
&GLOBAL-DEFINE LO_STATUS_TV_STB_PREFIX "8888"
/**/
&GLOBAL-DEFINE ESI_PASSPHRASE "9_/udEsi993Pass10nHede1mz"


&GLOBAL-DEFINE TENANT_MASMOVIL "TMasMovil"
&GLOBAL-DEFINE TENANT_YOIGO "Default"
&GLOBAL-DEFINE TENANTS "Default,TMasMovil"
&GLOBAL-DEFINE MAX_TENANT_ID 99

&GLOBAL-DEFINE PL_LIMIT_SHAPED 20971520

/* Customer.NWProfile */
&GLOBAL-DEFINE CUSTOMER_NW_PROFILE_YG 1
&GLOBAL-DEFINE CUSTOMER_NW_PROFILE_YG_OR 2
&GLOBAL-DEFINE CUSTOMER_NW_PROFILE_YG_OR_TEL 3

&ENDIF

/* this below code is for integration purpose*/
/* TMS Global Constants */
/* Hint: Use ({&SOME_VARIABLE}) with messages so that compiler notices if constant does not exists  */
&IF "{&TMSCONST_I}" NE "YES"
&THEN
&GLOBAL-DEFINE TMSCONST_I YES

&GLOBAL-DEFINE EXTERNAL_CM "CIM"
&GLOBAL-DEFINE EXTERNAL_SALESCHAIN "OPERATOR"
&GLOBAL-DEFINE MAIL_HOST operator.com
&GLOBAL-DEFINE MAIL_SENDER "noreply@operator.com"
&GLOBAL-DEFINE DEFAULT_CURRENCY "EUR"

&GLOBAL-DEFINE COMPANY_IDTYPE "Y-TUNNUS"
&GLOBAL-DEFINE THING_IDTYPE "SN"

&GLOBAL-DEFINE REQ_INACTIVE_STATUSES "2,4,9,99"
&GLOBAL-DEFINE BOOLEAN_TRUE "true,yes,1"
&GLOBAL-DEFINE BOOLEAN_FALSE "false,no,0"
&GLOBAL-DEFINE WAIT_CONFIG_LOCK 20
&GLOBAL-DEFINE CONFIG_ID_MAX 32
/* TMRule */
&GLOBAL-DEFINE TMRULE_PERIOD_BALANCE 3

/* Common TMS CUI Messages */
&GLOBAL-DEFINE MSG_ONG_REQUEST "Function not allowed due to existing requests"
&GLOBAL-DEFINE MSG_NOT_ALLOWED "Function not allowed due to business rules"
&GLOBAL-DEFINE MSG_NOT_RIGHTS  "Function not allowed due to user rights"
&GLOBAL-DEFINE MSG_INCORRECT_VALUE "Incorrect or missing value"
&GLOBAL-DEFINE MSG_TRY_AGAIN "Try again"
&GLOBAL-DEFINE MSG_DATA_NOT_FOUND "Data not found"
&GLOBAL-DEFINE MSG_RECORD_DELETED "Record has been deleted by another user!"
&GLOBAL-DEFINE MSG_RECORD_CHANGED "Record has changed while you were working"
&GLOBAL-DEFINE MSG_RECORD_LOCKED "Record has been locked by another user"

&GLOBAL-DEFINE MSG_LAST_PAGE "YOU ARE ON THE LAST PAGE !"

/* Limit Types */
&GLOBAL-DEFINE LIMIT_TYPE_TMRLIMIT 1
&GLOBAL-DEFINE LIMIT_TYPE_SUBQTY 2
&GLOBAL-DEFINE LIMIT_TYPE_BILLPERM 3

&GLOBAL-DEFINE LIMIT_BILLPERM_PROHIBITED 2
&GLOBAL-DEFINE LIMIT_BILLPERM_ALLOWED 0
&GLOBAL-DEFINE LIMIT_BILLPERM_SUSPENDED 2


/* CustContactTypes */
&GLOBAL-DEFINE CUSTCONTACT_REPRESENTATIVE 1
&GLOBAL-DEFINE CUSTCONTACT_CONTACT 5

/* DayCampaign.DCType */
&GLOBAL-DEFINE DCTYPE_SERVICE_PACKAGE "1"
&GLOBAL-DEFINE DCTYPE_RATING_LIMIT "2"
&GLOBAL-DEFINE DCTYPE_DISCOUNT "3"
&GLOBAL-DEFINE DCTYPE_BUNDLE "4"
&GLOBAL-DEFINE DCTYPE_INSTALLMENT "5"
&GLOBAL-DEFINE DCTYPE_POOL_RATING "6"
&GLOBAL-DEFINE DCTYPE_RECURRING_CHARGE "7"
&GLOBAL-DEFINE DCTYPE_PART_PAYMENT "8"
&GLOBAL-DEFINE DCTYPE_FIXED_TERM "9"
&GLOBAL-DEFINE DCTYPE_ADDITIONAL_PACKAGE "10"
&GLOBAL-DEFINE DCTYPE_RECURRING_CALLITEM "11"
&GLOBAL-DEFINE DCTYPE_USAGE_LIMIT "12"

&GLOBAL-DEFINE DCTARGET_SUBSCRIPTION "1"
&GLOBAL-DEFINE DCTARGET_CUSTOMER "2"

&GLOBAL-DEFINE LANGUAGE_FIN 1
&GLOBAL-DEFINE LANGUAGE_SWE 2
&GLOBAL-DEFINE LANGUAGE_ENG 3

/* Request Types */
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_TYPE_CHANGE 0        
&GLOBAL-DEFINE REQTYPE_SERVICE_CHANGE 1
&GLOBAL-DEFINE REQTYPE_SALDO_PAYMENT 2
&GLOBAL-DEFINE REQTYPE_USER_CHANGE 3
&GLOBAL-DEFINE REQTYPE_INVOICE_CUSTOMER_CHANGE 4
&GLOBAL-DEFINE REQTYPE_USER_ACCOUNT_CHANGE 5
&GLOBAL-DEFINE REQTYPE_ADDRESS_CHANGE 6
&GLOBAL-DEFINE REQTYPE_INVOICE_UPDATE 7
&GLOBAL-DEFINE REQTYPE_PERIODICAL_CONTRACT_ACTIVATION 8
&GLOBAL-DEFINE REQTYPE_PERIODICAL_CONTRACT_TERMINATION 9
&GLOBAL-DEFINE REQTYPE_AGREEMENT_CUSTOMER_CHANGE 10
&GLOBAL-DEFINE REQTYPE_PAYMENT_PLAN 11
&GLOBAL-DEFINE REQTYPE_MARKETING_VALUES 12
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_CREATE 13
&GLOBAL-DEFINE REQTYPE_IMSI_CHANGE 15
&GLOBAL-DEFINE REQTYPE_CLAIM_RESTRAIN 17
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_TERMINATION 18
&GLOBAL-DEFINE REQTYPE_MSISDN_CHANGE 19
&GLOBAL-DEFINE REQTYPE_ONDEMAND_INVOICE 20
&GLOBAL-DEFINE REQTYPE_CREDIT_NOTE 22       
&GLOBAL-DEFINE REQTYPE_REFUND 23       
&GLOBAL-DEFINE REQTYPE_BANK_ACCOUNT_NUMBER_CHANGE 24       
&GLOBAL-DEFINE REQTYPE_ADD_DATA_LIMIT 25
&GLOBAL-DEFINE REQTYPE_FEEMODEL_PRICE_CHANGE 27 
&GLOBAL-DEFINE REQTYPE_PRICE_GUARANTEE  28
&GLOBAL-DEFINE REQTYPE_FATIME_CREATION 29
&GLOBAL-DEFINE REQTYPE_SMS_SEND 30
&GLOBAL-DEFINE REQTYPE_PAYMENT_CREATE 31
&GLOBAL-DEFINE REQTYPE_BANK_REF_UPDATE 32       
&GLOBAL-DEFINE REQTYPE_CREDIT_CHECK 33
&GLOBAL-DEFINE REQTYPE_MANUAL_PAYMENT 34       
&GLOBAL-DEFINE REQTYPE_BARRING_REQUEST 35
&GLOBAL-DEFINE REQTYPE_HEAT_BALANCE 37
&GLOBAL-DEFINE REQTYPE_BALANCE 42
&GLOBAL-DEFINE REQTYPE_RERATE 65
&GLOBAL-DEFINE REQTYPE_CUSTOMER_MAINTENANCE 70
&GLOBAL-DEFINE REQTYPE_WAIT_FOR_BATCH_SIM_SHIPPING_NOTIFICATION 71
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_UPDATE 72
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_ACTIVATE 73
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_DEACTIVATE 74
&GLOBAL-DEFINE REQTYPE_PREPAID_LIFECYCLE_TERMINATION 75
&GLOBAL-DEFINE REQTYPE_CHARGE_AND_COMPENSATION 76
&GLOBAL-DEFINE REQTYPE_FAT_HANDLER 77
&GLOBAL-DEFINE REQTYPE_BUNDLE_ACTIVATION_TERMINATION 78
&GLOBAL-DEFINE REQTYPE_SMS_INVOICE 79
&GLOBAL-DEFINE REQTYPE_IMEI_CHANGE 80
&GLOBAL-DEFINE REQTYPE_BUNDLE_CHANGE 81
&GLOBAL-DEFINE REQTYPE_BALANCE_REMINDER_SMS 82
&GLOBAL-DEFINE REQTYPE_CONTRACT_PENALTY_SMS 83 /*NOT USED ANYMORE*/
&GLOBAL-DEFINE REQTYPE_FEE_DEACTIVATION 84
&GLOBAL-DEFINE REQTYPE_CM_SUBSCRIPTION_TERMINATION 85
&GLOBAL-DEFINE REQTYPE_CM_CONTRACT_TERMINATION 86
&GLOBAL-DEFINE REQTYPE_DISCOUNT_HANDLER 87
&GLOBAL-DEFINE REQTYPE_TERM_EMAIL_TO_CM 88
&GLOBAL-DEFINE REQTYPE_ZIP_INVOICE_PDF 89
&GLOBAL-DEFINE REQTYPE_SUBSCRIPTION_MAINTENANCE 90
&GLOBAL-DEFINE REQTYPE_BILLING_AGREEMENT 91
&GLOBAL-DEFINE REQTYPE_DUMP_RUN 96

/* Request Statuses */
&GLOBAL-DEFINE REQUEST_STATUS_NEW 0
&GLOBAL-DEFINE REQUEST_STATUS_UNDER_WORK 1
&GLOBAL-DEFINE REQUEST_STATUS_DONE 2
&GLOBAL-DEFINE REQUEST_STATUS_REJECTED 3               /* Rejected (error occurred) */
&GLOBAL-DEFINE REQUEST_STATUS_CANCELLED 4              /* Cancelled */
&GLOBAL-DEFINE REQUEST_STATUS_HLR_PENDING 5            /* Pending HLR */
&GLOBAL-DEFINE REQUEST_STATUS_HLR_DONE 6               /* Pending - HLR DONE */
&GLOBAL-DEFINE REQUEST_STATUS_SUB_REQUEST_PENDING 7    /* Pending Sub-request */
&GLOBAL-DEFINE REQUEST_STATUS_SUB_REQUEST_DONE 8       /* Sub-request DONE */
&GLOBAL-DEFINE REQUEST_STATUS_HANDLED 9                /* Handled */
&GLOBAL-DEFINE REQUEST_STATUS_APPROVED 15           /* Approved for handling */
&GLOBAL-DEFINE REQUEST_STATUS_FILE_PENDING 16       /* Pending file creation */
&GLOBAL-DEFINE REQUEST_STATUS_FILE_DONE 17             /* File created */
&GLOBAL-DEFINE REQUEST_STATUS_CONFIRMATION_PENDING 19 
&GLOBAL-DEFINE REQUEST_STATUS_WAIT_FOR_REQUEST 20
&GLOBAL-DEFINE REQUEST_STATUS_WAIT_FOR_TARGET_CREATION 21
&GLOBAL-DEFINE REQUEST_STATUS_PENDING 91
                                               /* Waiting for confirmation */
&GLOBAL-DEFINE REQUEST_STATUS_PROCESS_ERROR 99    
                        /* Process error (Qvantel maintenance required) */

/* Request Sources */
&GLOBAL-DEFINE REQUEST_SOURCE_SUBSCRIPTION_CREATION "1"
&GLOBAL-DEFINE REQUEST_SOURCE_STC "2"
&GLOBAL-DEFINE REQUEST_SOURCE_MANUAL_TMS "4"
&GLOBAL-DEFINE REQUEST_SOURCE_SCRIPT "5"
&GLOBAL-DEFINE REQUEST_SOURCE_NEWTON "6"
&GLOBAL-DEFINE REQUEST_SOURCE_RENEWAL "7"
&GLOBAL-DEFINE REQUEST_SOURCE_ICC_UCC "9"
&GLOBAL-DEFINE REQUEST_SOURCE_EXTERNAL_API "15"
&GLOBAL-DEFINE REQUEST_SOURCE_CUSTOMER_CREATION "16"

/* Request response statuses */
&GLOBAL-DEFINE REQUEST_RESPONSE_NOTREADY 0
&GLOBAL-DEFINE REQUEST_RESPONSE_CANSEND 1
&GLOBAL-DEFINE REQUEST_RESPONSE_SENT 2
&GLOBAL-DEFINE REQUEST_RESPONSE_NOTSENT 3
&GLOBAL-DEFINE REQUEST_RESPONSE_NEEDNOTSEND 4

/* user limits for charges and compensations types */
&GLOBAL-DEFINE POST_CHARGE_LIMIT_TYPE 1
&GLOBAL-DEFINE POST_COMP_LIMIT_TYPE 2
&GLOBAL-DEFINE PREP_CHARGE_LIMIT_TYPE 3
&GLOBAL-DEFINE PREP_COMP_LIMIT_TYPE 4
&GLOBAL-DEFINE POST_CHARGE_MONTHLY_LIMIT_TYPE 5
&GLOBAL-DEFINE POST_COMP_MONTHLY_LIMIT_TYPE 6
&GLOBAL-DEFINE PREP_CHARGE_MONTHLY_LIMIT_TYPE 7
&GLOBAL-DEFINE PREP_COMP_MONTHLY_LIMIT_TYPE 8
&GLOBAL-DEFINE CREDIT_NOTE_LIMIT_TYPE 9

/* BillItem Definitions */
&GLOBAL-DEFINE BITEM_GRP_COMPENSATION ""

/* Invoice.InvType */
&GLOBAL-DEFINE INV_TYPE_NORMAL 1
&GLOBAL-DEFINE INV_TYPE_CREDIT 5
&GLOBAL-DEFINE INV_TYPE_CASH 6
&GLOBAL-DEFINE INV_TYPE_RECEIPT 7
&GLOBAL-DEFINE INV_TYPE_PARTPAYM 15
&GLOBAL-DEFINE INV_TYPE_PARTPAYM_CREDIT 16
&GLOBAL-DEFINE INV_TYPE_COMMISSION 20
&GLOBAL-DEFINE INV_TYPE_SUBVENTION 21
&GLOBAL-DEFINE INV_TYPE_COMMISSION_CREDIT 22
&GLOBAL-DEFINE INV_TYPE_SUBVENTION_CREDIT 23
&GLOBAL-DEFINE INV_TYPE_TEST 99

/* Invoice.PrintState */
&GLOBAL-DEFINE INV_PRINTSTATE_PRINTHOUSE 1

/* Invoice delivery types */
&GLOBAL-DEFINE INV_DELTYPE_PAPER_INVOICE 1
&GLOBAL-DEFINE INV_DELTYPE_EMAIL 2
&GLOBAL-DEFINE INV_DELTYPE_EINVOICE 4
&GLOBAL-DEFINE INV_DELTYPE_SELFSERVICE 5
&GLOBAL-DEFINE INV_DELTYPE_FINVOICE 6
&GLOBAL-DEFINE INV_DELTYPE_DIRECT_PAYMENT 7
&GLOBAL-DEFINE INV_DELTYPE_ARCHIVE 8
&GLOBAL-DEFINE INV_DELTYPE_NO_PAPER_INVOICE 10

/* Invoice charge types */
&GLOBAL-DEFINE INV_CHARGETYPE_BANK_TRANSFER 5

/* call cases */
&GLOBAL-DEFINE ROAMING_CALLCASE "3,4,7,32,33,53,54"

/* Substerminal.TerminalType, OrderAccessory.TerminalType */
&GLOBAL-DEFINE TERMINAL_TYPE_PHONE 1
&GLOBAL-DEFINE TERMINAL_TYPE_LAPTOP 2

/* Customer.InvoiceTargetRule */
&GLOBAL-DEFINE INVOICE_TARGET_RULE_UNDEFINED 0
&GLOBAL-DEFINE INVOICE_TARGET_RULE_DEFAULT_GROUP 1
&GLOBAL-DEFINE INVOICE_TARGET_RULE_SEPARATE_GROUP 2

/* CLIType.PayType */
&GLOBAL-DEFINE CLITYPE_PAYTYPE_NOT_DEFINED 0
&GLOBAL-DEFINE CLITYPE_PAYTYPE_POSTPAID 1
&GLOBAL-DEFINE CLITYPE_PAYTYPE_PREPAID 2

/* ActionLog.ActionStatus */
&GLOBAL-DEFINE ACTIONLOG_STATUS_ACTIVE 0
&GLOBAL-DEFINE ACTIONLOG_STATUS_ERROR 1
&GLOBAL-DEFINE ACTIONLOG_STATUS_SUCCESS 2
&GLOBAL-DEFINE ACTIONLOG_STATUS_LOGGED 3
&GLOBAL-DEFINE ACTIONLOG_STATUS_CANCELLED 5

/* MobSub.MSStatus */
&GLOBAL-DEFINE MSSTATUS_ACTIVE 4
&GLOBAL-DEFINE MSSTATUS_BARRED 8

/* TMRule.TicketType */
&GLOBAL-DEFINE TICKET_TYPE_MOBILE 1
&GLOBAL-DEFINE TICKET_TYPE_FRAUD 2

/* periodical contract type */
&GLOBAL-DEFINE PERCONTRACT_RATING_PACKAGE "1,4"

/* ServiceLimit.InclUnit */
&GLOBAL-DEFINE INCLUNIT_MINUTE 1
&GLOBAL-DEFINE INCLUNIT_SECOND 2
&GLOBAL-DEFINE INCLUNIT_GIGABYTE 3
&GLOBAL-DEFINE INCLUNIT_MEGABYTE 4
&GLOBAL-DEFINE INCLUNIT_QUANTITY 5
&GLOBAL-DEFINE INCLUNIT_AMOUNT 6

&GLOBAL-DEFINE UPSELL_BILLCODES ""
&GLOBAL-DEFINE UPSELL_BASE_CONTRACTS ""
&GLOBAL-DEFINE BONO_CONTRACTS ""
/* these should not be used anymore, so instead of making a cparam and 
   fetching that for each request just list these here */
&GLOBAL-DEFINE ADDSERVFEEMODEL "TERMPAY24DHBL,TERMPAYM24DHS"

/* CallAlarm.CrediType */
&GLOBAL-DEFINE SMSTYPE_INFO 9
&GLOBAL-DEFINE SMSTYPE_SALDO_REMINDER 2 

/* payment type */
&GLOBAL-DEFINE PAYMTYPE_CREDITNOTE 5
&GLOBAL-DEFINE PAYMTYPE_TEST 14
&GLOBAL-DEFINE PAYMTYPE_UPFRONT 20
&GLOBAL-DEFINE PAYMTYPE_CREDITING 50
&GLOBAL-DEFINE PAYMTYPE_TO_OTHER_BANK 51
&GLOBAL-DEFINE PAYMTYPE_TO_OPERATOR_BANK 54
&GLOBAL-DEFINE PAYMTYPE_AR_DEBT_COLLECTION 60 
&GLOBAL-DEFINE PAYMTYPE_CREDIT_LOSS 63
&GLOBAL-DEFINE PAYMTYPE_OVERPAYMENT 99
&GLOBAL-DEFINE PAYMTYPE_EXT_DEBT_COLLECTION 77 
/* ---- exception payment types ---- */
&GLOBAL-DEFINE PAYMTYPE_OPERATOR_CL_REQUEST "MPOPR"
&GLOBAL-DEFINE PAYMTYPE_OPERATOR_OP_REQUEST "OPINV"
&GLOBAL-DEFINE PAYMTYPE_OPERATOR_OP_RETURN_REQUEST "OPRF"
&GLOBAL-DEFINE PAYMTYPE_OP_RETURN_TO_AR "OPAR"
&GLOBAL-DEFINE PAYMTYPE_RBS_FIXING "MPFIX"
&GLOBAL-DEFINE PAYMTYPE_MANUAL_OP_FIXING "OPFIX"

/* account type */
&GLOBAL-DEFINE ACCOUNT_TYPE_RECEIVABLES 1
&GLOBAL-DEFINE ACCOUNT_TYPE_REVENUE 12

/* service fee */
&GLOBAL-DEFINE SERVFEE_EVENTTYPE_ACTIVATION 1
&GLOBAL-DEFINE SERVFEE_EVENTTYPE_MODIFICATION 2
&GLOBAL-DEFINE SERVFEE_EVENTTYPE_DEACTIVATION 3

&GLOBAL-DEFINE CLAIMSTATE_NOT_CLAIMED 0.0
&GLOBAL-DEFINE CLAIMSTATE_SMSREMINDER_I 1.0
&GLOBAL-DEFINE CLAIMSTATE_SMSREMINDER_II 2.0
&GLOBAL-DEFINE CLAIMSTATE_SMSREMINDER_III 3.0
&GLOBAL-DEFINE CLAIMSTATE_BARRED 11.0
&GLOBAL-DEFINE CLAIMSTATE_SUSPENDED 12.0
&GLOBAL-DEFINE CLAIMSTATE_PROMISE_TO_PAY 12.1
&GLOBAL-DEFINE CLAIMSTATE_TERM_MAIL_SENT 13.0
&GLOBAL-DEFINE CLAIMSTATE_TERMINATED 14.0

&GLOBAL-DEFINE ITSENDLOG_INVOICE 3
&GLOBAL-DEFINE ITSENDLOG_CALLITEMIZATION 5
&GLOBAL-DEFINE ITSENDLOG_METHOD_MAIL 1
&GLOBAL-DEFINE ITSENDLOG_METHOD_PRINTHOUSE 3
&GLOBAL-DEFINE ITSENDLOG_METHOD_XML 5
&GLOBAL-DEFINE ITSENDLOG_METHOD_SMS 6
&GLOBAL-DEFINE ITSENDLOG_METHOD_MQ 7 

&GLOBAL-DEFINE INVOICE_SMS_NOTSENT 0
&GLOBAL-DEFINE INVOICE_SMS_DELIVERY 1
&GLOBAL-DEFINE INVOICE_SMS_DUEDATE 2

/* Interface constants */ 
&GLOBAL-DEFINE EXTERNAL_SUBSCRIPTION_ID "ExtIDSubscription"
&GLOBAL-DEFINE EXTERNAL_CONTRACT_ID "ExtContractID"
&GLOBAL-DEFINE EXTERNAL_DEVICE_ID "ExtDeviceId"
&GLOBAL-DEFINE CUSTOMER_PER_CONTRACT_ID "CustPerContract"
&GLOBAL-DEFINE EXTERNAL_USER_ID "ExtIDUserCust"
&GLOBAL-DEFINE EXTERNAL_PAYER_ID "ExtIDInvCust"
&GLOBAL-DEFINE EXTERNAL_OWNER_ID "ExtIDAgrCust"
&GLOBAL-DEFINE PERIODICAL_CONTRACT_ID "ExtIDPerContract"
&GLOBAL-DEFINE EXTERNAL_CUSTOMER_ID "ExtIDCustomer" 
&GLOBAL-DEFINE FSECURE_ID "FSecureID"
&GLOBAL-DEFINE MATKATV_ID "MatkaTvId"
&GLOBAL-DEFINE DEEZER1_ID "Deezer1Id"
&GLOBAL-DEFINE INSURANCE_ID "InsuranceId"
&GLOBAL-DEFINE EXTERNAL_USER_CUST "ExtIDNewUserCust"
&GLOBAL-DEFINE EXTERNAL_INV_CUST "ExtIDNewInvCust"
&GLOBAL-DEFINE EXTERNAL_OWNER_CUST "ExtIDNewOwnerCust"
&GLOBAL-DEFINE CM_CREATION_TIME "CMCreationTime"
&GLOBAL-DEFINE EXPERIMENT_EXPIRY "ExperimentDate"
&GLOBAL-DEFINE EXT_MAIN_CONTRACT_ID "ExtMainContractId"
&GLOBAL-DEFINE MAIN_CONTRACT_END "MainContractEndDate"
&GLOBAL-DEFINE EXT_SERV_PROD_ID "ExtServProdId"
&GLOBAL-DEFINE OLD_EXT_CUST_ID "OldExtCustId"
&GLOBAL-DEFINE EXTERNAL_BILLING_AGREEMENT_ID "ExtIDBillingAgreement" 
&GLOBAL-DEFINE EXTERNAL_COMPENSATION_ID "ExtIDCompensation"

/* CustSaldo SaldoType */
&GLOBAL-DEFINE SALDOTYPE_SL_END 1

/*FAT Targets*/
&GLOBAL-DEFINE FAT_SUBSCRIPTION "0"
&GLOBAL-DEFINE FAT_AGRCUSTOMER "1"
&GLOBAL-DEFINE FAT_INVCUSTOMER "2"
&GLOBAL-DEFINE FAT_USERCUSTOMER "3"
&GLOBAL-DEFINE FAT_CUSTPERCONTRACT "4"
&GLOBAL-DEFINE FAT_MAINCONTRACT "5"
&GLOBAL-DEFINE FAT_BILLING_AGREEMENT "6"

/*PaymenPlan statuses*/
&GLOBAL-DEFINE PPLAN_STATUS_CREATED 1
&GLOBAL-DEFINE PPLAN_STATUS_SENT_TO_CUSTOMER 2
&GLOBAL-DEFINE PPLAN_STATUS_ACCEPTED 3
&GLOBAL-DEFINE PPLAN_STATUS_CANCELLED 4
&GLOBAL-DEFINE PPLAN_STATUS_PAID 5
&GLOBAL-DEFINE PPLAN-STATUS_FAILED 6

/*Invoice paymentstates*/

&GLOBAL-DEFINE INVOICE_PAYMENTSTATUS_UNPAID 0
&GLOBAL-DEFINE INVOICE_PAYMENTSTATUS_PARTLY_PAID 1
&GLOBAL-DEFINE INVOICE_PAYMENTSTATUS_PAID 2
&GLOBAL-DEFINE INVOICE_PAYMENTSTATUS_CREDITLOSS 3
&GLOBAL-DEFINE INVOICE_PAYMENTSTATUS_PAYMENT_PLAN 4
&GLOBAL-DEFINE INVOICE_PAYMENTSTATUS_EPAYMENT 5

/*Einvoice deltype change failure(s)*/
&GLOBAL-DEFINE EINVOICE_DELTYPE_UPDATE_FAILED 9
/* crediting reason */
&GLOBAL-DEFINE CREDIT_REASON_OWNUSAGE "5011"

/* Subscription termination reasons */
&GLOBAL-DEFINE SUBSTERM_CUSTOMER "1"  /* Customer */
&GLOBAL-DEFINE SUBSTERM_MNP_OUTPORTING "2"  /* MNP outporting */
&GLOBAL-DEFINE SUBSTERM_DEBT_COLLECTION "4"  /* Debt collection */
&GLOBAL-DEFINE SUBSTERM_OTHER "5"  /* Other reason (ADMIN users) */
&GLOBAL-DEFINE SUBSTERM_RISK_COLLECTION "9"  /* Risk Collection */
&GLOBAL-DEFINE SUBSTERM_RISK_FRAUD "10" /* Risk Fraud */
&GLOBAL-DEFINE SUBSTERM_ORDER_CANC_ALL "13" /* Order cancellation, deactivate all */
&GLOBAL-DEFINE SUBSTERM_ORDER_CANC_EXC_DEV "14" /* Order cancellation, deactivate all except device */
&GLOBAL-DEFINE SUBSTERM_ORDER_TERM_ALL_BY_CSP "15" /* Order terminated by CSP, deactivate all */
&GLOBAL-DEFINE SUBSTERM_ORDER_TERM_EXC_DEV_BY_CSP "16" /* Order terminated by CSP, deact. all except device */
&GLOBAL-DEFINE SUBSTERM_DEV_ORDER_CANC "17" /* Device order cancellation */
&GLOBAL-DEFINE SUBSTERM_ACCEL_DEV_TERM "18" /* Accelerated termination of device contract */
&GLOBAL-DEFINE SUBSTERM_DEV_RETURN_AGR_CSP "19" /* Device return based on agreement with CSP */
&GLOBAL-DEFINE SUBSTERM_DEV_TERM_BY_CSP "20" /* Device contract termination by CSP, deact. charges */
&GLOBAL-DEFINE SUBSTERM_DEV_TERM_BY_CSP_CHARGE "21" /* Device contract termination by CSP with charges */
&GLOBAL-DEFINE SUBSTERM_ORDER_TERM_BY_CSP "22" /* Order terminated by CSP */
&GLOBAL-DEFINE SUBSTERM_CONTRACT_PAID "23" /* Contract fully paid */
&GLOBAL-DEFINE SUBSTERM_EXPERIMENTAL_PERIOD_CANC_ALL "24" /* Experiment period cancel, deact. all */
&GLOBAL-DEFINE SUBSTERM_EXPERIMENTAL_PERIOD_CANC_DEV "25" /* Experiment period cancel, deact. device */
&GLOBAL-DEFINE SUBSTERM_SALESPERS_CANC_DEV "26" /* SalesPerson cancelled device contract, deact. device */
&GLOBAL-DEFINE SUBSTERM_DEACT_DEV "27" /* Deactivate device, not subscription */
&GLOBAL-DEFINE SUBSTERM_DEV_CONTRACT_TERM_BY_OM "28" /* Device contract termination by OM */

&GLOBAL-DEFINE PAYMPLAN_STATUS_CREATED 1
&GLOBAL-DEFINE PAYMPLAN_STATUS_SENT 2
&GLOBAL-DEFINE PAYMPLAN_STATUS_ACCEPTED 3
&GLOBAL-DEFINE PAYMPLAN_STATUS_CANCELLED 4
&GLOBAL-DEFINE PAYMPLAN_STATUS_PAID 5
&GLOBAL-DEFINE PAYMPLAN_STATUS_FAILED 6

&GLOBAL-DEFINE RATING_DATA_KILO 1000
&GLOBAL-DEFINE RATING_DATA_MEGA 1000000
&GLOBAL-DEFINE RATING_DATA_GIGA 1000000000

&ENDIF



