...
form /* seek  CodeValue */
    ...
    WITH row 4 col 2 TITLE COLOR VALUE(Syst.Var:ctc) " FIND CODE VALUE "
    COLOR VALUE(Syst.Var:cfc) NO-LABELS OVERLAY FRAME f1.

form /* seek CodeName */
    "Code Name:" lcCodeName
    HELP "Enter code name"
    WITH row 4 col 2 TITLE COLOR VALUE(Syst.Var:ctc) " FIND CODE NAME "
...
ASSIGN ttMenu.MenuText:LABEL IN FRAME sel = "Type  Description" +
                                            FILL(" ",30) + 
...
FOR EACH RequestType NO-LOCK WHERE
   ...
   lcRowText = STRING(RequestType.ReqType) + lcSpace + 
               RequestType.ReqName.

   lcStatuses = "".
   FOR EACH MsReqStatFunc NO-LOCK WHERE
            MsReqStatFunc.ReqType = RequestType.ReqType AND
            MsReqStatFunc.FuncGroup > "":
      lcStatuses = lcStatuses + STRING(MsReqStatFunc.ReqStatus) + ",".
   END.
   lcStatuses = SUBSTRING(lcStatuses,1,LENGTH(lcStatuses) - 1).
   liFill = 44 - LENGTH(lcRowText).


   CREATE ttMenu.

   ASSIGN ttMenu.MenuText   = "   " + lcRowText + FILL(" ",liFill) + lcStatuses
          ttMenu.RType      = RequestType.ReqType
          FrmDown           = FrmDown + 1.
