...
WITH width 80 OVERLAY scroll 1 15 DOWN ROW 1
    ...
    title color value(Syst.Var:ctc) " " + Syst.Var:ynimi + " INVOICE File "
...
WITH
    OVERLAY ROW 3 centered COLOR value(Syst.Var:cfc) TITLE COLOR value(Syst.Var:ctc)
...
form /* Invoicen asnolla hakua varten */
    ...
    with row 4 col 2 title color value(Syst.Var:ctc) " FIND CUST # "
    COLOR value(Syst.Var:cfc) NO-LABELS OVERLAY FRAME hakie.
...
repeat WITH FRAME sel:
   ...
   repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF ufkey THEN DO:
         ...
         Syst.Var:ufk[1]= 94  Syst.Var:ufk[2] = 1492 Syst.Var:ufk[3] = 927 Syst.Var:ufk[4] = 1883
         Syst.Var:ufk[5]= 790 Syst.Var:ufk[6] = 829 Syst.Var:ufk[7] = 1796 Syst.Var:ufk[8]= 8 Syst.Var:ufk[9]= 1
      ...
      ELSE IF order = 4 THEN DO:
         ...
         COLOR DISPLAY value(Syst.Var:ccc) Invoice.InvAmt WITH FRAME sel.
      END.
     ...
     /* search functions */
     if lookup(Syst.Var:nap,"1,f1") > 0 THEN 
     etsi:    
     repeat WITH FRAME sel:

        ASSIGN Syst.Var:ufk = 0 Syst.Var:ehto = 0  Syst.Var:ufk[8] = 8
               Syst.Var:ufk[1]= 133 Syst.Var:ufk[2]= 28  Syst.Var:ufk[3]= 702 Syst.Var:ufk[4]= 789.

        RUN Syst/ufkey.p.
        ufkey = TRUE.

        IF Syst.Var:toimi = 8 THEN LEAVE etsi.
        ...
        /* Haku sarakk. 2 */
        IF Syst.Var:toimi = 2 THEN DO:  /* haku sar. 2 */
           Syst.Var:cfc = "puyr". RUN Syst/ufcolor.p.
           InvDate = ?.
           Syst.Var:ehto = 9. RUN Syst/ufkey.p. ufkey = TRUE.
        ...
        /* Haku 3 */
        ELSE IF Syst.Var:toimi = 3 THEN DO:  /* haku sarakk. 3 */
           Syst.Var:cfc = "puyr". RUN Syst/ufcolor.p.
           hakuCustNum = 0.
           Syst.Var:ehto = 9. RUN Syst/ufkey.p. ufkey = TRUE.
        ...
        /* Haku sarakk. 4 */
        IF Syst.Var:toimi = 4 THEN DO:  /* haku sar. 4 */
           InvDate = ?.
           Syst.Var:cfc = "puyr". RUN Syst/ufcolor.p.
           hakunetto = 0.
           Syst.Var:ehto = 9. RUN Syst/ufkey.p. ufkey = TRUE.
     ...
     ELSE IF LOOKUP(Syst.Var:nap,"3,F3") > 0 THEN DO TRANS: /* memo */
     ...
     else if lookup(Syst.Var:nap,"2,f2") > 0 THEN DO : /* claiming history */
     ...
     else if lookup(Syst.Var:nap,"6,f6") > 0 THEN DO : /* payments */
        FIND Invoice where recid(Invoice) = rtab[FRAME-LINE] no-lock no-error.
     ...
     else if lookup(Syst.Var:nap,"T") > 0 THEN DO : /* time stamps */
        FIND Invoice where recid(Invoice) = rtab[FRAME-LINE] no-lock no-error.
        MESSAGE 
     ...
     else if lookup(Syst.Var:nap,"5,f5") > 0 THEN DO :  /* laskulinet */
