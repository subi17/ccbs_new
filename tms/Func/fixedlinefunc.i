...
FUNCTION fIsConvergentAddLineOK RETURNS LOGICAL
   ...
   IF CAN-FIND(FIRST bCLIType NO-LOCK WHERE
      ...
      IF fMatrixAnalyse(Syst.Parameters:Syst.CUICommon:gcBrand,
                        "ADDLINE",
...
FUNCTION fCheckConvergentSTCCompability RETURNS LOGICAL
   ...
   FIND FIRST bOldClitype WHERE
              bOldClitype.brand EQ Syst.Parameters:Syst.CUICommon:gcBrand AND
...
FUNCTION fCheckOngoingConvergentOrder RETURNS LOGICAL
   ...
   FOR EACH bOrderCustomer NO-LOCK WHERE   
       ...
       EACH bOrder NO-LOCK WHERE
            bOrder.Brand      EQ Syst.Parameters:Syst.CUICommon:gcBrand AND
            bOrder.orderid    EQ bOrderCustomer.Orderid  AND
            bOrder.OrderType  NE {&ORDER_TYPE_RENEWAL},
      FIRST bOrderFusion NO-LOCK WHERE
            bOrderFusion.Brand   = Syst.Parameters:Syst.CUICommon:gcBrand AND
            bOrderFusion.OrderID = bOrder.OrderID,
      FIRST bCliType WHERE bCliType.Brand = Syst.Parameters:Syst.CUICommon:gcBrand AND bCliType.CliType = bOrder.CliType NO-LOCK:
...
FUNCTION fCheckOngoingProMigration RETURNS LOGICAL
   ...
   FOR FIRST Customer WHERE Customer.CustNum = iiCustNum NO-LOCK,
       EACH CustCat WHERE CustCat.Brand = Syst.Parameters:Syst.CUICommon:gcBrand AND CustCat.Category = Customer.Category AND CustCat.Pro = False NO-LOCK, 
       ...
       EACH bOrder WHERE bOrder.Brand EQ Syst.Parameters:Syst.CUICommon:gcBrand AND bOrder.orderid EQ bOrderCustomer.Orderid AND bOrder.OrderType NE {&ORDER_TYPE_RENEWAL} NO-LOCK,
       FIRST bOrderFusion WHERE bOrderFusion.Brand = Syst.Parameters:Syst.CUICommon:gcBrand AND bOrderFusion.OrderID = bOrder.OrderID NO-LOCK,
       FIRST bCliType WHERE bCliType.Brand = Syst.Parameters:Syst.CUICommon:gcBrand AND bCliType.CliType = bOrder.CliType NO-LOCK:
...
FUNCTION fIsProSubscription RETURNS LOGICAL
   ...
   DO:
       ...
       DO:
           FIND FIRST bCustCat WHERE bCustCat.Brand = Syst.Parameters:Syst.CUICommon:gcBrand AND bCustCat.Category = bCustomer.Category AND bCustCat.Pro = TRUE NO-LOCK NO-ERROR.
...
FUNCTION fCheckExistingConvergentAvailForExtraLine RETURNS LOGICAL
   ...
   FOR FIRST Customer WHERE
             Customer.Brand      = Syst.Parameters:Syst.CUICommon:gcBrand AND
       ...
       EACH  MobSub NO-LOCK WHERE
             MobSub.Brand    = Syst.Parameters:Syst.CUICommon:gcBrand AND
...
FUNCTION fCheckFixedLineStatusForMainLine RETURNS LOGICAL
   ...
   FOR EACH bOrderCustomer NO-LOCK WHERE
       ...
       EACH bOrder NO-LOCK WHERE
            ...
            bOrder.OrderType  NE {&ORDER_TYPE_RENEWAL},
      FIRST bOrderFusion NO-LOCK WHERE
            bOrderFusion.Brand        EQ Syst.Parameters:Syst.CUICommon:gcBrand AND
            bOrderFusion.OrderID      EQ bOrder.OrderID          AND 
            bOrderFusion.FusionStatus EQ {&FUSION_ORDER_STATUS_FINALIZED}, 
      FIRST bCLIType WHERE bCLIType.Brand   EQ Syst.Parameters:Syst.CUICommon:gcBrand AND 
