...
form /*  search WITH FIELD  */
    ...
    with row 4 col 2 title color value(Syst.Var:ctc) " FIND CLOSING Date "
...
form /* UPDATE whitelist FileName */
    ...
    title color value(Syst.Var:cfc) " ADD OPENED NUMBERS TO A File "
...
BROWSE:
   repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF ufkey THEN DO:
        ASSIGN
        Syst.Var:ufk[1]= 35  Syst.Var:ufk[2]= 28 Syst.Var:ufk[3]= 911 Syst.Var:ufk[4]= 912
        Syst.Var:ufk[5]= 903 Syst.Var:ufk[6]= 4  Syst.Var:ufk[7]= 0   Syst.Var:ufk[8]= 8   Syst.Var:ufk[9]= 1
      ...
      IF order = 1 THEN DO:
        ...
        COLOR DISPLAY value(Syst.Var:ccc) ClosedCust.CustNum WITH FRAME sel.
      END.
      ELSE IF order = 2 THEN DO:
        CHOOSE ROW ClosedCust.Date {Syst/uchoose.i} no-error WITH FRAME sel.
        COLOR DISPLAY value(Syst.Var:ccc) ClosedCust.Date WITH FRAME sel.
      END.
/*    IF order = 3 THEN DO:
        CHOOSE ROW ClosedCust.?? {Syst/uchoose.i} no-error WITH FRAME sel.
        COLOR DISPLAY value(Syst.Var:ccc) ClosedCust.?? WITH FRAME sel.
      END.
...
*/
   ...
   /*   ELSE IF order = 3 THEN FIND prev ClosedCust USE-INDEX index-3
     ...
     if lookup(Syst.Var:nap,"5,f5") > 0 THEN DO ON ENDKEY UNDO, NEXT LOOP:
        Syst.Var:ehto = 9. RUN Syst/ufkey.p. ufkey = TRUE.
        whitelist = whitelist + 
