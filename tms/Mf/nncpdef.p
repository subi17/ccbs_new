...
BROWSE:
   repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF order = 1 THEN DO:
        ...
        COLOR DISPLAY value(Syst.Var:ccc) MedDefTrunk.Ident WITH FRAME sel.
      END.
      ELSE IF order = 2 THEN DO:
        CHOOSE ROW MedDefTrunk.DefName {Syst/uchoose.i} no-error WITH FRAME sel.
        COLOR DISPLAY value(Syst.Var:ccc) MedDefTrunk.DefName WITH FRAME sel.
      END.
/*    IF order = 3 THEN DO:
        CHOOSE ROW MedDefTrunk.?? {Syst/uchoose.i} no-error WITH FRAME sel.
        COLOR DISPLAY value(Syst.Var:ccc) MedDefTrunk.?? WITH FRAME sel.
      END.
