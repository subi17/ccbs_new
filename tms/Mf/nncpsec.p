...
BROWSE:
   repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF order = 1 THEN DO:
        ...
        COLOR DISPLAY value(Syst.Var:ccc) MedSect.Type WITH FRAME sel.
      END.
      ELSE IF order = 2 THEN DO:
        CHOOSE ROW MedSect.Name {Syst/uchoose.i} no-error WITH FRAME sel.
        COLOR DISPLAY value(Syst.Var:ccc) MedSect.Name WITH FRAME sel.
      END.
  /*  IF order = 3 THEN DO:
        CHOOSE ROW MedSect.?? {Syst/uchoose.i} no-error WITH FRAME sel.
        COLOR DISPLAY value(Syst.Var:ccc) MedSect.?? WITH FRAME sel.
      END.
