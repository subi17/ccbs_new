...
WITH centered OVERLAY scroll 1 10 DOWN ROW 3 COLOR value(Syst.Var:cfc)
    title color value(Syst.Var:ctc) " Neighbour areas for " + AreaCode + " "
    + this-rnom.AreaName + " " FRAME sel.

form
    NeigArea label "Areanr"  AreaName NO-LABEL SKIP
WITH  OVERLAY ROW 4 centered
    COLOR value(Syst.Var:cfc) TITLE COLOR value(Syst.Var:ctc) fr-header WITH side-labels
    FRAME lis.

form /* angrAnsande riktnr. search WITH FIELD NeigArea */
    NeigArea
    help "Enter Areanumber"
    with row 4 col 2 title color value(Syst.Var:ctc) " FIND AREA NUMBER "
    COLOR value(Syst.Var:cfc) side-labels OVERLAY FRAME f1.
...
BROWSE:
   repeat WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF ufkey THEN DO:
    ASSIGN
    Syst.Var:ufk[1]= 36  Syst.Var:ufk[2]= 0 Syst.Var:ufk[3]= 0 Syst.Var:ufk[4]= 0
    ...
    COLOR DISPLAY value(Syst.Var:ccc) AreaPair.NeigArea WITH FRAME sel.
      END.
...
/*    IF order = 3 THEN DO:
    ...
    COLOR DISPLAY value(Syst.Var:ccc) AreaPair.?? WITH FRAME sel.
      END.
...
*/
  ...
  /*    ELSE IF order = 3 THEN FIND NEXT AreaPair USE-INDEX index-3
   ...
   COLOR DISPLAY value(Syst.Var:ccc) AreaPair.NeigArea AreaName.
   IF ok THEN DO:
