/* ----------------------------------------------------------------------
  MODULE .......: unbilled_subsqty_ui.p
  TASK .........: Print a report of unbilled subscriptions
  APPLICATION ..: tms
  AUTHOR .......: aam
  CREATED ......: 25.03.09
  Version ......: yoigo
---------------------------------------------------------------------- */
&GLOBAL-DEFINE TraceLog NO

{Syst/commali.i}
{Func/cparam2.i}

&IF "{&TraceLog}" = "YES" 
&THEN
{Func/log.i}
fSetLogFileName("/tmp/unbilled_subs.log").
fSetLogEntryTypes("4GLTrace:4").
fClearLog().
&ENDIF

{Mc/lib/tokenlib.i}
{Mc/lib/tokenchk.i 'MobSub'}

DEF VAR ufkey         AS LOG  NO-UNDO.
DEF VAR liCount       AS INT  NO-UNDO. 
DEF VAR lcFile        AS CHAR NO-UNDO.
DEF VAR liPeriod      AS INT  NO-UNDO.
DEF VAR lcCode        AS CHAR NO-UNDO. 
DEF VAR lcTransDir    AS CHAR NO-UNDO. 
DEF VAR liExtent      AS INT  NO-UNDO.
DEF VAR llDetails     AS LOG  NO-UNDO.
DEF VAR ldaInvDate    AS DATE NO-UNDO EXTENT 2.

FORM 
   SKIP(2)
   "Print a report of unbilled subscriptions." AT 10 
   SKIP(2)
                      
   liPeriod COLON 20 
      LABEL "Billing Period" 
      FORMAT "999999"
      HELP "Billing period YYYYMM"
      SKIP

   ldaInvDate[1] COLON 20
      LABEL "Invoice Dates"
      FORMAT "99-99-99"
      HELP "Invoice should be found on these dates"
      "-"
   ldaInvDate[2] 
      NO-LABEL 
      FORMAT "99-99-99"
      HELP "Invoice should be found on these dates"
      VALIDATE(INPUT ldaInvDate[2] >= INPUT ldaInvDate[1],
               "Upper limit cannot be earlier than lower limit")
      SKIP
      
   llDetails COLON 20
      LABEL "Details"
      HELP "Print details of subscriptions"
      FORMAT "Yes/No"
      SKIP(1)

   lcFile COLON 20
      LABEL "File Name"
      HELP "Name of the output file"
      FORMAT "X(55)"
      SKIP
   lcTransDir COLON 20
      LABEL "Transfer Directory"
      FORMAT "X(55)" 
   SKIP(6)

WITH ROW 1 SIDE-LABELS WIDTH 80
     TITLE " " + Syst.Var:ynimi + " UNBILLED SUBSCRIPTIONS " + 
           STRING(TODAY,"99-99-99") + " "
     FRAME fCrit.


ASSIGN 
   ufkey         = FALSE
   liPeriod      = IF MONTH(TODAY) = 1
                   THEN (YEAR(TODAY) - 1) * 100 + 12
                   ELSE YEAR(TODAY) * 100 + (MONTH(TODAY) - 1)
   llDetails     = FALSE
   ldaInvDate[1] = DATE(MONTH(TODAY),1,YEAR(TODAY))
   ldaInvDate[2] = IF MONTH(TODAY) = 12
                   THEN DATE(12,31,YEAR(TODAY))
                   ELSE DATE(MONTH(TODAY) + 1,1,YEAR(TODAY)) - 1
   lcFile        = fCParamC("UnbilledSubsFileName")
   lcTransDir    = fCParamC("UnbilledSubsTransDir").

CritLoop:
REPEAT WITH FRAME fCrit ON ENDKEY UNDO CritLoop, NEXT CritLoop:

   PAUSE 0.
   DISPLAY liPeriod
           ldaInvDate
           llDetails
           lcFile
           lcTransDir
   WITH FRAME fCrit.

   IF ufkey THEN DO:
      ASSIGN
         Syst.Var:ufk    = 0
         Syst.Var:ufk[1] = 132 
         Syst.Var:ufk[3] = 1069
         Syst.Var:ufk[5] = 795
         Syst.Var:ufk[8] = 8 
         Syst.Var:ehto   = 0.
      RUN Syst/ufkey.p.
   END.
   
   ELSE ASSIGN 
      Syst.Var:toimi = 1
      ufkey = TRUE.

   IF Syst.Var:toimi = 1 THEN DO:

      Syst.Var:ehto = 9. 
      RUN Syst/ufkey.p.
      
      REPEAT WITH FRAME fCrit ON ENDKEY UNDO, LEAVE:

         UPDATE liPeriod
                ldaInvDate
                llDetails
                lcFile
         WITH FRAME fCrit EDITING:

            READKEY.
            Syst.Var:nap = KEYLABEL(LASTKEY).

            IF LOOKUP(Syst.Var:nap,Syst.Var:poisnap) > 0 THEN DO:

            END.

            APPLY LASTKEY.
         END. 

         LEAVE. 
      END.

   END.

   ELSE IF Syst.Var:toimi = 3 THEN 
      RUN Syst/report_config.p ("UnbilledSubsQty").
 
   ELSE IF Syst.Var:toimi = 5 THEN DO:
      
      IF lcFile = "" OR lcFile = ? THEN DO:
         MESSAGE "File name has not been given."
         VIEW-AS ALERT-BOX ERROR.
         NEXT.
      END.

      IF liPeriod < 200101 THEN DO:
         MESSAGE "Invalid billing period"
         VIEW-AS ALERT-BOX ERROR.
         NEXT.
      END.
      
      IF ldaInvDate[1] = ? OR ldaInvDate[2] = ? THEN DO:
         MESSAGE "Invoice dates have not been defined"
         VIEW-AS ALERT-BOX ERROR.
         NEXT.
      END.
           
      RUN Inv/unbilled_subsqty.p (liPeriod,
                              ldaInvDate[1],
                              ldaInvDate[2],
                              lcFile,
                              llDetails,
                              0,
                              0,
                              "",
                              OUTPUT liCount).
                                                   
      MESSAGE liCount "subscriptions were picked to the report" SKIP
              RETURN-VALUE
      VIEW-AS ALERT-BOX TITLE " DONE ".
      
      LEAVE CritLoop.
   END.

   ELSE IF Syst.Var:toimi = 8 THEN DO:
      LEAVE CritLoop.
   END.

END. /* CritLoop */

HIDE MESSAGE NO-PAUSE.
HIDE FRAME fCrit NO-PAUSE.    

&IF "{&TraceLog}" = "YES" 
&THEN
fCloseLog().
&ENDIF

