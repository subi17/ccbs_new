...
WITH
   ...
   " " + Syst.CUICommon:ynimi + " DEPOSIT INVOICING " + string(pvm,"99-99-99") + " "
...
IF iiInvType = 6 OR iiInvType = 7 THEN 
   RUN pCashInvoice in pHandle(iiInvType,
                               iiOrderID, 
                               icMSSeq,
                               invDte,
                               atpvm1,
...
ELSE 
   RUN pDepositInvoice in pHandle(iiInvType,
                                  iiOrderID, 
                                  icMSSeq,
                                  invDte,
                                  atpvm1,
...
RUN pGetAmt in pHandle (OUTPUT olQty,
                        OUTPUT lAmt,
                        OUTPUT lVatAmt).
...
ELSE IF olQty NE 0 THEN RUN pCancel in pHandle.
/* no NEW invoices */
...
HIDE MESSAGE no-pause.
IF NOT ilSilent THEN put screen row 22 col 60 "               ".
HIDE FRAME rajat no-pause.
