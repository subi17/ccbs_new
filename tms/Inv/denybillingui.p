/* ----------------------------------------------------------------------
  MODULE .......: denybillingui.p
  TASK .........: Read billing denials from files (ui)
  APPLICATION ..: tms
  AUTHOR .......: aam
  CREATED ......: 15.02.08
  CHANGED ......: 
...
FORM 
   SKIP(2)
   "Read billing denials from a file." AT 5 
   SKIP(3)
...
WITH ROW 1 SIDE-LABELS WIDTH 80
     TITLE " " + Syst.Var:ynimi + "  DENY BILLING  " + 
           STRING(TODAY,"99-99-99") + " "
     FRAME fCrit.


ASSIGN ufkey      = FALSE
       lcFile     = fCParamC("DenyBillFiles")
       lcLogFile  = fCParamC("DenyBillLog").

lcFile = lcFile + "*" .
lcLogFile = lcLogFile + "bill_deny_#DATE.log". 
...
REPEAT WITH FRAME fCrit ON ENDKEY UNDO CritLoop, NEXT CritLoop:
   ...
   ELSE IF Syst.Var:toimi = 5 THEN DO:
      ...
      MESSAGE liRead "rows were read." SKIP
              liRead - liErrors
                 "subscriptions were updated succesfully," SKIP
