...
DEF VAR ldtFusionDueDate AS DATE NO-UNDO.
DEF VAR ldtIberPayDueDate AS DATE NO-UNDO.
DEF VAR ldtEBADueDate AS DATE NO-UNDO.
...
form
   ...
   invDte  label " Invoice Date ..........."
           help "Date of invoicing for all invoices"
                                                SKIP
  ldtIberPayDueDate LABEL " IberPay Due Date ......." FORMAT "99-99-99"
           help "IberPay Due date, overrides normal logic"
                                                SKIP
  ldtEBADueDate LABEL " EBA Due Date ..........." FORMAT "99-99-99"
           help "EBA Due date, overrides normal logic"
                                                SKIP
  ldtFusionDueDate 
             LABEL " Fusion Due Date........." FORMAT "99-99-99"
           help "Fusion due date"
                                                SKIP
...
toimi:
   repeat WITH FRAME valinta ON ENDKEY UNDO toimi, RETURN:
      IF kysy_rajat THEN DO:
         ...
         invDte ldtIberPayDueDate ldtEBADueDate ldtFusionDueDate
         ciperiod
         llRerate llDouble 
         mark
         lowvalue
      ...
      IF toimi = 5 THEN DO:
         ...
         IF ldtIberPayDueDate NE ? THEN DO:
            IF ldtIberPayDueDate < invDte THEN DO:
               MESSAGE "IberPay Due date cannot be earlier than invoice date"
               VIEW-AS ALERT-BOX ERROR.
               NEXT toimi.
            END.
            
            IF ldtIberPayDueDate > invDte + 60 THEN DO:
         ...
         IF ldtEBADueDate NE ? THEN DO:
            IF ldtEBADueDate < invDte THEN DO:
               MESSAGE "EBA Due date cannot be earlier than invoice date"
               VIEW-AS ALERT-BOX ERROR.
               NEXT toimi.
            END.
            
            IF ldtEBADueDate > invDte + 60 THEN DO:
               MESSAGE "Check the due date"
               VIEW-AS ALERT-BOX INFORMATION.
               NEXT toimi.
            END.
         ...
         IF ldtFusionDueDate NE ? THEN DO:
            IF ldtFusionDueDate < invDte THEN DO:
               MESSAGE "Fusion due date cannot be earlier than invoice date"
               VIEW-AS ALERT-BOX ERROR.
               NEXT toimi.
            END.
            
            IF ldtFusionDueDate > invDte + 60 THEN DO:
               MESSAGE "Check the fusion due date"
               VIEW-AS ALERT-BOX INFORMATION.
               NEXT toimi.
            END.
         END.
...
RUN pCreateInv in pHandle(invDte,
                          ...
                          atpvm2,
                          ciperiod,
                          ldtIberPayDueDate, /* IberPay Due Date */
                          ldtEBADueDate, /* EBA Due Date */
                          ldtFusionDueDate, /* Fusion Due Date */
                          llRerate,
                          llDouble,
                          liCustQty,
                          1,
