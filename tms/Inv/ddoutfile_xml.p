...
DEF VAR lcListFile    AS CHAR NO-UNDO. 
...
DEF VAR liXMLError AS INT NO-UNDO. 
...
PROCEDURE pInitialize:
   ...
   IF NOT AVAIL BankAccount THEN
      RETURN SUBST("ERROR:Invalid bank code: &1", lcBankID).
   ...
   lcListFile = fCParamC("StcContRepFolder") + "/" + lcPlainFile + ".log".
...
PROCEDURE pPrintInvoices:
   ...
   FOR EACH ttInvoice WHERE
      ...
      IF Invoice.MandateID = "" THEN DO:
         fErrLine("Blank Mandate ID value").
         NEXT PrintMainLoop.
      END.
   ...
   PUT STREAM sList UNFORMATTED
       "Total number of invoices:" oiInvCount SKIP
       "Total amount of invoices:" ldInvTot SKIP.
   OUTPUT STREAM sList CLOSE.
   ...
   IF ilCSBValidate AND
      ...
      ASSIGN
         ...
         lcCommand = lcSchema + lcXsdFile + " " + icFile + " > " +
            lcSchemaValLog + " 2>&1".
      UNIX SILENT VALUE(lcCommand).
      FILE-INFO:FILE-NAME = lcSchemaValLog.
      IF FILE-INFO:FILE-TYPE EQ ? OR
         NOT FILE-INFO:FILE-TYPE BEGINS "F" THEN
            ASSIGN
               lcTransDir = ""
               liXMLError = 2.
      ELSE DO:
         INPUT STREAM sin FROM VALUE(lcSchemaValLog).
         IMPORT STREAM sin UNFORMATTED lcline.

         IF INDEX(lcline,"validates") = 0 THEN
            ASSIGN
               lcTransDir = ""
               liXMLError = 1.
         ELSE
   ...
   IF CAN-FIND(FIRST ttError) OR 
      ...
      IF xMailAddr > "" THEN DO:
         ...
         PUT STREAM slog UNFORMATTED
            ...
            lcNewLine + lcNewLine + "  ".
         IF liXMLError = 1 THEN
            PUT STREAM slog UNFORMATTED
            "XML Schema validation failed, see results: " + lcNewLine +
            lcSchemaValLog + lcNewLine + lcNewLine + " ".
         IF liXMLError = 2 THEN
            PUT STREAM slog UNFORMATTED
            "XML Schema validation process was crashed, no log file was" +
            " created and no file transfer was made." + lcNewLine +
            "Bank file can be found from spool directory" + 
            lcNewLine + lcNewLine + "  ".
