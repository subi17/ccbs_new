...
form
    ...
    MobCDR.CLI                                   FORMAT "x(11)"
    MobCDR.GsmBnr                                FORMAT "x(8)"
...
IF AVAILABLE MobCDR THEN ASSIGN
   Memory       = recid(MobCDR)
   must-print   = TRUE
   must-add     = FALSE.
ELSE DO:
...
PrintPage:
   DO :
      IF must-print THEN DO:
        UP FRAME-LINE - 1.
        FIND MobCDR WHERE recid(MobCDR) = Memory NO-LOCK NO-ERROR.
        ...
        REPEAT WITH FRAME sel:
           IF AVAILABLE MobCDR THEN DO:
              RUN local-disp-row.
              rtab[FRAME-LINE] = recid(MobCDR).
              RUN local-find-NEXT.
           END.
           ELSE DO:
              CLEAR NO-PAUSE.
...
BROWSE:
   REPEAT WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF ufkey THEN DO:
        ASSIGN                               
        Syst.Var:ufk[1]= 35  Syst.Var:ufk[2]= 0   Syst.Var:ufk[3]= 1102 Syst.Var:ufk[4]= 0
      ...
      IF order = 1 THEN DO:
        ...
        COLOR DISPLAY VALUE(Syst.Var:ccc) MobCDR.CustNum WITH FRAME sel.
      END.
      ELSE IF order = 2 THEN DO:
        CHOOSE ROW MobCDR.CLI {Syst/uchoose.i} NO-ERROR WITH FRAME sel.
        COLOR DISPLAY VALUE(Syst.Var:ccc) MobCDR.CLI WITH FRAME sel.
      END.
      ...
      IF order <> pr-order AND MaxOrder > 1 THEN DO:
        ...
        FIND MobCDR WHERE recid(MobCDR) = Memory NO-LOCK.
        DO i = 1 TO FRAME-LINE - 1:
           RUN local-find-PREV.
           IF AVAILABLE MobCDR THEN
              ASSIGN FIRSTrow = i Memory = recid(MobCDR).
           ELSE LEAVE.
        END.
        must-print = TRUE.
        NEXT LOOP.
      ...
      IF LOOKUP(Syst.Var:nap,"cursor-up") > 0 THEN DO WITH FRAME sel:
        IF FRAME-LINE = 1 THEN DO:
           ...
           RUN local-find-PREV.
           IF NOT AVAILABLE MobCDR THEN DO:
      ...
      ELSE IF LOOKUP(Syst.Var:nap,"PREV-page,page-up,-") > 0 THEN DO:
        ...
        IF AVAILABLE MobCDR THEN DO:
           ...
           DO RowNo = 1 TO (FRAME-DOWN - 1):
              RUN local-find-PREV.
              IF AVAILABLE MobCDR THEN Memory = recid(MobCDR).
     ...
     ELSE IF LOOKUP(Syst.Var:nap,"1,f1") > 0 THEN DO ON ENDKEY UNDO, NEXT LOOP:
       ...
       IF NOT AVAILABLE MobCDR THEN DO:
          ...
          NEXT BROWSE.
       END.
       /* some MobCDR/DateSt was found */
       ASSIGN order = 1 Memory = recid(MobCDR) must-print = TRUE.
       NEXT LOOP.
     END. /* Search-1 */
     ...
     ELSE IF LOOKUP(Syst.Var:nap,"3,F3") > 0 THEN DO:
        ...
        CLI = SL_Prefix + MobCDR.CLI.
        find first mobsub where
                   mobsub.cli = MobCDR.CLI
        no-lock no-error.
     ...
     ELSE IF LOOKUP(Syst.Var:nap,"home,H") > 0 THEN DO:
        ...
        ASSIGN Memory = recid(MobCDR) must-print = TRUE.
       NEXT LOOP.
     END.
...
PROCEDURE local-find-this:
    ...
    IF exlock THEN
      FIND MobCDR WHERE recid(MobCDR) = rtab[frame-line(sel)] 
      EXCLUSIVE-LOCK.
    ELSE
       FIND MobCDR WHERE recid(MobCDR) = rtab[frame-line(sel)] 
       NO-LOCK.
...
PROCEDURE local-find-FIRST:
   ...
   IF order = 1 THEN FIND FIRST MobCDR WHERE
                                MobCDR.ErrorCode = 0
                     NO-LOCK NO-ERROR.
   ELSE IF order = 2 THEN FIND FIRST MobCDR WHERE
                                     MobCDR.ErrorCode = 0
                          USE-INDEX CustNum NO-LOCK NO-ERROR.
   ELSE IF order = 3 THEN FIND FIRST MobCDR WHERE
                                     MobCDR.ErrorCode = 0
...
PROCEDURE local-find-LAST:
   ...
   IF order = 1 THEN FIND LAST MobCDR WHERE
                               MobCDR.ErrorCode = 0
                     NO-LOCK NO-ERROR.
   ELSE IF order = 2 THEN FIND LAST MobCDR WHERE
                                    MobCDR.ErrorCode = 0
                          USE-INDEX CustNum NO-LOCK NO-ERROR.
   ELSE IF order = 3 THEN FIND LAST MobCDR WHERE
                                     MobCDR.ErrorCode = 0
                          USE-INDEX CLI NO-LOCK NO-ERROR.
...
PROCEDURE local-find-NEXT:
   ...
   IF order = 1 THEN FIND NEXT MobCDR WHERE
                               MobCDR.ErrorCode = 0
                     NO-LOCK NO-ERROR.
   ELSE IF order = 2 THEN FIND NEXT MobCDR WHERE
                                    MobCDR.ErrorCode = 0
                          USE-INDEX CustNum NO-LOCK NO-ERROR.
   ELSE IF order = 3 THEN FIND NEXT MobCDR WHERE
                                    MobCDR.ErrorCode = 0
                          USE-INDEX CLI NO-LOCK NO-ERROR.
...
PROCEDURE local-find-PREV:
   ...
   IF order = 1 THEN FIND PREV MobCDR WHERE
                               MobCDR.ErrorCode = 0
                     NO-LOCK NO-ERROR.
   ELSE IF order = 2 THEN FIND PREV MobCDR WHERE
                                    MobCDR.ErrorCode = 0
                          USE-INDEX CustNum NO-LOCK NO-ERROR.
   ELSE IF order = 3 THEN FIND PREV MobCDR WHERE
                                    MobCDR.ErrorCode = 0
                          USE-INDEX CLI NO-LOCK NO-ERROR.
...
PROCEDURE local-disp-row:
       ...
       DISPLAY 
       MobCDR.DateSt
       STRING(MobCDR.TimeSt,"HH:MM") @ lcTime
       MobCDR.CustNum
...
PROCEDURE local-find-others.
       FIND MobSub WHERE MobSub.MsSeq = MobCDR.MsSeq NO-LOCK NO-ERROR.
       ...
       FIND MobError WHERE 
            MobError.MobError = MobCDR.ErrorCode NO-LOCK NO-ERROR.   
END PROCEDURE.
