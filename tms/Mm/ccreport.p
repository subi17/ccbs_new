...
IF icRunMode = "test" THEN lcPenInvGrain = lcTrans.
ELSE lcPenInvGrain = fCParamC("InvGrainRepo").
...
UNIX SILENT VALUE("cp " + icFileName + " " + lcPenSpool).
lcPenFileName = fGetFileName(icFileName).
IF lcPenBITotals > "" THEN 
   fMove2TransDirOverwrite(lcPenSpool + "/" + lcPenFileName,
                           lcPenBITotals).
/* move InvGrain */
IF lcPenInvGrain > "" THEN 
   fMove2TransDirOverwrite(lcInvGrainFile,
                           lcPenInvGrain).

lcFinalFile = "".
IF lcTrans > "" THEN DO:
   lcFinalFile = fMove2TransDir(icFileName,
                                "",
                                lcTrans).
END.
IF lcFinalFile = "" THEN lcFinalFile = icFileName.
