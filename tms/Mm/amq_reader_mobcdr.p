/* ----------------------------------------------------------------------
  MODULE .......: Mm/amq_reader_mobcdr.p
  TASK .........: ActiveMQ MobCDR reader for high performance data
  APPLICATION ..: tms
  AUTHOR .......: Ivailo
  CREATED ......: 20.02.15
  Version ......: yoigo
...
RUN pInitialize(INPUT "amq_mcdr").
...
DO WHILE TRUE :
   ...
   "F8 TO QUIT, OTHER KEYS START MobCDR READER IMMEDIATELLY".
...
PROCEDURE pStartReader:
   ...
   DO WHILE idaReadDate <= TODAY:
      ...
      FOR EACH MobCDR NO-LOCK USE-INDEX ReadDate WHERE
               ...
               MobCDR.ReadTime > iiReadTime
               BY ReadTime:

         IF MobCDR.RerateTS  <> 0 OR
            MobCDR.ErrorCode <> 0 THEN NEXT.

         IF MobCDR.ReadInTS > ldeBufferTS THEN NEXT.
         ...
         /* Treshold value */
         IF oiCount >= 20000 THEN LEAVE Reader.
