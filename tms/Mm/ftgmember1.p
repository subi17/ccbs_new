...
WITH ROW FrmRow width 80 OVERLAY FrmDown  DOWN
    ...
    TITLE COLOR VALUE(Syst.Var:ctc) "Members Of " + FTGrp
    FRAME sel.
...
ADD-ROW:
      ...
      FIND FIRST FATGMember WHERE
                 FATGMember.FTGrp = FTGrp AND 
                 FAtgmember.Brand = Syst.Var:gcBrand NO-LOCK NO-ERROR.
...
BROWSE:
   REPEAT WITH FRAME sel ON ENDKEY UNDO, RETURN:
      ...
      IF order = 1 THEN DO:
        ...
        COLOR DISPLAY VALUE(Syst.Var:ccc) FATGMember.FTGMember WITH FRAME sel.
      END.
     ...
     ELSE IF LOOKUP(Syst.Var:nap,"6,f6") > 0 THEN DO TRANSACTION:  /* DELETE */
       ...
       IF ok THEN DO:
           ...
           WHERE Fatgmember.Brand = Syst.Var:gcBrand) THEN DO:
...
PROCEDURE local-find-FIRST:
       IF order = 1 THEN FIND FIRST FATGMember WHERE
           FATGMember.FTGrp = FTGrp
        AND FATgmember.Brand = Syst.Var:gcBrand NO-LOCK NO-ERROR.
       ELSE IF order = 2 THEN FIND FIRST FATGMember USE-INDEX FTGrp
       WHERE FATgmember.Brand = Syst.Var:gcBrand NO-LOCK NO-ERROR.
END PROCEDURE.
