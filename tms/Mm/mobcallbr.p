/* ----------------------------------------------------------------------
  ...
  AUTHOR .......: pt
  CREATED ......: 29-09-99
  CHANGED ......: 11-01-00  jpo)  F6 Call total
                  21.03.03  jp  xbsub
                  25.03.02  jp  xbsub -> func.i
                  20.05.03  tk  mobsub.mpref in hidebsub
                  26.05.03  jp  find b-number
                  08.07.03  jp  cli format x11
                  30.04.06  jp  temp-table for mobcdr 
                  20.06.07  jp  Prepaid handling
                  27.06.07  vk  Changed so, that this module works also 
                                with prepaid calls
...
form
    ...
    ttCall.BillCode     COLUMN-LABEL "Pr"          FORMAT "x(4)"
...
IF AVAIL ttCall AND 
         ttCall.ppFlag =  1 THEN llPrePaid = TRUE.
ELSE                             llPrePaid = FALSE.
...
/*          CLI = SUBSTR(sl_prefix,LENGTH(def-ccode) + 1) + CLI. */
     ...
     ELSE IF LOOKUP(nap,"4,f4") > 0 THEN DO ON ENDKEY UNDO, NEXT LOOP:
       ...
       IF gsmbnr NE "" THEN DO:
          ...
          END.
          /* some ttCall/CLI was found */
          ASSIGN order = 4 Memory = recid(ttCall) must-print = TRUE.
          NEXT LOOP.
       END.
     END. /* Search-4 */
...
PROCEDURE local-find-FIRST:
       ...
       IF ORder = 1 AND llPrePaid THEN FIND FIRST ttCall NO-LOCK NO-ERROR.
       ELSE IF order = 1 THEN FIND FIRST ttCall      USE-INDEX Date
       NO-LOCK NO-ERROR.
       ELSE IF order = 2 THEN FIND FIRST ttCall USE-INDEX BillCode
       WHERE ttCall.custnum > 0  NO-LOCK NO-ERROR.
                     
       ELSE IF order = 3 THEN FIND FIRST ttCall USE-INDEX CustNum
       WHERE ttCall.custnum > 0
       NO-LOCK NO-ERROR.
       ELSE IF order = 4 THEN FIND FIRST ttCall USE-INDEX CLI 
              NO-LOCK NO-ERROR.
       ELSE IF order = 5 THEN FIND FIRST ttCall USE-INDEX GsmBnr
       
       NO-LOCK NO-ERROR.   
END PROCEDURE.

PROCEDURE local-find-LAST:
       IF ORder = 1 AND llPrePaid THEN FIND LAST ttCall NO-LOCK NO-ERROR.
       ELSE IF order = 1 THEN FIND LAST ttCall      USE-INDEX Date
       NO-LOCK NO-ERROR.
       ELSE IF order = 2 THEN FIND LAST ttCall USE-INDEX BillCode
       WHERE ttCall.custnum > 0  NO-LOCK NO-ERROR.
       ELSE IF order = 3 THEN FIND LAST ttCall USE-INDEX CustNum
       NO-LOCK NO-ERROR.
       ELSE IF order = 4 THEN FIND LAST ttCall USE-INDEX CLI
       NO-LOCK NO-ERROR.
       ELSE IF order = 5 THEN FIND LAST ttCall USE-INDEX GsmBnr
       no-lock no-error.
       
END PROCEDURE.

PROCEDURE local-find-NEXT:
       IF llPrePaid THEN FIND NEXT ttCall NO-LOCK NO-ERROR.
       ELSE IF order = 1 THEN FIND NEXT ttCall      USE-INDEX Date
       NO-LOCK NO-ERROR.
       ELSE IF order = 2 THEN FIND NEXT ttCall USE-INDEX BillCode
       WHERE ttCall.custnum > 0  NO-LOCK NO-ERROR.
       ELSE IF order = 3 THEN FIND NEXT ttCall USE-INDEX CustNum
        NO-LOCK NO-ERROR.
       ELSE IF order = 4 THEN FIND NEXT ttCall USE-INDEX CLI
       NO-LOCK NO-ERROR.
       ELSE IF order = 5 THEN FIND NEXT ttCall USE-INDEX GsmBnr
       NO-LOCK NO-ERROR.   
END PROCEDURE.

PROCEDURE local-find-PREV:
       IF ORder = 1 AND llPrePaid THEN FIND PREV ttCall NO-LOCK NO-ERROR.
       IF order = 1 THEN FIND PREV ttCall      USE-INDEX Date
       NO-LOCK NO-ERROR.
       ELSE IF order = 2 THEN FIND PREV ttCall USE-INDEX BillCode 
       WHERE ttCall.custnum > 0  NO-LOCK NO-ERROR.
       ELSE IF order = 3 THEN FIND PREV ttCall USE-INDEX CustNum
       NO-LOCK NO-ERROR.
       ELSE IF order = 4 THEN FIND PREV ttCall USE-INDEX CLI
       NO-LOCK NO-ERROR.
       ELSE IF order = 5 THEN FIND PREV ttCall USE-INDEX GsmBnr
       NO-LOCK NO-ERROR. 
END PROCEDURE.
...
PROCEDURE local-find-others.
       Stime = STRING(ttCall.TimeStart,"HH:MM").
