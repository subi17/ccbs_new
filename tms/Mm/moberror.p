...
if llDoEvent THEN DO:
    ...
    ON F12 ANYWHERE DO:
        RUN Mc/eventview2.p(lhMobError).
    END.
END.
...
BROWSE:
   REPEAT WITH FRAME sel on ENDkey undo, return:
      ...
      IF ufkey THEN DO:
        ...
        Syst.Var:ufk[5]= 1961 Syst.Var:ufk[6]= 1962 Syst.Var:ufk[7]= 0 Syst.Var:ufk[8]= 8 Syst.Var:ufk[9]= 1
      ...
      IF order = 1 THEN DO:
        ...
        COLOR DISPLAY VALUE(Syst.Var:ccc) MobError.MobError WITH FRAME sel.
      END.
      ELSE IF order = 2 THEN DO:
        choose row MobError.MEName {Syst/uchoose.i} NO-ERROR WITH FRAME sel.
        COLOR DISPLAY VALUE(Syst.Var:ccc) MobError.MEName WITH FRAME sel.
      END.
     ...
     ELSE IF LOOKUP(Syst.Var:nap,"6,f6") > 0 THEN DO: 
       ...
       IF ok THEN 
       RUN Rate/error_rate.p(INPUT MobError.MobError).
       ufkey = true.
       RUN Syst/ufkey.p.
     ...
     ELSE IF LOOKUP(Syst.Var:nap,"5,f5") > 0 THEN DO : 
       ...
       WITH OVERLAY ROW 10 CENTERED 1 COLUMNS SIDE-LABELS TITLE " Error CDRs"
          FRAME fLimits.
       HIDE FRAME fLimits NO-PAUSE.

       RUN Mm/mobcallbr.p(INPUT  "post,Qty:" + STRING(liMaxQty),
                     INPUT  ldaFromDate,
                     INPUT  ldaToDate,
                     INPUT  0, 
                     INPUT  "",
...
PROCEDURE local-find-others.
   ...
   FOR EACH mobcdr USE-index ErrorCode where 
            mobcdr.ErrorCode = MobError.MobError no-lock.
      iqty = iqty + 1.
   ...
   IF iQty < 100 THEN 
   FOR EACH ErrorCDR WHERE
            ErrorCDR.ErrorCode = MobError.MobError NO-LOCK:
      iQty = iQty + 1.
      IF iQty >= 100 THEN LEAVE.
   END.
   ...
   IF iqty > 0 AND 
      LOOKUP(STRING(MobError.MobError),"1001,1002,1003,1004,2001,3001") > 0 
      THEN lccheck = TRUE.
