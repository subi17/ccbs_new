...
REPEAT WITH FRAME Main:
   ...
   REPEAT WITH FRAME Main:
      ASSIGN
      Syst.Var:ufk =  0 Syst.Var:ufk[1] = 7 Syst.Var:ufk[4] = 241
      Syst.Var:ufk[5] = 795 Syst.Var:ufk[8] = 8 Syst.Var:ehto = 0.  RUN Syst/ufkey.p.
      IF Syst.Var:toimi = 1 THEN NEXT  main.
      IF Syst.Var:toimi = 8 THEN LEAVE main.
      IF Syst.Var:toimi = 4 THEN LEAVE Action.
   ...
   PhaseLoop:
   DO  phase = 1 TO (IF Syst.Var:toimi = 4 THEN 1 ELSE 2):    /* 1: preceeding check;  
                                                        2: UPDATE */
      IF phase = 2 THEN 
      DO:
         /*******************************
   ...
   IF Syst.Var:toimi = 5 AND d-icc + d-IMSI = 0 AND NOT lError THEN 
      lRealUpdate = TRUE.

   IF Syst.Var:toimi = 5 OR d-icc + d-IMSI > 0 THEN LEAVE Main.
END. /* Main */
