...
class AddTableOrderFusion(Migration):
    ...
    def up(self):
        t = self.table('OrderFusion', area="Sta_Data_32", multitenant="yes", label="Order Fusion", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-orderfusion.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-orderfusion.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="orderfusion", desc="Order Fusion")
        ...
        t.column('FixedCurrOperCode', 'character', format="x(7)", initial="", max_width=14, label="FixedCurrOperCode", position=20, order=191)
        t.column('SerialNumber', 'character', format="x(15)", initial="", max_width=30, label="SerialNumber", position=21, order=200)
        t.column('AppointmentDate', 'character', format="x(40)", initial="", max_width=80, label="Appointment Date", column_label="AppointmentDate", position=22, order=210, description="Installation appointment info")
        t.column('CancellationReason', 'character', format="x(40)", initial="", max_width=80, label="Cancellation Reason", column_label="CancellationReason", position=23, order=220, description="Installation cancellation reason")
        t.column('FixedInstallationTS', 'decimal', format="99999999.99999", decimals=5, initial="0", max_width=20, label="FixedInstallationTS", position=24, order=230)
        t.column('ADSLLinkState', 'character', format="x(5)", initial="", max_width=10, label="ADSL Link State", column_label="ADSLLinkState", position=25, order=250)
        t.column('FixedStatusTS', 'decimal', format="99999999.99999", decimals=5, initial="0", max_width=20, label="FixedStatusTS", position=26, order=260)
        t.column('FusionStatusDesc', 'character', format="x(30)", initial="", max_width=60, label="Fusion Status Desc", column_label="FusionStatusDesc", position=27, order=270)
        t.column('EstimatedDataSpeed', 'integer', format=">>>>>>>>9", initial="0", max_width=4, label="EstDataSpeed", column_label="EstDataSpeed", position=28, order=190, help="Estimated Data Speed")
