...
class AddTableMSISDN(Migration):
    ...
    def up(self):
        t = self.table('MSISDN', area="Sta_Data_2_64", multitenant="yes", dump_name="msisdn-------1", desc="A Single MSISDN number")
        ...
        t.column('ActionDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="ActionDate", column_label="PortDate", position=13, order=28, help="Date when action happens")
        ...
        t.column('MSSeq', 'integer', format="zzzzzzzz9", initial=self.unknown, max_width=4, label="MSSeq", column_label="MSSEq", position=21, order=21, help="Sequence for subscriptions")
        ...
        t.column('PortingDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="PortDate", column_label="PortDate", position=24, order=120, help="Date when this MSISN no. was ported in or out")
        ...
        t.index('CLI', [['Brand'], ['CLI'], ['ValidFrom', 'DESC']], area="Sta_Index_4", primary=True, unique=True)
        t.index('ActionDate', [['Brand'], ['StatusCode'], ['ActionDate']], area="Sta_Index_4")
        t.index('CLI_s', [['CLI']], area="Sta_Index_4")
        t.index('CustNum', [['Brand'], ['CustNum'], ['ValidFrom', 'DESC']], area="Sta_Index_4")
        t.index('MSSeq', [['MSSeq'], ['ValidFrom', 'DESC']], area="Sta_Index_4")
        t.index('OrderID', [['Brand'], ['OrderID', 'DESC']], area="Sta_Index_4")
        t.index('POS', [['POS'], ['StatusCode'], ['CLI'], ['ValidFrom', 'DESC']], area="Sta_Index_4")
        t.index('POSDCLI', [['POS'], ['StatusCode'], ['CLI', 'DESC'], ['ValidTo', 'DESC']], area="Sta_Index_4")
        t.index('StatusCode', [['Brand'], ['StatusCode'], ['CLI'], ['ValidFrom', 'DESC']], area="Sta_Index_4")
