...
class AddTableMinConsumption(Migration):
    ...
    def up(self):
        t = self.table('MinConsumption', area="Sta_Data_128_2", multitenant="yes", label="Minimum Consumption", dump_name="minconsumption-------1", desc="Minimum consumption for a period")
        ...
        t.column('FromDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="From", position=3, order=20, help="Period begin")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="To", position=4, order=30, help="Period end")
        ...
        t.index('MsSeq', [['MSSeq'], ['ToDate', 'DESC']], area="Sta_Index_5", primary=True, unique=True)
        t.index('InvNum', [['InvNum']], area="Sta_Index_4")
