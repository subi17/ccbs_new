...
class AddTableMSRange(Migration):
    ...
    def up(self):
        t = self.table('MSRange', area="Sta_Data_2_128", multitenant="yes", dump_name="msrange", desc='''MSISDN Range
''')
        ...
        t.column('ReserveDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="ResDate", column_label="ResDate", position=5, order=40, help="Date when Range was Reserved")
        t.column('ExpireDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="ResEnds", column_label="ResEnds", position=6, order=50, help="Date When Reservation Expires if Not Used")
