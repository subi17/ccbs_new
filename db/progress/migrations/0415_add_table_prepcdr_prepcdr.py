...
class AddTablePrepCDR(Migration):

    database = "prepcdr"

    def up(self):
        t = self.table('PrepCDR', area="Dyn_Data_64", label="PrePaid CDR", dump_name="prepcdr")
...
''')
        ...
        t.column('ReadDate', 'date', format="99-99-99", max_width=4, label="Read In Date", column_label="Read", position=200, order=2480, help="Read in date")
        t.column('DCEvent', 'character', format="x(12)", initial="", max_width=24, label="Periodical Contract", column_label="P.Contract", position=201, order=2490)
        t.column('ReadTime', 'integer', format="zzzzz9", initial="0", max_width=4, label="ReadTime", column_label="ReadTime", position=202, order=2500, description="Time while collecting CDR's")
        t.column('Accumulator', 'integer', format=">>>>>>>>>9", initial="0", max_width=4, label="Accumulator", column_label="Accumulator", position=203, order=2510, help="Accumulator value")
