...
class AddTableIMSI(Migration):
    ...
    def up(self):
        t = self.table('IMSI', area="Sta_Data_64", multitenant="yes", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-imsi.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-imsi.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="imsi", desc='''IMSI number
''')
