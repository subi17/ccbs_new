...
class AddTableBItemGroup(Migration):
    ...
    def up(self):
        t = self.table('BItemGroup', area="Sta_Data_128", multitenant="yes", label="Product Groups", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-bitemgroup.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-bitemgroup.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="bitemgro", desc="Product Groups")
        ...
        t.index('BIGroup', [['Brand'], ['BIGroup']], area="Sta_Index_3", primary=True, unique=True)
        t.index('BIGName', [['Brand'], ['BIGName']], area="Sta_Index_3")
