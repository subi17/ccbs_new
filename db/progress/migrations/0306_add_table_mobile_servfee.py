...
class AddTableServFee(Migration):
    ...
    def up(self):
        t = self.table('ServFee', area="Sta_Data_128_2", multitenant="yes", label="ServFee", dump_name="servfee", desc='''Fees from activating services
''')
        ...
        t.column('FromDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="From", position=7, order=60, help="Valid from")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="To", position=8, order=70, help="Valid to")
