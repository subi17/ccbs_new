...
class AddTableProCommand(Migration):
    ...
    def up(self):
        ...
        t.column('ActivationTS', 'datetime-tz', format="99/99/9999 HH:MM:SS.SSS+HH:MM", initial=self.unknown, max_width=12, label="Activation", position=4, order=30, help="Date and time when procommand is sent to BUS/BPM")
        t.column('CompletedTS', 'datetime-tz', format="99/99/9999 HH:MM:SS.SSS+HH:MM", initial=self.unknown, max_width=12, label="Completed", position=5, order=40, help="Date and time when BUS/BPM response is received")
        ...
        t.column('ProCommandStatus', 'integer', format=">>9", initial="0", max_width=4, label="Status", position=9, order=80, help="Status: 0 -  New, 1 - Ongoing, 2 - Done")
        t.column('ProCommandTarget', 'character', format="x(8)", initial="", help="NB for Northbound, BPM for BPM", max_width=16, label="Target", position=10, order=90, description="NB for Northbound, BPM for BPM")
        ...
        t.index('Ix_MsSeq', [['MsSeq'], ['ActivationTS', 'DESC']], area="Sta_Index_2")
        t.index('Ix_OrderId', [['OrderId'], ['ActivationTS', 'DESC']], area="Sta_Index_2")
        t.index('Ix_Status', [['ProCommandStatus'], ['ActivationTS']], area="Sta_Index_2")
