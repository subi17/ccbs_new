...
class AddTableRequestAction(Migration):
    ...
    def up(self):
        t = self.table('RequestAction', area="Sta_Data_64", multitenant="yes", label="Request Actions", dump_name="requestaction", desc="Request actions")
        ...
        t.column('ValidFrom', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Valid From", column_label="From", position=5, order=40, help="Date when configuration becomes effective")
        t.column('ValidTo', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Valid To", column_label="To", position=6, order=50, help="Date when configuration expires")
        ...
        t.index('RequestActionID', [['RequestActionID']], area="Sta_Index_3", primary=True, unique=True)
        t.index('CLIType', [['Brand'], ['CLIType'], ['ReqType'], ['ValidTo', 'DESC']], area="Sta_Index_3")
        t.index('PayType', [['Brand'], ['PayType'], ['ReqType'], ['ValidTo', 'DESC']], area="Sta_Index_3")
        t.index('ReqType', [['Brand'], ['ReqType'], ['ValidTo', 'DESC']], area="Sta_Index_3")
