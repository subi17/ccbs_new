...
class AddTablePListConf(Migration):
    ...
    def up(self):
        t = self.table('PListConf', area="Sta_Data_128", multitenant="yes", dump_name="plistcon", desc='''Price configuration

''')
        t.column('dFrom', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="DateFrom", column_label="DateFrom", position=3, order=20, help="FROM date for price list validation")
        t.column('dTo', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="DateTo", column_label="DateTo", position=4, order=30, help="TO date for price list validation")
        ...
        t.index('RatePlan', [['Brand'], ['RatePlan'], ['dFrom', 'DESC'], ['dTo', 'DESC'], ['Prior']], area="Sta_Index_3", primary=True)
        t.index('Browse', [['Brand'], ['RatePlan'], ['dFrom'], ['dTo'], ['Prior']], area="Sta_Index_3")
