...
class AddTableppInvseq(Migration):
    ...
    def up(self):
        t = self.table('ppInvseq', area="Sta_Data_128", multitenant="yes", label="Invoice Sequence", dump_name="ppInvseq", desc="Customers invoice sequences")
        ...
        t.column('FromDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="DateFrom", column_label="DateFrom", position=3, order=20, help="FROM date for sequence")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="DateTo", column_label="DateTo", position=4, order=30, help="TO date for sequence")
