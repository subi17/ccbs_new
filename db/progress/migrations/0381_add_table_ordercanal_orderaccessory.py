...
class AddTableOrderAccessory(Migration):
    ...
    def up(self):
        t = self.table('OrderAccessory', area="Sta_Data_32", multitenant="yes", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-orderaccessory.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-orderaccessory.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="orderacc")
