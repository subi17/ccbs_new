...
class AddTableDiscountPlan(Migration):
    ...
    def up(self):
        t = self.table('DiscountPlan', area="Sta_Data_64", multitenant="yes", label="Discount Plans", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-discountplan.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-discountplan.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="discountplan")
        ...
        t.column('ValidFrom', 'date', format="99-99-9999", initial=self.unknown, max_width=4, label="Valid From", column_label="From", position=11, order=100, help="Effective from date")
        t.column('ValidTo', 'date', format="99-99-9999", initial=self.unknown, max_width=4, label="Valid To", column_label="To", position=12, order=110, help="Effective to date")
