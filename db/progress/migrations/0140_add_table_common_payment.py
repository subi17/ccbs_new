...
class AddTablePayment(Migration):
    ...
    def up(self):
        t = self.table('Payment', area="Dyn_Data_32", multitenant="yes", label="Payments", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-payment.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-payment.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="payment", desc="Payments")
