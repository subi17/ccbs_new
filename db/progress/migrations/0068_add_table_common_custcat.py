...
class AddTableCustCat(Migration):
    ...
    def up(self):
        t = self.table('CustCat', area="Sta_Data_128", multitenant="yes", label="Customer Category", dump_name="custcat", desc="Customer category")
...
''')
        ...
        t.column('PRO', 'logical', format="yes/no", initial="no", max_width=1, label="PRO", column_label="PRO", position=15, order=140, help="PRO customer")
        t.column('Segment', 'character', format="x(20)", initial="", max_width=40, label="Segment", column_label="Segment", position=16, order=150, help="Segment")
        t.index('Category', [['Brand'], ['Category']], area="Sta_Index_3", primary=True, unique=True)
        t.index('CatName', [['Brand'], ['CatName'], ['MaxCredit']], area="Sta_Index_3")
