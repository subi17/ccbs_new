...
class AddTableOrderProductParam(Migration):
    ...
    def up(self):
        t = self.table('OrderProductParam', area="Sta_Data_64", multitenant="yes", dump_name="orderproductparam")
        ...
        t.column('DateValue', 'date', format="99-99-9999", initial=self.unknown, max_width=4, label="DecValue", column_label="DecValue", position=6, order=50)
        t.column('DecValue', 'decimal', format="->>>>>>>9.99<<", decimals=4, initial="0", max_width=19, label="DecValue", column_label="DecValue", position=7, order=60)
        t.column('IntValue', 'integer', format="->>>>>>>9", initial="0", max_width=4, label="IntValue", column_label="IntValue", position=8, order=70)
        t.index('OrderProductID', [['OrderProductID'], ['ParamName']], area="Sta_Index_64", primary=True)
