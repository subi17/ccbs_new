...
class AddTableTopupSchemeRow(Migration):
    ...
    def up(self):
        t = self.table('TopupSchemeRow', area="Sta_Data_128", multitenant="yes", label="Topup Scheme Row", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-topupschemerow.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-topupschemerow.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="topupschemerow", desc="Topup scheme row")
        ...
        t.index('TopupSchemeRow', [['Brand'], ['TopupScheme'], ['TopupSchemeRowID']], area="Sta_Index_3", primary=True)
        t.index('EndStamp', [['Brand'], ['TopupScheme'], ['EndStamp', 'DESC']], area="Sta_Index_3")
        t.index('TopupSchemeRowID', [['TopupSchemeRowID']], area="Sta_Index_3", unique=True)
