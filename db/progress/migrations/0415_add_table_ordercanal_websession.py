...
class AddTableWebSession(Migration):
    ...
    def up(self):
        ...
        t.column('createdate', 'date', format="99.99.99", initial=self.unknown, max_width=4, position=5, order=40, help="Creation date (see also createtime)")
        t.column('createtime', 'integer', format=">>,>>>", initial="0", max_width=4, position=6, order=50, help="Creation time (see also createdate)")
        t.column('lastdate', 'date', format="99.99.99", initial=self.unknown, max_width=4, position=7, order=60, help="Last day user requested a page (see also lasttime)")
