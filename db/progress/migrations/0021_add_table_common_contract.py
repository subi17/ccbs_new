...
class AddTableContract(Migration):
    ...
    def up(self):
        t = self.table('Contract', area="Sta_Data_128", multitenant="yes", label="Contract", dump_name="contract", desc='''Customers' contracts
''')
        ...
        t.column('FromDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Valid From", column_label="From", position=4, order=30, help="Contract valid from")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Valid To", column_label="To", position=5, order=40, help="Contract valid to")
        ...
        t.column('CloseDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Closing Date", column_label="Closed", position=10, order=90, help="Actual date when contract was closed")
        ...
        t.index('Contract', [['Brand'], ['Contract']], area="Sta_Index_4", primary=True, unique=True)
        t.index('ContrType', [['Brand'], ['ContrType'], ['CloseDate', 'DESC']], area="Sta_Index_4")
        t.index('CustNum', [['Brand'], ['CustNum'], ['FromDate', 'DESC']], area="Sta_Index_4")
        t.index('CustNum_s', [['CustNum'], ['FromDate', 'DESC']], area="Sta_Index_4")
        t.index('FromDate', [['Brand'], ['FromDate', 'DESC'], ['CustNum']], area="Sta_Index_4")
        t.index('Salesman', [['Brand'], ['Salesman'], ['CustNum'], ['FromDate', 'DESC']], area="Sta_Index_4")
