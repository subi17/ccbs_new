...
class AddTableDPConf(Migration):
    ...
    def up(self):
        t = self.table('DPConf', area="Sta_Data_2_256", multitenant="yes", label="Volume Discount Config.", dump_name="dpconf", desc='''Configuration for volume discount. Additional configurations in DpBasis.
''')
        ...
        t.column('ValidFrom', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Begin Date", column_label="Begin Date", position=7, order=30, help="Date when discount becomes effective")
        ...
        t.column('ValidTo', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="End Date", column_label="EndDate", position=10, order=41, help="Date when discount becomes invalid")
        ...
        t.index('DpConfNum', [['DPConfNum']], area="Sta_Index_3", primary=True, unique=True)
        t.index('Basis', [['Brand'], ['DiscPlan'], ['DiscType'], ['ValidFrom', 'DESC']], area="Sta_Index_3")
        t.index('DPCName', [['Brand'], ['DPCName'], ['ValidFrom', 'DESC']], area="Sta_Index_3")
        t.index('ValidFrom', [['Brand'], ['DiscPlan'], ['ValidFrom', 'DESC']], area="Sta_Index_3")
