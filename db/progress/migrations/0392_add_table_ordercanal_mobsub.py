...
class AddTableMobSub(Migration):
    ...
    def up(self):
        t = self.table('MobSub', area="Sta_Data_32", multitenant="yes", label="Mobile Subscriber", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-mobsub.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-mobsub.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="mobsub-1", desc="Mobile Subscription")
        ...
        t.column('CreationDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Created", position=9, order=80, help="Date when subscription was created (from order)")
        ...
        t.column('ActivationDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Activation Date", column_label="Activation", position=15, order=140, help="Date when subscription is to be activated in HLR")
        ...
        t.column('SegmentDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Segmentation Date", column_label="SegmentDate", position=28, order=831, help="Date when segmentation code was changed")
        ...
        t.column('FixedNumber', 'character', format="x(11)", initial=self.unknown, max_width=22, label="Fixed Number", column_label="FixedNumber", position=37, order=895, help="Fixed line number")
        t.column('TerritoryOwner', 'character', format="x(15)", initial="", max_width=30, label="TerritoryOwner", column_label="TerritoryOwner", position=38, order=900, help="Territory Owner")
        t.column('CliType', 'character', format="x(13)", initial="", max_width=16, label="MType", column_label="Mtype", position=43, order=440, help="Type Of Mobsub")
        t.column('ActivationTS', 'decimal', format="99999999.99999", decimals=5, initial="0", max_width=20, label="Activated", position=48, order=480, help="Time when subscription was activated in TMS (HLR response OK)")
        ...
        t.column('SIMActDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="sadate", column_label="sadate", position=51, order=510, help="Date When Sim delevery request was saved")
...
''')
        ...
        t.index('FixedNumber', [['Brand'], ['FixedNumber']], area="Dyn_Index_1", unique=True)
