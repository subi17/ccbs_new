...
class AddTableLimit(Migration):
    ...
    def up(self):
        t = self.table('Limit', area="Limit_Data", multitenant="yes", label="Limit", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-limit.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-limit.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="limit-------1", desc="Limits for customer and subscription")
        ...
        t.column('TMRuleSeq', 'integer', format=">>>>>>>>9", initial="0", max_width=4, label="Rule Sequence", column_label="Seq", position=8, order=30, help="Rule ID")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="To", position=9, order=60, help="Date when rule expires")
        ...
        t.index('CustNum', [['CustNum'], ['LimitType'], ['TMRuleSeq'], ['LimitID'], ['ToDate', 'DESC']], area="Sta_Index_4", primary=True)
        t.index('LimitType', [['Brand'], ['LimitType'], ['LimitID'], ['ToDate', 'DESC']], area="Sta_Index_4")
        t.index('MsSeq', [['MsSeq'], ['LimitType'], ['TMRuleSeq'], ['ToDate', 'DESC']], area="Sta_Index_4")
        t.index('TMRuleSeq', [['TMRuleSeq'], ['LimitID'], ['ToDate', 'DESC']], area="Sta_Index_4")
