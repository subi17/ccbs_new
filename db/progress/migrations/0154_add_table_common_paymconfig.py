...
class AddTablePaymConfig(Migration):
    ...
    def up(self):
        t = self.table('PaymConfig', area="Sta_Data_64", multitenant="yes", label="Payment Configuration", dump_name="paymconfig", desc="Configuration rules for payments")
        ...
        t.index('PaymType', [['Brand'], ['PaymType'], ['PaymSrc'], ['ToDate', 'DESC']], area="Sta_Index_3", primary=True, unique=True)
        t.index('PaymConfig', [['PaymConfig']], area="Sta_Index_3", unique=True)
        t.index('PaymSrc', [['Brand'], ['PaymSrc'], ['PaymType'], ['ToDate', 'DESC']], area="Sta_Index_3")
