...
class AddTableInvRowCounter(Migration):
    ...
    def up(self):
        t = self.table('InvRowCounter', area="Sta_Data_64", multitenant="yes", label="Invoice Row Counter", dump_name="invrowcounter", desc="Invoice row counter")
        ...
        t.column('FromDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Events From", column_label="From", position=12, order=110, help="Beginning of event period")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Events To", column_label="To", position=13, order=120, help="Events to date")
