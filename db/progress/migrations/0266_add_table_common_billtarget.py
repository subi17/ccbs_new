...
class AddTableBillTarget(Migration):
    ...
    def up(self):
        t = self.table('BillTarget', area="Sta_Data_2_128", multitenant="yes", label="Billing Target", dump_name="billtarget", desc="Customer's Billing Target")
        ...
        t.column('RatePlan', 'character', format="x(15)", initial="", max_width=30, label="Rating Plan", column_label="RatePlan", position=15, order=130, help="Rating plan code")
        t.index('CustNum', [['CustNum'], ['BillTarget']], area="Sta_Index_3", primary=True, unique=True)
        t.index('RatePlan', [['RatePlan'], ['CustNum']], area="Sta_Index_3")
