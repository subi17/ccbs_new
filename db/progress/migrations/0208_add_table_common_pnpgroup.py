...
class AddTablePNPGroup(Migration):
    ...
    def up(self):
        t = self.table('PNPGroup', area="Sta_Data_64", multitenant="yes", dump_name="pnpgroup")
        ...
        t.column('dFrom', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="DateFrom", column_label="DateFrom", position=8, order=40, help="FROM date for PNP Group validation")
        t.column('dTo', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="DateTo", column_label="DateTo", position=9, order=50, help="TO date for PNP Group validation")
...
''')
        t.index('PNPGroup', [['Brand'], ['GroupType'], ['PNPGroup'], ['dTo', 'DESC']], area="Sta_Index_3", primary=True, unique=True)
        t.index('CCN', [['Brand'], ['CCN']], area="Sta_Index_3")
        t.index('Name', [['Brand'], ['GroupType'], ['Name'], ['PNPGroup']], area="Sta_Index_3")
        t.index('PNPSeq', [['Brand'], ['PNPSeq']], area="Sta_Index_3", unique=True)
