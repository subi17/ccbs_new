...
class AddTableMSRequest(Migration):
    ...
    def up(self):
        t = self.table('MSRequest', area="Sta_Data_64", multitenant="yes", label="MobSub Requests", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-msrequest.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-msrequest.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="msreques", desc='''MobSub related action requests
''')
