...
class AddTableMNPOperation(Migration):
    ...
    def up(self):
        t = self.table('MNPOperation', area="Sta_Data_64", multitenant="yes", dump_name="mnpoper")
        ...
        t.column('XMLRequest', 'clob', column_label="XMLRequest", clob_collation="basic", format="x(40)", initial=self.unknown, label="XML Request", lob_size="1M", lob_bytes=1048576, lob_area="Lob_Data", position=14, clob_type=1, order=100, clob_codepage="utf-8", help="XML request message")
        t.column('XMLResponse', 'clob', column_label="XML Response", clob_collation="basic", format="x(40)", initial=self.unknown, label="XMLResponse", lob_size="1M", lob_bytes=1048576, lob_area="Lob_Data", position=15, clob_type=1, order=110, clob_codepage="utf-8", help="XML response message")
        ...
        t.index('MNPOperation', [['MNPOperationID']], area="Sta_Index_3", primary=True, unique=True)
        t.index('CreatedTS', [['CreatedTS', 'DESC']], area="Sta_Index_4")
        t.index('ErrorCode', [['ErrorHandled'], ['ErrorCode']], area="Sta_Index_4")
        t.index('MNPSeq', [['MNPSeq'], ['CreatedTS']], area="Sta_Index_5")
