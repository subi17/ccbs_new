...
class AddTableServiceLimit(Migration):
    ...
    def up(self):
        t = self.table('ServiceLimit', area="Sta_Data_64", multitenant="yes", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-servicelimit.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-servicelimit.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="service1")
...
''')
        ...
        t.index('GroupCode', [['GroupCode']], area="Sta_Index_3", primary=True)
        t.index('SLSeq', [['SLSeq']], area="Sta_Index_3", unique=True)
