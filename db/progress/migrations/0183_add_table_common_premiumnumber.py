...
class AddTablePremiumNumber(Migration):
    ...
    def up(self):
        ...
        t.column('ValidFrom', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Valid From", column_label="ValidFrom", position=5, order=40, help="Date when configuration becomes active")
        t.column('ValidTo', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Valid To", column_label="ValidTo", position=6, order=50, help="Date after configuration becomes inactive")
