...
class AddTableNatHoliday(Migration):
    ...
    def up(self):
        t = self.table('NatHoliday', area="Sta_Data_128", label="National holidays", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-natholiday.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-natholiday.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="natholid", desc="National holidays (weekend prices used)")
