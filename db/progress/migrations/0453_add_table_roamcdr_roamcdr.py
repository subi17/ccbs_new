...
class AddTableRoamCDR(Migration):

    database = "roamcdr"

    def up(self):
        t = self.table('RoamCDR', area="Dyn_Data_64", dump_name="roamcdr")
        ...
        t.column('TSDump', 'decimal', format="99999999.99999", decimals=5, initial="0", max_width=20, label="TSDump", column_label="TSDump", position=19, order=180, help="TimeStamp when dumped to TAP3 file")
        t.index('DateRead', [['DateRead'], ['PLMN']], area="Dyn_Index1", primary=True)
        t.index('DateStart', [['DateStart'], ['TimeStart'], ['CLI']], area="Dyn_Index2")
        t.index('PLMN', [['PLMN'], ['DateStart', 'DESC']], area="Dyn_Index1")
