...
class AddTableReportConfRow(Migration):
    ...
    def up(self):
        t = self.table('ReportConfRow', area="Sta_Data_2_256", label="Report Configuration Row", dump_name="reportconfrow", desc="Report configuration row")
        ...
        t.column('DateValue', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Date Value", column_label="Date", position=6, order=50, help="Date value")
        ...
        t.column('FromDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Valid From", column_label="From", position=9, order=80, help="Date when configuration becomes effective")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="To", position=10, order=90, help="Date when configuration expires")
