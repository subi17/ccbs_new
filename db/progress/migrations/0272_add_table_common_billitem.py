...
class AddTableBillItem(Migration):
    ...
    def up(self):
        t = self.table('BillItem', area="Sta_Data_64", multitenant="yes", label="Billing Items", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-billitem.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-billitem.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="billitem", desc="Billing items")
...
''')
        ...
        t.index('BillCode', [['Brand'], ['BillCode']], area="Sta_Index_3", primary=True, unique=True)
        t.index('BIGroup', [['Brand'], ['BIGroup'], ['BillCode']], area="Sta_Index_3", unique=True)
        t.index('BIName', [['Brand'], ['BIName']], area="Sta_Index_3")
