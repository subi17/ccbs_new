...
class AddTableOrderAction(Migration):
    ...
    def up(self):
        t = self.table('OrderAction', area="Sta_Data_128", label="Order Action", table_trigger=[{'crc': '?', 'procedure': 'rd-orderaction.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'rw-orderaction.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="orderaction", desc="Order action")
