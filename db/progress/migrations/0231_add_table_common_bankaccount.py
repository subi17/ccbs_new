...
class AddTableBankAccount(Migration):
    ...
    def up(self):
        t = self.table('BankAccount', area="Sta_Data_64", multitenant="yes", label="Company's Bank Accounts", dump_name="bankacco", desc="Company's bank accounts")
        ...
        t.column('DDAllocation', 'decimal', format="ZZ9.99", decimals=2, initial="0", max_width=17, label="DDAllocation", column_label="DDAllocation", position=14, order=130, help="Direct debit allocation value")
        t.index('UnitBank', [['Brand'], ['UnitCode'], ['BankAccount']], area="Sta_Index_3", primary=True, unique=True)
        t.index('AccNum', [['Brand'], ['BankAccount']], area="Sta_Index_3")
