...
class AddTableFFItem(Migration):
    ...
    def up(self):
        t = self.table('FFItem', area="Sta_Data_64", multitenant="yes", label="Contract's Billing Periods", dump_name="ffitem", desc="Monthly item (one billable transaction) of a contract fee")
        ...
        t.column('Amt', 'decimal', format="->>>,>>9.999", decimals=3, initial="0", max_width=17, label="Amount", column_label="Amount", position=5, order=40, help="Payable Contract Payment per bill")
        t.column('OwnCost', 'decimal', format="->>>,>>9.999", decimals=3, initial="0", help="Our own costs per bill", max_width=17, label="Our costs", column_label="Our costs", position=6, order=50, description="Our own costs per bill")
