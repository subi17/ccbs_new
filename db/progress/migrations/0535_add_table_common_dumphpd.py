...
class AddTableDumpHPD(Migration):
    ...
    def up(self):
        ...
        t.column('UnitsToDump', 'integer', format=">>>>>>>9", initial="0", max_width=4, label="Units to dumps", column_label="UnitsToDump", position=7, order=60, help="How many unit to dump. Zero value means no restriction.")
        ...
        t.column('MaxRecordsPerFile', 'integer', format=">>>>>>>9", initial="0", max_width=4, label="Max records per file", column_label="MaxRecords", position=2, order=10, help="Maximum amount of record per a file (0=unlimited)")
