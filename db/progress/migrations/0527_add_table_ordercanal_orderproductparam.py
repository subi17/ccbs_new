...
class AddTableOrderProductParam(Migration):
    ...
    def up(self):
        t = self.table('OrderProductParam', area="Sta_Data_128", multitenant="yes", dump_name="orderproductparam", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-orderproductparam.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-orderproductparam.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}])
        ...
        t.index('OrderProductID', [['OrderProductID'], ['ParamName']], area="Dyn_Index_1", primary=True, unique=True)
