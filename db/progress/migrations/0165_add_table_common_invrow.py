...
class AddTableInvRow(Migration):
    ...
    def up(self):
        t = self.table('InvRow', area="Dyn_Data_64", multitenant="yes", label="Invoice Rows", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-invrow.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-invrow.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="invrow", desc="Invoice rows")
        t.column('InvNum', 'integer', format="zzzzzzzz9", initial="0", max_width=4, label="InvoiceNo", column_label="InvoiceNo", position=2, order=10, help="Consecutive Invoice Number, 1 ... 99999999")
        ...
        t.column('FromDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="FirstCall", column_label="FirstCall", position=7, order=60, help="Date of first billed call of this type ")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="LastCall", column_label="LastCall", position=8, order=70, help="Date of last billed call of this type")
