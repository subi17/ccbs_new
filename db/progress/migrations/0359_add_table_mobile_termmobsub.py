...
class AddTableTermMobSub(Migration):

    database = "mobile"

    def up(self):
        t = self.table('TermMobSub', area="Sta_Data_32", multitenant="yes", label="Mobile Subscriber", dump_name="Termms", desc="Mobile Subscription")
        ...
        t.column('SegmentDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Segmentation Date", column_label="SegmentDate", position=28, order=830, help="Date when segmentation code was changed")
        t.column('SegmentOffer', 'character', format="x(8)", initial="", max_width=16, label="Segmentation Renewal Offer", column_label="SegmentOffer", position=29, order=840)
        t.column('MultiSimID', 'integer', format=">>>>>>>9", initial="0", max_width=4, label="Multi Sim ID", column_label="Multi Sim ID", position=31, order=861, help="Multi Sim ID")
        t.column('MultiSimType', 'integer', format=">>>9", initial="0", max_width=4, label="Multi Sim Type", column_label="Multi Sim Type", position=32, order=870, help="Multi Sim type (1=primary, 2=secondary)")
        t.column('BarrCode', 'character', format="x(10)", initial="", max_width=20, label="BarrCode", column_label="BarrCode", position=33, order=880)
        t.column('TariffBundle', 'character', format="x(10)", initial="", max_width=20, label="Tariff Bundle", column_label="TariffBundle", position=34, order=890)
        t.column('TariffActDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Tariff Activation Date", column_label="TariffActDate", position=35, order=900)
        t.column('TariffActTS', 'decimal', format="99999999.99999", decimals=5, initial="0", max_width=20, label="TariffActTS", column_label="TariffActTS", position=36, order=860)
        t.column('FixedNumber', 'character', format="x(11)", initial=self.unknown, max_width=22, label="Fixed Number", column_label="FixedNumber", position=37, order=910, help="Fixed line number")
        t.column('TerritoryOwner', 'character', format="x(15)", initial="", max_width=30, label="TerritoryOwner", column_label="TerritoryOwner", position=38, order=920, help="Territory Owner")
        t.column('CliType', 'character', format="x(13)", initial="", max_width=16, label="MType", column_label="Mtype", position=43, order=440, help="Type Of TermMobSub")
...
''')
        ...
        t.column('PayType', 'logical', format="PrePaid/PostPaid", initial="no", max_width=1, label="PayType", column_label="PayType", position=80, order=800, help="PayType")
        t.column('SegmentCons', 'decimal', format="999999.99999", decimals=5, initial="0", max_width=20, label="SegmentCons", position=81, order=850, help="Average consumption in euros")
        t.index('MsSeq', [['MsSeq']], area="Sta_Index_1", primary=True)
        ...
        t.index('FixedNumber', [['Brand'], ['FixedNumber']], area="Sta_Index_1")
        ...
        t.index('MultiSimID', [['Brand'], ['MultiSimID', 'DESC']], area="Sta_Index_1")
