...
class AddTableRepText(Migration):
    ...
    def up(self):
        t = self.table('RepText', area="Sta_Data_128", multitenant="yes", label="Report Texts", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-reptext.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-reptext.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="reptext", desc="Report texts")
        ...
        t.index('LinkCode', [['Brand'], ['TextType'], ['LinkCode'], ['Language'], ['ToDate', 'DESC']], area="Sta_Index_3", primary=True, unique=True)
        t.index('Language', [['Brand'], ['TextType'], ['Language'], ['LinkCode']], area="Sta_Index_3")
