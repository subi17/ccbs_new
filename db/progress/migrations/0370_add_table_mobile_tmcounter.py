...
class AddTableTMCounter(Migration):
    ...
    def up(self):
        t = self.table('TMCounter', area="TMCounter_Data", multitenant="yes", label="TM Counter", dump_name="tmcounter-------1", desc="TM counter")
        ...
        t.column('TMRuleSeq', 'integer', format=">>>>>>>>9", initial="0", max_width=4, label="Rule Sequence", column_label="Seq", position=6, order=10, help="Rule ID")
        ...
        t.column('Amount', 'decimal', format="->>>>>>9.99999", decimals=5, initial="0", max_width=20, label="Amount", column_label="Amt", position=8, order=70, help="Amount")
        ...
        t.index('MsSeq', [['MsSeq'], ['TMRuleSeq'], ['ToDate', 'DESC']], area="Sta_Index_3", primary=True)
        t.index('CustNum', [['CustNum'], ['TMRuleSeq'], ['ToDate', 'DESC']], area="Sta_Index_3")
