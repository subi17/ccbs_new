...
class AddTableOrderPayment(Migration):
    ...
    def up(self):
        t = self.table('OrderPayment', area="Sta_Data_32", multitenant="yes", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-orderpayment.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-orderpayment.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="orderpay")
