...
class AddTableMNPCancelProposal(Migration):
    ...
    def up(self):
        t = self.table('MNPCancelProposal', area="Sta_Data_128", multitenant="yes", dump_name="mnpcancel")
        ...
        t.column('Pdf', 'blob', format="x(8)", initial=self.unknown, lob_size="1M", lob_bytes=1048576, lob_area="Lob_Data", position=7, order=60)
        t.index('MNPSeq', [['MNPSeq'], ['CreatedTS', 'DESC']], area="Sta_Index_4", primary=True)
        t.index('StatusCode', [['StatusCode'], ['CreatedTS', 'DESC']], area="Sta_Index_4")
