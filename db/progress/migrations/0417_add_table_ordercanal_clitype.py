...
class AddTableCLIType(Migration):
    ...
    def up(self):
        t = self.table('CLIType', area="Sta_Data_2_128", multitenant="yes", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-clitype.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-clitype.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="mobtype", desc='''


''')
        t.column('Clitype', 'character', format="x(13)", initial="", max_width=16, label="CLI Type", column_label="CLIType", position=2, order=10, help="CLI Type")
        ...
        t.index('Clitype', [['Brand'], ['Clitype']], area="Sta_Index_3", primary=True, unique=True)
        t.index('CliType_s', [['Clitype']], area="Sta_Index_3")
