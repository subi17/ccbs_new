...
class AddTablexxAuthLog(Migration):

    database = "common"

    def up(self):
        t = self.table('xxAuthLog', area="Sta_Data_128", label="AuthLog", dump_name="authlogx", desc="3rd party user authentication log")
        ...
        t.column('TimeStamp', 'datetime', format="99-99-9999 HH:MM:SS", initial=self.unknown, max_width=8, label="TimeStamp", column_label="TimeStamp", position=8, order=70)
        t.column('ResponseTS', 'datetime', format="99-99-9999 HH:MM:SS", initial=self.unknown, max_width=8, label="ResponseTS", column_label="ResponseTS", position=9, order=80)
        ...
        t.index('TimeStamp', [['TimeStamp', 'DESC']], area="Sta_Index_2", primary=True)
        t.index('Username', [['Username']], area="Sta_Index_2")

    def down(self):
        self.drop_table('xxAuthLog')
