...
class AddTableOrderAction(Migration):
    ...
    def up(self):
        t = self.table('OrderAction', area="Sta_Data_128", multitenant="yes", label="Order Action", table_trigger=[{'crc': '?', 'procedure': 'triggers/rd-orderaction.p', 'override_proc': True, 'event': 'REPLICATION-DELETE'}, {'crc': '?', 'procedure': 'triggers/rw-orderaction.p', 'override_proc': True, 'event': 'REPLICATION-WRITE'}], dump_name="orderaction", desc="Order action")
        ...
        t.index('OrderId', [['Brand'], ['OrderId']], area="Sta_Index_3", primary=True)
