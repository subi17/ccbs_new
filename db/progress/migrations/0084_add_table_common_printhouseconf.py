...
class AddTablePrintHouseConf(Migration):
    ...
    def up(self):
        ...
        t.column('FromDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="Effective Date", column_label="Eff.Date", position=5, order=40, help="Date when rule becomes effective")
        t.column('ToDate', 'date', format="99-99-99", initial=self.unknown, max_width=4, label="End Date", column_label="To", position=6, order=50, help="Date when rule expires")
