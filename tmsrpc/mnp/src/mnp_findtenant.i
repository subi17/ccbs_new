...
DO ON ERROR UNDO, THROW:
    ...
    FOR EACH {3} WHERE &IF {1} &THEN {3}.Brand = Syst.CUICommon:gcBrand AND &ENDIF {3}.{4} = {5} TENANT-WHERE TENANT-ID() > -1 NO-LOCK
        ON ERROR UNDO, THROW:
    ...
    FOR FIRST {3} WHERE &IF {1} &THEN {3}.Brand = Syst.CUICommon:gcBrand AND &ENDIF {3}.{4} = {5} TENANT-WHERE TENANT-ID() > -1 NO-LOCK:
