...
import xmlrpclib
from confluence_doc_conf_vip import applications
import os
import re
import sys
...
def get_docs(appname):
    allfiles = [fn for fn in os.listdir(os.path.join(appname, 'rpcmethods/external/viptool'))
                   if re.search(r'\.p$', fn)]
    allfiles.sort()
    return '\n'.join([DocFile(os.path.join(appname, 'rpcmethods/external/viptool', fn)).to_confluence() \
                      for fn in allfiles])
