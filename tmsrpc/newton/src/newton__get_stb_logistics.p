...
FOR EACH OrderDelivery WHERE
   ...
   STRING(OrderDelivery.LOStatusID) BEGINS {&LO_STATUS_TV_STB_PREFIX} NO-LOCK:
   ...
   add_int(od_struct, "lo_status_id", INT(SUBSTRING(STRING(OrderDelivery.LOStatusId), LENGTH({&LO_STATUS_TV_STB_PREFIX}) + 1))).
