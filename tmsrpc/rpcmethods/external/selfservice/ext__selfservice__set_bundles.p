...
DEF VAR ldaBonoVoipRenovalDate AS DATE NO-UNDO.
...
ldaBonoVoipRenovalDate   = DATE(fCParamC("BonoVoipRemovalDate")).
...
IF TODAY LT ldaBonoVoipRenovalDate AND 
   LOOKUP(pcBundleId,lcBONOContracts + ",BONO_VOIP") = 0 AND
   pcBundleId <> {&DSS}
THEN RETURN appl_err("Incorrect Bundle Id").
ELSE IF LOOKUP(pcBundleId,lcBONOContracts) = 0 AND pcBundleId <> {&DSS}
   THEN RETURN appl_err("Incorrect Bundle Id").
...
CASE pcActionValue :
   ...
   WHEN "off" THEN DO: 
      ...
      ELSE IF TODAY LT ldaBonoVoipRenovalDate AND 
